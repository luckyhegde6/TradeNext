...
**In progress — v3.41.0 Spec 16 Decision Engine CORE (engine + POC A/B) — CODE + VERIFIED, docs DONE, commit pending approval** (2026-09-24): branch `feature/ph22-decision-engine` on `2909b22` (v3.40.8 spike VERDICT APPROVE). User approved "Build engine core + POC A/B" → `lib/services/decision/` **Laya-only** mock engine (types/provider/gate/layaProvider/client/fusion; `DECISION_PROVIDER=none|laya`, unknown → warn+coerce none; confidence-gated ACT/REVIEW via `shapeConfidence`) + audit tags `DECISION_EVALUATED`/`DECISION_GATE` + POC A screener scoring + POC B Swing `gateAutoGenerate` (`DECISION_POC_ENABLED`-gated, off = byte-identical) + `POST /api/decision/evaluate` + `GET /api/admin/decision/ping` + OpenAPI (tag 'Decision Engine') + admin panel + `check-tsc-baseline` harness ENOENT fix (**Lesson 135**). `typesafeProvider.ts` deleted (Jev docs-only). Verified: `tsc` **46 exact baseline** · lint **0** · tests **107/107 (1401 pass / 4 skip / 0 fail)** · quickbuild **188/188**. Full detail: `.agents/changelog/versions-v3.41.md`. **Commit as v3.41.0 pending user approval (no push/PR).**
>
