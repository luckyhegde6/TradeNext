...
| **Prisma Postgres migration planning — standalone Accelerate RETIRES Dec 1, 2026** | [ ] **INFORMATIONAL / PLANNING** (investigated 2026-09-04 on `v3.26.0-prod-failure-triage`): `@prisma/extension-accelerate` is installed but **never used** → `cacheStrategy` query caching is NOT active (client uses legacy `new PrismaClient({ accelerateUrl })` = proxy-only transport). Standalone Accelerate (`prisma+postgres://accelerate.prisma-data.net`) is **retired Dec 1, 2026** (~3 months). Follow-ups: (a) optional short-term win — wire `withAccelerate()` + `cacheStrategy` on high-frequency direct-Prisma reads; (b) **REQUIRED** — spec+plan the move to Prisma Postgres (Accelerate caching built-in; `DATABASE_URL`+`DIRECT_URL` already documented in v3.20.5). See BUGS.md #14. | Planning |
