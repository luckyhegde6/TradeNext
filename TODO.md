...
**In progress — v3.41.1 Spec 17 Decision Engine monitoring + e2e hardening — CODE + VERIFIED, docs DONE, commit pending approval** (2026-09-24): branch `feature/ph22-decision-engine` on `ab6fd65` (v3.41.0 COMMITTED). NEW `lib/services/decision/monitoring.ts` (zero-Prisma in-memory ring buffer max 500 + stats aggregation) wired into client evaluate/ping + POC A screener + POC B swing (`DECISION_TRACE_ENABLED`-gated, off = byte-identical) + `GET /api/admin/decision/monitoring` (stats\|traces) + `DELETE` clear + `DECISION_MONITORING_CLEARED` audit + OpenAPI + **Decision Engine tab in AI Monitoring** (6 stat cards, breakdowns, trace rows, Clear). e2e hardening: NEW `e2e/decision-monitoring.spec.ts` — **env-only creds** (no credential literals — user directive), serial, resilient `loginAsAdmin()` (**first WebKit login in the suite** — Lesson 136: server-side signIn 302+Set-Cookie never completes client-side under single-threaded dev-server load; 3 attempts + URL probe + reload→session re-check) + `pingAdmin()` inline retry + strict `.last()` on duplicated labels. Verified: `tsc` **46 exact baseline** · lint **0** · tests **108/108 (1416 pass / 4 skip / 0 fail)** · quickbuild **189/189** · full e2e **94 passed / 1 flaky (pre-existing nav flake) / 0 failed**. Full detail: `.agents/changelog/versions-v3.41.md`. **Commit as v3.41.1 pending user approval (no push/PR).**
>
