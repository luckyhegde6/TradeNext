...
| Category | Status |
|----------|--------|
| **DB Health ops visibility — SQLite ops-counter persistence + Total Operations/Plan Usage UI + sql.js WASM fix (v3.21.1)** | [x] Complete (on `main`, 7 files): **Live-site bug FIX** — `/admin/utils/db-health` showed "SQLite Not Ready"; sql.js is a native/WASM module → `next.config.ts` `serverExternalPackages: ['sql.js']` + `lib/sqlite.ts` `resolveSqlWasm()` (`node_modules/sql.js/dist` → `public/`) wired into `initSqlJs({ locateFile })`. **IO-count reconciliation (user-approved "Display + persist")** — Prisma dashboard Total Operations is authoritative (reads+writes through the proxy) vs in-memory `dbOpsCounter` (resets on every deploy); `lib/prisma.ts` exports `getIstDayKey`; `lib/sqlite.ts` adds `persistOpsCounter()`/`restoreOpsCounter()` (key `ops_counter` in `_backup_meta`, IST-day guard + `Math.max` merge, restore at init + after initial sync) + 60s `startOpsCounterPersistence()` booted from `instrumentation.ts`; `/api/admin/db-health` GET returns `totalOperations`/`planLimit` (env `DB_PLAN_LIMIT_OPS`, default 10,000)/`planOperationsRemaining` + persists (POST sync too). UI: 6th "Total Ops Today" stat + "Plan Operations Usage" bar (reads vs writes vs plan, remaining, footnote) + "Plan Ops n% Used" badge > 80%. Tests: `sqlite.test.ts` mock fixes (`getIstDayKey`, `exec()` column projection, `INSERT OR REPLACE` semantics) + 3 new → **suite 920 pass / 4 skip** (was 917/4); tsc 46 = baseline (0 new production errors). **Docs updated; commit pending user (no auto-commit/deploy)** |
