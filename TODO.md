...
> Live / in-progress work is tracked in **`.agents/session-todos.md`**.
...
**In progress — v3.41.3 Spec 18 Laya real inference P1–P3 + real provider path — CODE + TESTS + VERIFIED, docs pass in progress, commit pending user approval** (2026-09-25): branch `feature/ph22-decision-engine` on `5b088e3` (v3.41.2 COMMITTED; parent `a269057` = v3.41.1 COMMITTED; grandparent `ab6fd65` = v3.41.0). Real Laya inference runtime behind a parity gate (`DECISION_LAYA_REAL=1` → `LayaRealProvider`; else laya-mock default, byte-identical): `lib/services/laya/` P1 pure-TS ports (version/qtypes/serialize/calibration/collate/presets/email/buildSequence/lang/router, Python 1:1) · P2 `tokenizer.ts` WASM lazy-singleton via @huggingface/tokenizers (`add_special_tokens=false` encode parity; v1 special ids CLS 50281 / SEP 50282 / PAD 50283 / MASK 50284; `DECISION_LAYA_MODEL_DIR` override) · P3 `decisionModel.ts` two chained ONNX InferenceSessions encoder_q8→head_q8 (onnxruntime-node@1.30.0; int64/bool casts; K≥2 pad clamp; `act_logits` graph-name read — spike camelCase-key bug corrected; crypto probe hidden-1024 fail-fast; lazy singleton; child-process probe `scripts/dev-checks/laya-forward.ts`; weights `lib/services/laya/weights/v1/` gitignored + `scripts/fetch-laya-weights.mjs`) · minimal `agent.ts` systemOne decode (buildSequence→collate→forward→temperature→max-sub softmax→choice/score/noul + confidence 4dp + usage). NEW 7 laya suites + decisionClient/layaProvider updates. Verified: tsc **46 exact (0 new)** · lint **0 (1155 pre-existing warnings; Lesson 139 flat-config disable-directive fix)** · **116/116 suites (1538 pass / 4 skip / 0 fail)** · quickbuild **189/189** ✓ · doc budget 85.4/100 KB ✓ · live in-process real ping (503 MB chain — `laya: ok`) + HTTP ping 200 user-accepted (plan step 21). Full detail: `.agents/changelog/versions-v3.41.md` §v3.41.3. **Commit as v3.41.3 pending user approval (no push/PR).**
>
**v3.41.2 Spec 01 Recommendations plan-limit fallbacks (History/Performance/Ideas) + HistoryTab error state — COMMITTED `5b088e3`** (2026-09-25): branch `feature/ph22-decision-engine` on `a269057` (v3.41.1 COMMITTED). P6003 plan-limit hold (until ~2026-10-02) ⇒ prod `/recommendations` History/Performance/Ideas 500'd on Prisma-only reads while the SQLite mirror held the same fresh rows. Fix: `lib/sqlite.ts` NEW `getRecommendationRuns()` → `top-stocks/route.ts` `topStocksFromSqlite()` fallback (serialize → 1 h cache) → `recommendationPerformanceService.ts` `listItemFromMirrorTracker`/`getPerformanceListFromSqlite` (JS status filter, timerange) + wrapped Prisma branch → `syncedDataService.ts` breaker/hold-aware `mirrorWriteThrough` + `mirrorReadMarketCache` (`isPlanLimitBreakerOpen()` + `getDbHealthState` + DateTime-safe cache key). Contract: zero Prisma ops in fallbacks; degraded `[]` only when mirror HAS data — mirror-exhausted/not-ready RETHROWS the ORIGINAL 500 (Lesson 138: never mask a 500 as empty); only `isDbUnavailableError` caught; Prisma happy path byte-identical. **HistoryTab error-card + Retry** (was masked-500-as-empty-state). NEW `recommendationsPlanLimitFallbacks.test.ts` **21/21**; verified: `tsc` **46 exact (0 new)** · lint **0** · tests **109/109 (1440 pass / 4 skip / 0 fail)** · quickbuild **189/189** ✓ · e2e **recommendations.spec.ts 10/10** (live dev server). Full detail: `.agents/changelog/versions-v3.41.md` §v3.41.2. **Committed `5b088e3` (no push/PR).**
>
**v3.41.1 Spec 17 Decision Engine monitoring + e2e hardening — COMMITTED `a269057`** (2026-09-24): branch `feature/ph22-decision-engine` on `ab6fd65` (v3.41.0 COMMITTED). NEW `lib/services/decision/monitoring.ts` (zero-Prisma in-memory ring buffer max 500 + stats aggregation) wired into client evaluate/ping + POC A screener + POC B swing (`DECISION_TRACE_ENABLED`-gated, off = byte-identical) + `GET /api/admin/decision/monitoring` (stats\|traces) + `DELETE` clear + `DECISION_MONITORING_CLEARED` audit + OpenAPI + **Decision Engine tab in AI Monitoring** (6 stat cards, breakdowns, trace rows, Clear). e2e hardening: NEW `e2e/decision-monitoring.spec.ts` — **env-only creds** (no credential literals — user directive), serial, resilient `loginAsAdmin()` (**first WebKit login in the suite** — Lesson 136: server-side signIn 302+Set-Cookie never completes client-side under single-threaded dev-server load; 3 attempts + URL probe + reload→session re-check) + `pingAdmin()` inline retry + strict `.last()` on duplicated labels. **Auth-gate hardening (Lesson 137: Auth.js double-submit CSRF race → in-context 2-attempt resubmit loop in e2e/auth.setup.ts + e2e/login.spec.ts)**. Verified: `tsc` **46 exact baseline** · lint **0** · tests **108/108 (1416 pass / 4 skip / 0 fail)** · quickbuild **189/189** · full e2e **94 passed / 1 flaky (pre-existing nav flake) / 0 failed**. Full detail: `.agents/changelog/versions-v3.41.md`. **Superseded by v3.41.2 (above).**
>
**v3.41.0 Spec 16 Decision Engine CORE (engine + POC A/B) — COMMITTED `ab6fd65`** (2026-09-24): branch `feature/ph22-decision-engine` on `2909b22` (v3.40.8 spike VERDICT APPROVE). User approved "Build engine core + POC A/B" → `lib/services/decision/` **Laya-only** mock engine (types/provider/gate/layaProvider/client/fusion; `DECISION_PROVIDER=none|laya`, unknown → warn+coerce none; confidence-gated ACT/REVIEW via `shapeConfidence`) + audit tags `DECISION_EVALUATED`/`DECISION_GATE` + POC A screener scoring + POC B Swing `gateAutoGenerate` (`DECISION_POC_ENABLED`-gated, off = byte-identical) + `POST /api/decision/evaluate` + `GET /api/admin/decision/ping` + OpenAPI (tag 'Decision Engine') + admin panel + `check-tsc-baseline` harness ENOENT fix (**Lesson 135**). `typesafeProvider.ts` deleted (Jev docs-only). Verified: `tsc` **46 exact baseline** · lint **0** · tests **107/107 (1401 pass / 4 skip / 0 fail)** · quickbuild **188/188**. Full detail: `.agents/changelog/versions-v3.41.md`. **Superseded by v3.41.1 (above).**
>
**In progress — v3.40.5 Laya / System-One decision-engine RESEARCH (docs-only, DONE; engine not built)** (2026-09-22): 5 parallel research subagents scraped Laya HF card + TypeSafe.JS docs + Medium → **Jev premise CORRECTED** (`github.com/answers-ai/jev` = 404 — Jev 1.13.0 is TypeSafe AI's **proprietary hosted** System One API: `POST https://api.typesafe.ai/v1/systemone` via `@typesafe-ai/sdk` v0.6.0, Node 20+, env `TYPESAFE_API_KEY`/`BASE_URL`/`DEFAULT_MODEL=jev-latest`/`LOG_LEVEL`). Primitives Choice/Score/Noul + confidence-gated routing; 4 patterns (fan-out, confidence routing, composite scoring, intent routing). Durable docs: `memory.md`, `docs/laya.md`, `docs/designDoc/ph22-laya-decision-engine-design.md`. User-requested Jev docs: **NEW `docs/jev.md`** (full reference + from-scratch implementation) + **NEW `docs/jev-tradenext-integration.md`** (Jev SDK usage for TradeNext + integration) + **NEW `docs/jev.html`** (visual Mermaid guide). Engine (ph22) = spec + plan + human approval, then `npm i @typesafe-ai/sdk` + live spike. Full detail: `.agents/changelog/versions-v3.40.md` §v3.40.5. **SUPERSEDED — engine implemented Laya-only as v3.41.0 (above); no `@typesafe-ai/sdk`; Jev stays docs-only.**
>
> **In progress — v3.40.3 predeploy mirror-preservation guard** (branch `feature/predeploy-mirror-preserve`, on `main` `27c0770` = v3.40.2 MERGED via PR #129 `38ee7da`; v3.40.1 MERGED via PR #128 `15fa0a3`): P6003 plan-limit hold (until 2026-10-02) ⇒ a production deploy could cold-start with an EMPTY SQLite mirror (same served-empty class as v3.40.1). Fix: `netlify.toml` build = `node scripts/predeploy/preserve-mirror.mjs && npx prisma generate && npm run quickbuild` — the build-time guard calls LIVE `POST /api/admin/predeploy/preserve` (`x-deploy-guard-token` vs 64-hex `DEPLOY_GUARD_TOKEN` via `timingSafeEqual` + admin-session fallback): `getOutboxPending()` → snapshot (persist+export+upload Blobs) → versioned backup (`backups/sqlite-mirror-<ISO>.sqlite`, keep 5) → **breaker-closed only** `pushSqliteToPrisma({reason:"deploy", leaderGate:false})` → audit `ADMIN_DB_SYNC`; modes `pushed|backed_up|skipped` (hold → `backed_up`); soft-fail exit 0. CODE + TESTS DONE & VERIFIED: **99/99 suites / 1334 pass / 4 skip / 0 fail** (25 new), `tsc` 46 exact (prod 0), lint 0 errors, doc budget 74.9/100 KB, `quickbuild` 185/185; `DEPLOY_GUARD_TOKEN` upserted via Netlify MCP. Docs phase 8 done (Lessons **132/133**, `versions-v3.40.md` §v3.40.3 + §v3.40.1/§v3.40.2 MERGED status fixes, CHANGELOG addenda, Primer, agent-memory, session-todos, handoff). Legacy finding (record-only): `.github/workflows/deploy.yml` publishes `./out` via `nwtgck/actions-netlify@v3.0` — NOT the real netlify.toml → quickbuild → `.next` path. **Commit/push/PR/deploy pending user**. Full detail: `.agents/changelog/versions-v3.40.md` §v3.40.3.
>
> **v3.40.0 agentic context + orchestration + harness — MERGED (PR #127) + LIVE in production** (branch `feat/agentic-context-orchestration`, stacked on `fix/turbopack-tracing-harness @ ca56a74`): commits `b6552ca` → `6a9135a` (W1 tool-output protocol, W2 durable memory, W3 `compaction.reserved` 30000, W4 orchestrator agent, W5+W6 orchestrator-health + handoff SCHEMA v1.1, W7 tsc-baseline + doc-budget + hook + CI gates, Phase 7 tests 23/23). Verified: `tsc` **46 = exact baseline**, `quickbuild` **0 Turbopack warnings / 185/185 pages**, doc budget **73.0 KB / 100 KB**. Post-merge: first deploy failed on UI-installed `@netlify/plugin-emails` → disabled in Netlify UI (Lesson 127) → deploy `6aacebd948646f5a24068933` **ready**. Full detail: `.agents/changelog/versions-v3.40.md`.
>
