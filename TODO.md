...
**In progress — v3.40.1 production recovery serving fix** (branch `fix/production-analytics-rec-serve`, on `main` `898a3f6` = v3.40.0 MERGED via PR #127 + LIVE): user finding — production cold start served EMPTY analytics all day (P6003 plan-limit hold until 2026-10-02 + empty SQLite mirror). Root cause: boot `getMirrorBlobsStore()` throw **memoized as null forever** (one-shot Blobs region-context resolution) → v3.39.0 boot restore (disk → Blobs → fresh) never reached Blobs on cold instances. CODE + TESTS DONE: negative-TTL failure cache (60s) + one-shot 30s deferred Blobs restore (`schedule/cancel/retryDeferredMirrorRestore`, retry clears failedAt BEFORE download) + `mirrorHasLiveData()` swap guard + NEW `lib/netlify.ts` `isNetlifyRuntime()` + `/api/health` fields. Verified: **93/93 / 1283 pass / 4 skip / 0 fail**, `tsc` 46 exact, `quickbuild` 185/185, Blobs re-seeded. Docs applied (agent-memory, Lessons 128, CHANGELOG row, versions-v3.40.md, Primer, HANDOFF, versions-index); remaining: session-todos + this Quick Reference + budget/hygiene. **Commit/push/PR/deploy pending user**. Full detail: `.agents/changelog/versions-v3.40.md` §v3.40.1.
>
