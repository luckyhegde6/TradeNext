...
| Category | Status |
|----------|--------|
| **Prisma Postgres plan limit — 10K ops/day exceeded (v3.19.0 plan)** | [ ] **IN PROGRESS** — Live site DB-dependent routes returning 500. Root causes identified: (1) historical price sync 3K-6K ops/day, (2) dual heartbeats 2,880 ops/day, (3) no cache on chartink templates, (4) recommendations fingerprint always hits DB. **Plan**: `.agents/plans/01-db-plan-limit-resilience.md`. Priority: stop-the-bleeding (graceful degradation) → reduce ops (heartbeat throttle, batch upserts) → cache hardening (memory-first everywhere) → monitoring (ops counter) |
