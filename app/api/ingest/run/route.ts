...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!validationResult.success) {
            logger.warn({ msg: 'Invalid ingestion request', errors: validationResult.error.issues });
        ...
        // Background processing disabled - Redis not configured
        // Always process synchronously
        if (!sync) {
            logger.info({ msg: 'Background processing disabled, falling back to sync processing', csvPath });
        }
