...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.model !== undefined) {
      const valid = AVAILABLE_MODELS.find((m) => m.id === body.model);
      if (!valid) {
        return NextResponse.json({ error: `Invalid model. Choose from: ${AVAILABLE_MODELS.map(m => m.id).join(", ")}` }, { status: 400 });
      }
