    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center border-b dark:border-slate-800 pb-4">
                <div className="space-y-1">
                    <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">User Management</h1>
                    <p className="text-sm text-gray-500 dark:text-slate-400">Manage application users, roles, and access requests.</p>
                </div>
                {activeTab === 'users' && (
                    <button
            ...
            {/* Tab Navigation */}
            <div className="flex space-x-1 p-1 bg-gray-100 dark:bg-slate-800/50 rounded-2xl w-fit">
                <button
                    onClick={() => setActiveTab('users')}
                    className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === 'users' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700'}`}
                >
                    System Users
                </button>
                <button
                    onClick={() => setActiveTab('requests')}
                    className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center ${activeTab === 'requests' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700'}`}
                >
                    Join Requests
                    {requests.length > 0 && (
            ...
            {activeTab === 'users' ? (
                ...
                <div className="bg-white dark:bg-slate-900 shadow-xl shadow-slate-200/50 dark:shadow-none border border-gray-100 dark:border-slate-800 overflow-hidden rounded-2xl">
                    <div className="px-6 py-5 border-b border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/50 flex justify-between items-center">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                            Pending Join Requests <span className="ml-2 px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-xs">{requests.length}</span>
                        </h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 dark:bg-slate-800/30 text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-widest border-b dark:border-slate-800">
                                    ...
                                    <th className="px-6 py-4">Contact & Message</th>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                                {requests.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="px-6 py-10 text-center text-gray-500 dark:text-slate-400 font-medium"> No requests found. </td>
                                    </tr>
                                ) : (
                                    requests.map((request) => (
                                        <tr key={request.id} className="group hover:bg-gray-50/50 dark:hover:bg-blue-900/10 transition-colors">
                                            <td className="px-6 py-5">
                                                <div className="flex items-center">
                                                    <div className="w-10 h-10 rounded-xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-lg mr-4">
                                                        {request.name[0].toUpperCase()}
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold text-gray-900 dark:text-white">{request.name}</p>
                                                        <p className="text-xs text-gray-500 dark:text-slate-400 font-medium">{request.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-5">
                                                <div className="max-w-xs overflow-hidden">
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-bold mb-1">{request.mobile || 'No mobile provided'}</p>
                                                    <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2 italic italic whitespace-normal">
                                                        {request.message || "No background message provided."}
                                                    </p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-5">
