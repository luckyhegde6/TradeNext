/**
 * Ready-to-use Laya question presets (spec 18, P1 verbatim port of
 * `laya/presets.py` + `laya/email.py` `email_questions`).
 *
 * These are runtime question configs in the Python shape
 * {type, instructions, criteria?} — the agent's `_to_internal` maps them to the
 * internal {t, ins, crit} form, exactly like Python.
 */

export interface PresetQuestionConfig {
  type: "choice" | "score" | "noul";
  instructions: string;
  criteria?: Record<string, unknown> | unknown[];
}

/** Questions keyed by id (insertion order preserved). */
export type PresetSet = Record<string, PresetQuestionConfig>;

export function triageQuestions(): PresetSet {
  return {
    intent: {
      type: "choice",
...
/** Canonical email-triage preset (Python defines an identical copy in email.py — not duplicated). */
export function emailQuestions(categories?: Record<string, string>): PresetSet {
...
export function guardQuestions(): PresetSet {
  return {
    jailbreak: {
      type: "noul",
...
export function moderationQuestions(): PresetSet {
  return {
    toxic: {
      type: "noul",
...
export function routerQuestions(): PresetSet {
  return {
    difficulty: {
      type: "score",
