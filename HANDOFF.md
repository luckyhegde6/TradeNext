...
feature: "v3.40.3 IN PROGRESS on feature/predeploy-mirror-preserve (on main 27c0770 = v3.40.2 MERGED via PR #129 38ee7da; v3.40.1 MERGED via PR #128 15fa0a3): predeploy mirror-preservation guard (Spec 14). P6003 plan-limit hold until 2026-10-02 => a production deploy could cold-start with an EMPTY SQLite mirror (same served-empty class as v3.40.1). Fix: netlify.toml build = node scripts/predeploy/preserve-mirror.mjs && npx prisma generate && npm run quickbuild; build-time guard calls LIVE POST /api/admin/predeploy/preserve (x-deploy-guard-token vs DEPLOY_GUARD_TOKEN 64-hex timingSafeEqual + admin-session fallback; token never logged) => getOutboxPending() -> snapshot (persist+export+upload Blobs) -> createMirrorBackup (backups/sqlite-mirror-<ISO>.sqlite, keep 5, fail-open) -> breaker-closed ONLY pushSqliteToPrisma({reason:'deploy', leaderGate:false}) -> audit ADMIN_DB_SYNC; modes pushed|backed_up|skipped (P6003 hold => backed_up); push throw => 500 push_failed; GET read-only diagnostics; runtime=nodejs + dynamic=force-dynamic; soft-fail exit 0 (deploy never blocked). CODE+TESTS DONE & VERIFIED: 99/99 suites / 1334 pass / 4 skip / 0 fail (25 new: mirrorBackup 14 + predeployPreserveRoute 11); tsc 46 exact; lint 0 errors; doc budget 74.9/100 KB; quickbuild 185/185; DEPLOY_GUARD_TOKEN upserted via Netlify MCP manage-env-vars (production, scopes builds+runtime, site 78401e5d-b137-4b6d-94bb-ad1ec8de6b05; temp file deleted). Legacy finding (record-only): .github/workflows/deploy.yml publishes ./out via nwtgck/actions-netlify — NOT the real path. Docs phase 8 DONE (Lessons 132/133, versions-v3.40.md §v3.40.3 + PR #128/#129 MERGED fixes, versions-index row, CHANGELOG addenda, Primer, agent-memory, TODO, session-todos, latest.md). Remaining: session archive decisions/flow, wiki publish, doc-budget re-check, hygiene. COMMIT/PUSH/PR/DEPLOY PENDING USER."
```
