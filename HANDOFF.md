...
feature: "v3.41.2 COMMITTED 5b088e3 + v3.41.3 CODE+TESTS+VERIFIED on feature/ph22-decision-engine (on 5b088e3 = v3.41.2 COMMITTED; parent a269057 = v3.41.1 COMMITTED; grandparent ab6fd65 = v3.41.0 COMMITTED): v3.41.3 = Spec 18 Laya real inference P1–P3 + real provider path (user-approved spec/plan 18). Real Laya inference runtime behind parity gate DECISION_LAYA_REAL=1 → LayaRealProvider (else laya-mock default, byte-identical): lib/services/laya/ P1 pure-TS ports (version/qtypes/serialize/calibration/collate/presets/email/buildSequence/lang/router, Python 1:1) · P2 tokenizer.ts WASM lazy-singleton (@huggingface/tokenizers, add_special_tokens=false, v1 specials CLS 50281/SEP 50282/PAD 50283/MASK 50284, DECISION_LAYA_MODEL_DIR) · P3 decisionModel.ts two chained ONNX InferenceSessions encoder_q8→head_q8 (onnxruntime-node@1.30.0; int64/bool casts; K≥2 pad clamp; act_logits snake_case graph-name read — spike camelCase-key bug corrected; crypto probe hidden-1024 fail-fast; lazy singleton; child-process probe scripts/dev-checks/laya-forward.ts; weights lib/services/laya/weights/v1/ gitignored + scripts/fetch-laya-weights.mjs) · agent.ts minimal systemOne decode (buildSequence→collate→forward→temperature→max-sub softmax→choice/score/noul + confidence 4dp + usage). NEW 7 laya suites + decisionClient/layaProvider updates. Verified: tsc 46 exact (0 new) · lint 0 (1155 pre-existing warnings; Lesson 139 flat-config disable-directive fix) · 116/116 suites (1538 pass / 4 skip / 0 fail) · quickbuild 189/189 · doc budget 85.4/100 KB · live in-process real ping (503 MB chain — laya: ok) + HTTP ping 200 user-accepted (plan step 21). Docs pass DONE + hygiene clean (junk deleted, doc budget 89.0/100 KB). COMMITTED `3c765b9` (46 files, +4426/−80, hook green) · pushed (origin in sync) · wiki updated + pushed `5cdba9e` · **PR #132 OPEN** (https://github.com/luckyhegde6/TradeNext/pull/132, carries v3.41.0–v3.41.3, 9 commits) — merge/deploy pending user."
```
