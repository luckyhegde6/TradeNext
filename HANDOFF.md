...
- **Next (consolidated)**: both workstreams committed — v3.33.0 watchdog `6e22eca`, v3.33.1 swing `f86d9d0`, v3.34.0 monthly-ops `5d754b7`, v3.34.1 wasm gate-fix `d91fb01` — all merged into PR #118 branch `fix/leader-watchdog-self-heal`; v3.35.0 intelligence-test-fix = test-only +21 UNCOMMITTED (docs pass DONE: AGENTS.md/CHANGELOG/TODO/Primer/Lessons #115/agent-memory/session-todos/HANDOFF + NEW versions-v3.35.md + latest.md + session archive `2026-09-11-intelligence-test-fix`); delete `.dev-otel.log`; run `/pre-commit-check` → **no push/merge/deploy of PR #118 without explicit user approval** (v3.35.0 included).
