...
**Branch `feat/db-health-ops-visibility` — base committed `4c47348` + docs `47e6677`, PUSHED; PR NOT created (user decision). Increment uncommitted — offer commit of increment + PR to user.**
...
- **Increment (uncommitted) — per-type DB-error summary + lazy SQLite re-init**: `classifyDbError()` (`lib/db-utils.ts`, `DbErrorType` = plan_limit/timeout/accelerate_proxy/connection/write_budget/other) + per-type `dbErrorCounts` (`__dbErrorCounts` globalThis, lazy IST-day rollover) in `recordDbError()` + `getDbErrorCounts()`; `persistDbErrorCounts()`/`restoreDbErrorCounts()` (key `db_error_counts`, IST-day + per-key `Math.max` merge, same 60s tick); **`ensureSqliteBackup()`** lazy on-demand init (`_initPromise` finally-reset — never stuck "Not Ready" after a temp WASM/DB hiccup) + `resetSqliteStateForTests()` in-place test hook; `/api/admin/db-health` GET returns `dbErrorSummary {day, counts}` (+ POST ensures init); UI per-type summary chips (plan_limit/connection red w/ ring >0, timeout/accelerate_proxy/write_budget amber, other gray) + error total + IST-day footnote. Tests: `db-utils.test.ts` `classifyDbError` (7 cases) + `sqlite.test.ts` 5 new → **suite 932 pass / 4 skip** (was 920/4, +12); tsc 46 = baseline (0 new production errors).
- **Verification**: base committed + pushed; post-commit full suite **66 suites, 920 pass / 4 skip** (47.7s); increment verified after: two-file run (db-utils + sqlite) 48/48, full suite **932 pass / 4 skip** (+12), tsc 46 = baseline (0 new production errors); pre-commit hooks passed (tsc production clean, no baseline pollution). No schema change → no migration.
