...
**Branch `fix/v3.28.1-sqlite-self-heal` (on top of v3.28.3 `a1dd094`) — v3.28.4 read-first recommendations route + edge-cache of heavy latest-run reads: code + regression test + docs VERIFIED; commit/push pending user (no auto-merge/deploy).**
...
- **Docs updated (all)**: AGENTS.md v3.28.4 row, `.agents/CHANGELOG.md` index + `.agents/changelog/versions-v3.28.md`, session-todos, this file. (TODO.md/Primer/agent-memory pending in the same doc pass.)
- **Next**: present the v3.28.4 diff for user commit approval (separate commit, no amend of `a1dd094` or older; no push/merge without say-so). Then merge/deploy v3.28.1-4 when approved.
