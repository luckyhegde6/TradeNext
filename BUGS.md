...
| 14 | **Accelerate CACHING was never active + standalone Accelerate RETIRES Dec 1, 2026** (investigated 2026-09-04 on `v3.26.0-prod-failure-triage`). `@prisma/extension-accelerate@^1.3.0` is in `package.json` but is **imported/used NOWHERE** in app code; the client (`lib/prisma.ts:81-86`) is built with `new PrismaClient({ accelerateUrl })` — that routes through the Accelerate **proxy/transport** but does NOT enable the `cacheStrategy` feature (that requires `$extends(withAccelerate())`). So every query is a real proxy round-trip to the DB; zero Accelerate query caching. **INFORMATIONAL — no code change.** Two follow-ups: (a) optional short-term ops/latency win = add `withAccelerate()` + `cacheStrategy` to high-frequency direct-Prisma read candidates; (b) **REQUIRED** — standalone Accelerate is **retired Dec 1, 2026** (~3 months), so plan the Prisma Postgres migration (Accelerate caching is built-in there) — tracked as TODO row "Prisma Postgres migration planning (Dec 2026 Accelerate retirement)". | Low | — | Informational (doc'd; see TODO migration row) |
| 15 | **Prod: `/alerts` crashes to a BLANK page** (live-verified 2026-09-18, admin session). `GET /api/alerts` → 500 `{"error":"Failed to fetch alerts"}` (`app/api/alerts/route.ts:57-62` — `getUserAlerts` is Prisma-only, no SQLite fallback → throws under the P6003 plan-limit hold), and `app/alerts/page.tsx:112` calls `data.filter(...)` **unguarded** (also `:230` `alerts.filter`, `:519` `alerts.map`) → `TypeError: data.filter is not a function` → React Error Boundary → header-only body (112 chars). `/api/alerts/check` also returns **405 on GET**. **FIX (APPLIED 2026-09-18, `fix/mirror-contract-fixes`)**: `getMirrorAlerts(userId)` (mirror `alert` rows are camelCase-aliased + `condition` JSON-parsed, but cover ALL users → scoped to the session user, booleans coerced) wired into both the list and `action=count` branches via `isDbUnavailableError`; client now `Array.isArray` guards + a `fetchError` state with a Retry button. Tests: `lib/__tests__/alertsMirrorFallback.test.ts` (6). Lesson 130. | High | — | ✅ Fixed (2026-09-18, pending deploy) |
| 16 | **Prod: `/markets/calendar` shows ZERO events despite 40 fetched corporate actions** (live-verified 2026-09-18) — API↔UI shape mismatch. The `/api/corporate-actions/combined` SQLite-mirror fast path (`app/api/corporate-actions/combined/route.ts:290-297`, `source:"sqlite_mirror"`; identical `sqlite_backup` at `:445-448`) returns **RAW snake_case rows** from `lib/sqlite.ts` `getCorporateActions()` (`SELECT * FROM corporate_action` → `ex_date`/`company_name`/`action_type`/`dividend_per_share`/`dividend_yield`), while the primary Prisma path maps camelCase (`route.ts:353-363`). Consumers read camelCase: `app/markets/calendar/page.tsx:101-103` filters on `action.exDate` → **all rows dropped**; `/markets/analytics` + admin corporate-actions tables render `-` for COMPANY/TYPE/DIVIDEND/YIELD; mirror `dividend_yield` null (40/40). Sub-issue: calendar key `new Date(y,m,d).toISOString().split("T")[0]` → **IST off-by-one**. **FIX (APPLIED 2026-09-18, `fix/mirror-contract-fixes`)**: new shared `lib/services/corpActionMirror.ts` `mapMirrorCorporateAction` (tolerant snake→camel, idempotent, never throws) applied to BOTH mirror branches (`sqlite_mirror` + `sqlite_backup`); calendar day key switched to a local `toDayKey()` (IST off-by-one: 22-Sep IST midnight keyed as 21-Sep). Tests: `lib/__tests__/corpActionMirror.test.ts` (5). Lesson 129. | High | — | ✅ Fixed (2026-09-18, pending deploy) |
| 17 | **Prod: admin/user APIs are Prisma-only while mirror-backed routes degraded gracefully** (live-verified 2026-09-18, P6003 hold until 2026-10-02). `/api/admin/workers/status` has **NO auth check AND NO SQLite fallback** → 500 + a ~2s poll loop (186-188 console errors; mirror DOES hold `worker_status` 15 + `worker_task` 49 rows); `/api/admin/workers` 500; `/api/admin/users` **500 even with a valid OTP session** → `/admin/users` "Failed to fetch users", Platform Users 0; `/api/admin/monitoring?type=stats\|alerts\|rate-limits\|nse-logs\|db-logs` all 500 (10 errors across 2 polls); `/api/admin/cron` + `/admin/utils/tasks` render silent zeros; `/api/dividends/calendar` 200 but `totalDividends: 0` (dashboard widget showed 6 upcoming ex-dates); `/api/screener/saved` 500. Mirror tables exist → SQLite-first fallback is feasible. **FIX (APPLIED 2026-09-18, `fix/mirror-contract-fixes`)**: `/api/admin/workers/status` GET+POST now require an admin session (401) and GET falls back to the `getWorkerStatuses()` mirror (`mapMirrorWorkerStatus` + 5-min `filterWorkers`); the admin Workers poll is now self-scheduling 10s→20s→40s→60s backoff with a "paused/retrying" hint; `/api/dividends/calendar` (`fetchDividends`) now falls back to the mirror (`fetchMirrorDividends`) instead of reporting `totalDividends: 0`. Tests: `lib/__tests__/workersStatusRoute.test.ts` (10) + `dividendCalendarMirror.test.ts` (5). **STILL OPEN (follow-up)**: `/api/admin/users` (mirror has NO user table), `/api/admin/monitoring` (5 types), `/api/admin/workers` list, `/api/admin/cron`, `/api/screener/saved`. (Note: other admin routes DO return proper 401/403 unauthenticated — `auth()` itself is fine; these are route-specific defects.) | Medium | — | 🟡 Partial (workers/status + dividends fixed 2026-09-18, pending deploy; others follow-up) |
