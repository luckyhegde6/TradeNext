...
## Last Updated
2026-09-04 (v3.27.0 — Lesson 104 added)
...
### 104. Prisma Query-Cache `cacheStrategy` Is Not Typed on the Base Client — Inject It at the Query Boundary Via a Pure Transform
**Issue**: Wiring the Accelerate/Prisma-Postgres edge cache (`@prisma/extension-accelerate` `withAccelerate()` + per-query `cacheStrategy`) hit two type/coupling walls. (1) The base `PrismaClient` model **read-args hard-code `cacheStrategy: never`** — so a plain `$extends()` read keeps rejecting it, and **a manual client-type intersection does NOT override `never`** (the `never` from the base wins). (2) Any **new named export from `lib/prisma.ts`** (the `withAccelerateCache` helper) silently resolves `undefined` in the test factories that mock `@/lib/prisma` with only `{__esModule: true, default: mock}` — so the wrapped calls threw `withAccelerateCache is not a function`.
**Solution**: (1) **Boundary transform, not client-type restructure** — NEW `withAccelerateCache(strategy)(args)` in `lib/prisma.ts` typed `const withAccelerateCache = <T extends (...args: any[]) => any>(strategy: CacheStrategy) => (args: Parameters<T>[0]): ReturnType<T> => ...` and called at the call site as `withAccelerateCache({ttl,swr})(prisma.model.findX(args))`. Because the transform is applied **at the call site**, the caller's argument expression is contextually typed by the base client's real read-args (so `orderBy: 'asc'|'desc'` literals still resolve), while `Parameters<T>[0]` keeps that same shape for the inner call and `cacheStrategy` is injected via a final `as any` spread — the runtime `withAccelerate()` client accepts it, the base type simply can't express it. Top type stays `type AccelerateClient = PrismaClient` so the 400+ call sites are unaffected. (2) **Keep test factories in lockstep** — any factory that mocks `@/lib/prisma` with `{__esModule, default}` must also export a pure stub for the new named helper (`(strategy) => (args) => ({ ...(args as object), cacheStrategy: strategy })`), and exact-args assertions on wrapped reads must loosen to `expect.objectContaining({ where, orderBy })` since the real call now carries the injected `cacheStrategy`.
**Rule**: (a) A query-cache `cacheStrategy` that the base client types as `never` cannot be forced through a client-type intersection — a **pure call-site boundary transform typed via `Parameters<T>[0]`/`ReturnType<T>`** is the way to preserve contextual argument typing while injecting the runtime-only option. (b) **Every new named export from `lib/prisma.ts` breaks `{__esModule, default}`-only mock factories** — update them in the same change with a matching pure stub, never leave a named export dangling as `undefined`.

---

### 105. A Fail-Open Leader Lock + Log-Only `onLost` = Zombie Workers — `onLost` Must Stop the Engine It Guards
**Issue**: After a DB blip, multiple workers executed different tasks concurrently ("only 1 worker / task is active at a time") even though leader election was in place. Root cause was TWO compounding defects: (1) `acquireLeaderLock` **fails OPEN on DB-unavailable** (returns `true` on every instance during a blip, so all instances start their worker poll loop + cron daemon), and (2) `instrumentation.ts` `onLost` callbacks only `logger.warn` — when the DB recovered, only one instance kept the `leader-<role>` row; the losers' `renewLeaderLock` returned false → `onLost` fired → they **logged and kept polling forever** → N active workers/tasks. A log-only `onLost` is a liveness trap: the lock loss is precisely the signal that this instance must RESIGN, not merely record why.
**Solution**: worker `onLost` → `stopWorkerEngine()`; cron `onLost` → `stopCronDaemon()` (both synchronous: clear poll timeout + heartbeats + scheduler intervals / destroy node-cron tasks + intervals). sqlite-sync `onLost` stays log-only because sync is gated per-run by `isLeader` — nothing long-running to stop. Added `lib/__tests__/instrumentation.test.ts` (5) which captures the `onLost` callback from `startLeaderHeartbeat(role, cb)`, invokes it, and asserts the corresponding stop is called.
**Rule**: Any engine started under a leader lock MUST have its `onLost` wired to its stop function — never log-only. If the lock acquire path can fail open (start-on-blip), the recovery path must self-resign the losers, or the "single active worker" invariant silently dies. The idle `worker_status` rows of resigned instances are harmless (reaper ignores stale) — the multi-worker polling, not the rows, is what needs pruning. Test the callback, not just the start path.

---

### 107. A Component's Local "Loading" Flag Cleared Only on the Authenticated Path Makes the Unauthenticated Branch Dead Code — and Logged-In-First E2E Structurally Misses It
**Issue**: The logged-out `/watchlist` page rendered an **infinite skeleton** instead of the "Please sign in to view your watchlist." card. Root cause in `app/watchlist/page.tsx` :303: the render guard `if (status === "loading" || loading)` made the `status === "unauthenticated"` branch unreachable because the LOCAL `loading` (`useState(true)`) is **only cleared inside `fetchWatchlists()`**, which runs only when `status === "authenticated"`. For a logged-out visitor `status` is `"unauthenticated"`, `loading` stays `true`, and neither condition ever flips to clear the guard → eternal skeleton. The committed e2e suite missed it because the watchlist spec **logs in first** — it can never exercise the logged-out render path, so the dead-code branch persisted silently.
**Solution**: narrow the local-flag contribution to the authenticated case: `if (status === "loading" || (status === "authenticated" && loading))`. Now `status === "unauthenticated"` falls through to the sign-in card regardless of the never-cleared local flag; authenticated behavior (skeleton while `fetchWatchlists()` runs, then data/empty/CTA) is unchanged. Added a path-deviation jest spec `app/watchlist/__tests__/page.test.tsx` (3) with `useSession`/`fetch`/`AiActionButton`/`Autocomplete`/`useLivePrices` mocked — the unauthenticated→sign-in+no-skeleton test **fails pre-fix, passes post-fix**.
**Rule**: (a) Any component that keeps a `useState` loading flag AND branches on auth status must key the local flag to the SAME branch that clears it (`status === "authenticated" && loading`), or every other branch becomes dead code. (b) A test/e2e suite that logs in before visiting an auth-gated page **cannot verify the logged-out UI** — when a page has a distinct logged-out state, cover it with an explicit isolated-context browser check + a component test that mounts unauthenticated. (c) Browser/DevTools visual observation of a user-reported flow is a legitimate bug-hunt that scripted suites structurally miss — run the reported state, not the happy path.

### 109. A Schema-Init SQL Parse Failure Makes SQLite "Not Ready" **Silently** — Every SQLite-First Read Falls Back to Prisma; and Stale Mirrors Mask Themselves Until Telemetry Shows 100% Prisma
**Issue**: The recurring db-health noise ("SQLite Not Ready", SQLite-first reads showing constant Prisma hits) traced to THREE compounding root causes. (1) **SCHEMA_SQL contained a stray `;` inside a `--` comment** — sql.js ran the whole boot script through `exec()` and threw `near "Prisma": syntax error` on EVERY boot → the mirror never reached `ready:true` → every SQLite-first read silently fell back to Prisma (degraded but functional — no loud failure to pin down). (2) **`discoverPendingTask()`'s ready-DB path never refreshed the mirror freshness mark** — an idle system (no pending tasks) stopped writing `control_write_at:<table>`, so `isControlMirrorFresh` went stale after the (old) window and the system silently flipped to Prisma every poll. (3) **`upsertCronJob` bound Prisma `Date` instances raw** — `String(Date)` is LOCALE-formatted ("Sat Sep 06 2026 10:30:00 GMT+0530 …"), and `reconcileControlToPrisma` re-reads via `new Date(String(col))` → the 12h reconcile could write corrupt `nextRun`/`lastRun` back to Prisma.
**Solution**: (1) Removed the stray `;` from the SCHEMA_SQL comment block → sql.js boots clean. (2) NEW `SqliteFallback.touchControlMirror(table)` re-marks `control_write_at:<table>` = NOW (non-empty gate, best-effort, never throws); `discoverPendingTask()` calls it on the ready-DB early-return so freshness tracks poll cadence, not task activity. (3) NEW `toIso(v)` (`v instanceof Date ? v.toISOString() : String(v)`) on the lastRun/nextRun/createdAt binds in `upsertCronJob` — the same caveat applies to any mirror write whose value Prisma reads back (correct pattern already existed at :2524-2525 in `syncFromPrisma`).
**Rule**: (a) When a "degraded-but-working" fallback path quietly absorbs a boot-time failure, the failure is invisible — validate the SQLite boot SQL independently (it must parse in isolation) and assert `ready` in tests. (b) Any mirror freshness gate needs an explicit touch at the HIGH-FREQUENCY read site — a freshness window that depends on the very writes it gates goes stale under idle load. (c) Datetimes written to the mirror must be ISO strings (`toISOString()`), never locale `String(Date)` — any value Prisma re-reads via `new Date(String(col))` will corrupt on the 12h reconcile.

### 110. `isLeader()` Returns a Promise — `if (!isLeader("role"))` Is ALWAYS Truthy and Silent-Disables the Guard It Was Meant to Enforce
**Issue**: In the 6h PUSH engine (`lib/sqlite.ts`), the probe pivot to `pushSqliteToPrisma()` was wrapped in `if (!isLeader("sqlite-sync")) { …probe/pull… }`. But `isLeader()` is **async** — it returns a `Promise<boolean>`, so `!isLeader(...)` is `!Promise` (always `false`) and the guard NEVER fired. The intended single-writer gate (only the elected leader pushes to Prisma on the 6h tick) silently ran on EVERY instance, multiplying the very Prisma ops Plan 09 was built to eliminate. Async-result bugs like this are invisible because the branch is "false" either way — a truthy-Promise guard degrades to the *opposite* of its intent without a compiler or runtime error.
**Solution**: `await` the leader check: `if (!(await isLeader("sqlite-sync"))) { … }`. The PUSH path now runs only on the elected sqlite-sync leader; non-leaders fall through to the PULL path. Added a test asserting the leader-gate drives the push-vs-pull decision.
**Rule**: (a) When the "leader gate" is a boolean function you KNOW is async, verify whether your call site is inside an `async` context and `await`s it — a `Promise` coerced by `!` is always truthy, silently flipping the guard. (b) A gate whose failure mode is "both branches run the same degraded path" is invisible — the 6h cadence test must assert which SyncDirection actually lands in `sync_history`, not just that *some* sync happened. (c) The `sync_history` direction ledger (`prisma_to_sqlite` vs `sqlite_to_prisma`) is the observability hook that proves the PUSH engine is actually leader-gated and reaching Prisma — without it a mis-wired guard is indistinguishable from a healthy one.

## Lesson 111: A "Parse Validity" Guard That Compares the Parsed Result Against the *Input* Timezone Silently Corrupts Epochs on Non-IST Hosts — and Server-Clock Round-Trips Need an Explicit Round-Trip, Not a Shape Check

**Issue**: TradeNext stored cron `nextRun` as IST wall-clock strings while a Netlify host clock held IST wall-clock as if it were UTC → persisted `nextRun` skew ≈ +5.5h. Fixing it required BOTH an on-demand Postgres time probe with a persisted offset AND a fix to `parseIstDateTimeLocal`'s validity guard: the pre-fix round-trip guard compared the parsed-**UTC** projection against the **IST wall-clock input**, which never matches for IST inputs (UTC ≠ IST) → genuinely valid IST inputs were silently rejected → corrupt epochs and misleading `nextRun`s. The fix compares the **IST round-trip** of the parsed instant against the raw input.

**Fix**:
- `parseIstDateTimeLocal` — `epoch = Date.UTC(y, m-1, d, h, min) − offset` (construct the instant from IST wall-clock components minus the fixed +5:30 offset; IANA rules unnecessary for a fixed-offset zone), then `valid = toIstIso(parsed).slice(0, 16) === input`.
- Round-trip guards must compare the parsed instant **in the target zone** (`toIstIso`), never in UTC.
- NEW `lib/services/timeCorrection.ts` pure helpers: `offsetMinutes = trueNow − serverNow` (fast clock → `-330`); `applyOffset` identity@0; `getCorrectedNow()`/`getCronFrom()` lazily apply the persisted offset (read-through, no cache); `getTimeDiagnostics()`. Wired at EVERY `nextRun` compute/claim site (`recommendationCronService.ts:5/:118`, `worker-engine.ts:598/631/645/657`).
- `probeDbTimeNow()` (`lib/sqlite.ts`) — `SELECT NOW()` via `$queryRawUnsafe` (10s timeout), persisted to `_backup_meta` keys `time_correction`/`time_probe_db`; `IST_OFFSET_MINUTES = 330`, `TIME_ALIGN_TOLERANCE_MS = 60_000`.

**Related**:
- NSE EOD trade-date keys are IST — `getIstDayKey` lives in `lib/prisma.ts` (single source).
- node-cron in-process is OS-`TZ`-bound — the durable root fix is correct `TZ`/`UTC` env on Netlify, not code-level offsets (deferred).

**Testing**: NEW `lib/__tests__/timeCorrection.test.ts` **20/20** — full manual `jest.mock("@/lib/sqlite")` (factory-closure store, `__esModule: true`, NO `...actual` spread, `jest.requireMock` casts), **never `process.env.TZ`** (V8 caches `getTimezoneOffset`; env is unreliable on Windows). sqlite.test.ts **71/71** (probe persist/restore describe). Targeted **118/118**. Full **1106 pass / 4 skip / 1 fail** (1 = documented pre-existing `intelligence.test.ts` flake). tsc **46 = exact baseline (0 new)**.

### 112. A Web `Request` Body Is Single-Use — Re-Reading `req.json()` in a Branch 400s with the Wrong Error, Silently Breaking Features Since v3.21.2

**Issue**: POST `app/api/admin/db-health/route.ts` read `req.json()` at the top (~:238) to derive `action`, then BOTH `restore` (~:272) and `set_time_correction` (~:434) RE-READ the body. A Web `Request` body stream is single-use — `bodyUsed` is `true` after the first `json()` — so the second read throws and both actions 400'd with the *wrong* message: `restore` → `"Invalid restore payload"` (the base64 backup-restore feature had been **broken since v3.21.2**), `set_time_correction` → `"Invalid payload"` (inherited the pattern when v3.32.0 added the action, masking the real zod `"istDateTime is required"` error). Unit tests that mock `req.json()` as a plain jest.fn can never catch this — a mock doesn't enforce the single-read contract.

**Solution**: parse the body ONCE at the top of POST — `let action = "sync_sqlite"; let requestBody = {}; try { requestBody = (await req.json()) ... }` — and reuse `requestBody` in every branch (`// v3.32.1 fix: parse the body ONCE here and reuse requestBody`). `restore` reuses `requestBody as { data?: string; file?: string }` (unparseable → `{}` → the existing `"Missing base64 sqlite data"` 400 stays honest); `set_time_correction` reuses `requestBody as { istDateTime?: string }` (missing → zod `"istDateTime is required"` surfaces instead of the masked `"Invalid payload"`). Regression: NEW `lib/__tests__/dbHealthRoute.test.ts` **5/5** uses REAL `Request` objects via a `jsonPost` helper that enforces `bodyUsed` — it fails pre-fix.

**Rule**: (a) Any route that touches the request body in more than one branch must PARSE IT ONCE and reuse — never call `req.json()` (or `req.text()`) twice; the second read throws on a real `Request`. (b) Mock `req.json()` as a spy over a real `Request` (or assert `bodyUsed`), never an arbitrary jest.fn, or the single-read contract decays silently. (c) When a route action passes mock-based unit tests but never worked through the UI, suspect an environment-only failure mode like this — re-verify with a real HTTP/Playwright flow.

### 114. `resetSqliteStateForTests()` Nulls the Module's Captured `state` IN PLACE — a Suite That Resets Must Re-Init (`await ensureSqliteBackup()`) Before the Next Test

**Issue**: Adding the v3.34.0 monthly describe to `sqlite.test.ts` broke LATER tests in the same file with `TypeError: Cannot read properties of null (reading 'persistTimeCorrection')`. Root cause: the monthly test-3 calls `resetSqliteStateForTests()` — the v3.21.1 test hook that NULLS `state.db`/`state.ready`/… **IN PLACE** (it cannot reassign the global because the module closure already captured the `state` binding — Lesson 96) — followed by `resetOpsMonthlyForTests()`, but NOTHING re-initialized the module → every subsequent test inherited `state = null`.

**Solution**: the monthly test-3 ends with `await ensureSqliteBackup();` BEFORE `resetOpsMonthlyForTests()` so the module is re-initialized (ready DB) before the next test runs. After the fix: sqlite.test.ts **83/83**; targeted **94/94** across 7 suites (opsMonthly, dbHealthRoute, timeCorrection, backtestDataService, worker-engine, recommendationPerformanceService, recommendationCronService); tsc **46 = exact baseline (0 new)**.

**Rule**: (a) Any test that calls an in-place state-reset hook MUST re-run the module's init (`await ensureSqliteBackup()` / equivalent) BEFORE the test exits — the NEXT test inherits null state otherwise. (b) A cascading `Cannot read properties of null (reading '<next-called-method>')` after adding a new describe normally points at a reset/init ordering bug in the PREVIOUS test, not the new production code — check the reset-suite boundary first.

### 113. A One-Shot Leader Election With Stop-Only `onLost` Callbacks Never Recovers — a Lost Role Silently Stays Standby Until Manual Intervention
**Issue**: The prod scheduler died ~2026-09-08 07:06 UTC with NO automatic recovery. Leadership was ONE-SHOT: `instrumentation.ts` called `acquireLeaderLock()` once at boot, then `startLeaderHeartbeat(role, onLost)`. v3.28.2 "fixed" the zombie-worker bug by making `onLost` STOP the engine (`stopWorkerEngine()` / `stopCronDaemon()`) — but STOP ≠ RECOVER: nothing ever re-ran acquisition, so after a lost/stale claim every instance stayed a standby poller forever. Stop-on-loss prevents N concurrent engines but leaves ZERO engines; the only recovery was a manual admin "Start Engine" click.
**Solution**: NEW `watchLeaderRole(role, handlers)` in `lib/services/leader.ts` — a self-healing loop that re-claims without manual intervention: standby → adaptive probe (fresh foreign row → SLOW re-probe `LEADER_CLAIM_SLOW_MS` = 300s; stale/absent → claim via `updateMany` count>0 | `create` | P2002 → false | `isDbUnavailableError` → fail-open) → re-probe OWN row (null/not-ours → `failOpenEvents++`) → **leader** + heartbeat every `LEADER_HEARTBEAT_MS` = 300s; heartbeat renewal count 0 → internal `onLost` → standby + `handlers.onLost()` + FAST re-probe (`LEADER_CLAIM_FAST_MS` = 60s). `onAcquired` fires only on a genuine phase transition (phase-guard, no double-start); `stop()` clears timers. `LEADER_STALENESS_MS` 15 → 10 min (human-approved). Zero-Prisma diagnostics: `LeaderWatchStatus`/`getLeaderWatchStatuses()` (globalThis `__leaderWatchStatus`), surfaced in db-health (`leaderWatch`/`leaderTuning`).
**Rule**: (a) Any "watchdog" whose loss handler only STOPS work — with no path back to the acquired state — degrades availability rather than restoring it; a lost-leader handler must re-enter the acquisition loop, not dead-end. (b) Steady-state standby must be cheap (1 `findUnique` per 300s per instance) or the watchdog itself becomes an ops/plan-limit problem. (c) A recovery loop must re-probe the OWN claimed row before declaring leadership (`failOpenEvents` tracks DB-unavailable claims) so a stale/failed claim is aborted and retried instead of double-leadering.

### 115. A Fire-and-Forget Prisma Write in Tested Code Is a Real-DB Write in Jest — Mock `@/lib/prisma` (or Await the Write) or the `beforeEach` Teardown Races the In-Flight Upsert

**Issue**: `intelligence.test.ts` flaked at :187/:246/:279 (spurious `INTELLIGENCE_CACHE_HIT` from an unexpected DB row) — "documented pre-existing flake" since v3.25.0. Root cause: `setIntelligenceCache` (`lib/services/intelligence/cache.ts` :101-124) fires `prisma.intelligenceCache.upsert(...)` UN-AWAITED. In a test that uses the real Prisma client, the `beforeEach` `deleteMany` teardown races the still-in-flight upsert — whichever lands last wins, so the "clean" DB sometimes still holds the cache row → the next assertion counts it as a cache hit.

**Solution**: in `lib/__tests__/intelligence.test.ts`, isolate the service from the DB entirely with a full prisma mock — `jest.mock("@/lib/prisma", () => ({ __esModule: true, default: { intelligenceCache: { findUnique: jest.fn().mockResolvedValue(null), upsert: jest.fn().mockResolvedValue({}), delete: jest.fn().mockResolvedValue({}), deleteMany: jest.fn().mockResolvedValue({ count: 0 }), count: jest.fn().mockResolvedValue(0), findMany: jest.fn().mockResolvedValue([]) } } }))` inserted between the `beforeEach` close and the Tests header (test-only +21). `jest.clearAllMocks()` clears call history while keeping the impls (a plain mock resets to no-impl). After the fix: targeted **13/13**; full **86/86 suites / 1170 pass / 4 skip / 0 fail** — FIRST fully-green full run (the pre-existing-flake asterisk is gone); tsc 46 = exact baseline (0 new).

**Rule**: (a) An un-awaited fire-and-forget DB write in production code is a REAL write in any Jest test that doesn't mock `@/lib/prisma` — a `beforeEach` "clean" via `deleteMany` cannot guarantee a clean DB against an in-flight upsert; isolate with a prisma mock (or await the write). (b) Prefer an explicit `jest.mock("@/lib/prisma", () => ({ __esModule: true, default: {...} }))` factory in targeted suites: a real-client setup is fine for integration tests but makes any async write a race source. (c) When a "documented pre-existing flake" keeps failing run-to-run, the doc itself is the bug report — fix the root cause rather than re-documenting it.

### 116. A "Steady-State" Serve Path That Only Gates on Age Re-Runs the Whole Pipeline on Every Plain Load — Separate Liveness (Is a Fresh Job In Flight?) From Terminality (Is There a Done/Failed Result?) and Let Only an Explicit `force` Regenerate

**Issue**: plain `/api/recommendations/swing` loads RE-RAN the full screener+AI pipeline at steady state — after a done/in-flight `SwingAnalysisJob` existed, every load past the DB fast-path's age gate re-scanned Chartink + re-analyzed the top-20 with OpenRouter, burning NSE requests + tokens with no new data (Swing tab slow again; the v3.13.0+ DB-job model only gates the fast-path on age).
**Solution**: NEW serve semantics in `lib/services/swingRecommendationService.ts` (~L988-1135): (a) an existing **done/failed result is served INDEFINITELY** — no age gate, no auto-regeneration on a plain load; (b) **`force=1` is the ONLY regeneration trigger** — it supersedes pending/running jobs AND kicks `checkSwingPerformance()` for the superseded one; (c) liveness uses a status-anchored staleness window — `staleBefore = Date.now() - SWING_JOB_STALE_MS`, anchor `pending→createdAt`, `running→startedAt`, else `NaN` (never stale); (d) when the newest job is fresh in-flight, fall back to the **newest-TERMINAL (done/failed)** row so the tab still shows the last result; (e) when the SQLite mirror is empty, **Prisma fallback serves a done job without re-scanning**; (f) on a plain FIRST load (mirror+Prisma both empty) create a **durable pending job** — never an in-memory-only run that a process death would strand.
**Rule**: (a) Distinguish *in-flight freshness* from *terminal-result presence* — an age gate alone can't decide "serve the existing result"; a plain-load serve path must be strictly read-only except when NO terminal/in-flight state exists at all. (b) Regeneration should require an explicit signal (`force=1`) at steady state; background/refresh loads must never re-run the expensive pipeline behind the user's back. (c) Test each serve decision as a first-class case: old-done-served-indefinitely, newest-done-behind-stale-pending, Prisma-fallback-serve-without-rescan, durable-pending-on-first-load, force-perf-kick (swing **57/57**).

### 117. Test Mocks: `jest.clearAllMocks()` Keeps Installations — a New Describe Must Re-Isolate Shared Mocks via the Exported Factory, Never Rely on a Prior `mockReturnValue`

**Issue**: Adding the v3.38.1 "memory-buffer timeframe filter" describe to `ai-monitoring.test.ts` leaked the FIRST describe's sqlite fallback into the new describe: `jest.clearAllMocks()` clears CALL history but keeps the installed IMPLEMENTATIONS, and `getSqliteFallbackMock.mockReturnValue(sqliteMock)` PERSISTS across describe blocks — so the new tests ran against a non-null fallback and could not prove the "memory-only" source path.

**Solution**: in the new describe's `beforeEach` (L202-211), RE-DRIVE the exported mock factory + explicitly neutralize the shared mocks: `findManyMock.mockResolvedValue([])` + `getSqliteFallbackMock.mockReturnValue(null)` (with a comment: "clearAllMocks keeps implementations, so ALSO neutralise the sqlite fallback mock the first describe installed (mockReturnValue persists across describe blocks)"). After the fix: `ai-monitoring.test.ts` **5/5** (v3.38.1 timeframe describe 3/3 + prior 2/2).

**Rule**: (a) `jest.clearAllMocks()` ≠ `jest.resetAllMocks()` — implementations survive; a `mockReturnValue` installed by an earlier describe is still live unless explicitly overwritten. (b) When a new describe must ISOLATE a module from a shared mock, rebuild it through the exported factory (or set the exact return values) in its own `beforeEach` — never assume ordering/leak-freeness. (c) Mocks are module-global across the WHOLE test file — every new describe starts from whatever the previous one left behind.

### 118. A Mocked Module Must Export EVERY Function Its Callers Invoke — Even Fire-and-Forget Helpers Reached Only After a Dynamic Import

**Issue**: the v3.38.1 timeframe tests seed the memory ring via `trackAiCall(...)`; the real persistence path (`persistAiCallToDb`) fire-and-forgets `enqueueWriteBehind` from `@/lib/sqlite` AFTER a dynamic import. The `@/lib/sqlite` mock factory exported `getSqliteFallback` but NOT `enqueueWriteBehind` → the seeded `.then()` threw `TypeError: enqueueWriteBehind is not a function` as an UNHANDLED REJECTION (test still passed — the assertion ran before the rejection surfaced, so the failure was invisible until the suite reported the stray rejection).

**Solution**: add `enqueueWriteBehind: jest.fn()` to the `@/lib/sqlite` mock factory (`ai-monitoring.test.ts` L30) with a comment explaining the contract: "trackAiCall → persistAiCallToDb fire-and-forgets `enqueueWriteBehind` after a dynamic import — the mock must export it or the seeded `.then()` throws an unhandled rejection in the v3.38.1 tests." After the fix: 5/5 with ZERO unhandled-rejection noise.

**Rule**: (a) A mocked module must export every function the REAL module's callers may invoke — a missing export only errors when that code path actually runs (often asynchronously, AFTER the assertion). (b) Fire-and-forget helpers are still executed in Jest: the promise resolves before the suite exits, and an unhandled rejection is a real failure hiding behind a green test. (c) When writing tests around mocked dependencies, grep the REAL module for every cross-module call before finalizing the mock factory.

### 119. Agent Context Is a Budget: Never Auto-Inject Large Files, and Never Age-Chunk a Rulebook (v3.39.4)
**Issue**: Agent sessions hit the context ceiling (`78% → compaction`) after only a few small reads, then looped — compacting, immediately refilling, compacting again.

**Root Cause**: `.opencode/opencode.json` → `instructions` injects the **full contents** of every listed file into **every** request. The injected set had grown to **358 KB ≈ 90K tokens** (`AGENTS.md` 191 KB, `TODO.md` 134 KB, `README.md` 19 KB, two rule files 14 KB). Compaction summarised the conversation, but the next request re-injected all 358 KB, so context refilled instantly → loop. `compaction.reserved` was only 10000, leaving almost no headroom.

**Rule**:
- (a) **Injected files must be thin indexes/pointers** — never bulk history, long tables, or per-version logs. Move detail OUT (`.agents/changelog/`, `.agents/session-archive/`, `.agents/docs/`) and link it from `.agents/INDEX.md`.
- (b) **Keep the injected total ≤ ~100 KB**; enforce mechanically with `node scripts/dev-checks/check-doc-sizes.mjs` (parses `instructions`, 100 KB total / 32 KB per-file budgets).
- (c) **Age ≠ obsolete.** Only chunk **append-only logs** (activity/timeline files) by age. Never age-chunk a **rulebook** (`Lessons.md`): old entries there are still active rules — removing them by date silently deletes guidance.
- (d) Prefer a **deterministic, reversible script** over hand-editing giant files (`.context/slim-docs.mjs` = marker-based + EOL-preserving; `.context/chunk-history.mjs` = append-only archives; both safe because the files are git-tracked).
- (e) Fixing the baseline injection (not just raising `compaction.reserved`) is the real cure — after slimming, 358 KB → 72 KB and the context meter fell 78% → 12%.

### 120. Harness Tests Must Never Touch the Real Repo: Pass the Script Path In, and Guard Protected Real Files (v3.40.0)
**Issue**: writing the CLI-spawn tests for `check-tsc-baseline.mjs` / `check-doc-sizes.mjs`, the helper was `run(args, cwd, tscCmd)` and **hardcoded the repo script path**. Every "harness" case therefore executed the **REAL** script against the **REAL** repo — and the `--update` case **rewrote the committed `scripts/dev-checks/tsc-baseline.json`** to `{total:2,prod:1}`, silently poisoning both the pre-commit hook and the CI gate it is supposed to feed. Diagnosed only because the test read `baseline.total` back as `2`. The suite was green.

**Solution**: `run(script, args, cwd, tscCmd?)` — the script path is an explicit first parameter; harness cases pass `harness.script` (the tmpdir copy), real-repo cases pass the repo `SCRIPT`. Defence in depth: both suites snapshot the protected real file at module scope (`tsc-baseline.json`, `.opencode/opencode.json`) and assert it **byte-identical** in `afterAll`. Recovery: `git checkout -- scripts/dev-checks/tsc-baseline.json`, then re-verify the value.

**Rule**:
- (a) A helper that spawns a script takes the script path as an explicit parameter — never embed the production path inside the helper. A test that can silently reach real data is a bug even when green.
- (b) Mirror the real layout in a tmpdir (`<root>/scripts/dev-checks/<script>`), not just the script file — relative paths and siblings resolve like production.
- (c) For any file the code under test can WRITE, snapshot it at module scope and assert byte-identity after the run; a write-capable test must prove it did not write.
- (d) Assert on the FIXTURE's own outputs (`baseline.total`), so a stray real-repo effect shows up as a wrong number instead of a passing assertion.
- (e) Recover a clobbered tracked file with `git checkout -- <file>` and re-verify the expected value before continuing; never leave a poisoned baseline behind.

### 121. The Real Context Leak Is the Conversation Transcript, Not the Injected Docs (v3.40.0)
**Issue**: after v3.39.4 slimmed the injected instruction files to 72 KB, the session STILL filled up and looped into compaction.

**Root Cause**: the refill source was the **conversation transcript itself** — large tool outputs and long summaries are re-sent on every request, so after a compaction they are summarised but the *raw* payloads keep arriving (`compaction.reserved` was only 10000, leaving almost no headroom).

**Rule**:
- (a) Never land large output in-context: redirect it to a file, then read narrow slices or grep it. Chunk when it is too big (`scripts/dev-checks/chunk-output.mjs`).
- (b) Keep durable facts in `.remember/now.md` + the memory graph, not in the transcript — query memory before re-reading a doc.
- (c) `memory` MCP `search_nodes` is a **literal contiguous substring match over the whole query** (not tokenised/ranked/fuzzy): `"Injected context budget doc-size"` → 0 hits while `"Injected context budget"` → 1. **An empty result is NOT evidence of no memory** — retry a shorter query before re-reading a doc.
- (d) Raise `compaction.reserved` for genuine headroom; do not treat it as the cure (the cure is (a)+(b)).
- (e) **Age ≠ obsolete** (extends 119c): only age-chunk append-only logs; never age-chunk a live rulebook such as this file.

### 122. Verify a Capability Before Designing Around It — Free-Tier Subagents Are Unusable Here (v3.40.0)
**Issue**: the orchestration plan assumed parallel subagent dispatch. Every one of the four `task` subagent types failed with `Error from provider (Console): OpenCode's free tier can only be used from within OpenCode` — discovered only when trying to delegate.

**Rule**:
- (a) Probe a capability with ONE cheap call before writing a plan that depends on it — a green design that cannot execute is worthless.
- (b) Design **capability-detected** paths (Tier A parallel / Tier B chunked-sequential) so no workstream is ever blocked on an unavailable capability.
- (c) Record an environment limit as an explicit constraint (memory graph + handoff + `latest.md`) so the next agent does not burn a turn rediscovering it.
- (d) Never report a capability as demonstrated when it was not — state the limit plainly (D3/D11).

### 123. Next.js 16 Removed `next lint` — and Flat-Config Rule Overrides Must Register Their Own Plugins (v3.40.0 pre-merge)
**Issue**: `npm run lint` (`"next lint"`) failed with `Invalid project directory provided, no such directory: …\lint` (Next 16 removed the command, so `lint` was parsed as a directory). Switching to `eslint .` then crashed with `TypeError: Converting circular structure to JSON` inside `@eslint/eslintrc`'s `config-validator` — the legacy `FlatCompat` bridge chokes on `eslint-config-next` + ESLint 9.39.x. With the native flat configs it next failed with *"A configuration object specifies rule 'react-hooks/exhaustive-deps', but could not find plugin 'react-hooks'"* — flat config scopes plugins **per config object**, so the repo's rule overrides could not see the plugins the spread configs declared.

**Rule**:
- (a) On Next 16+ use `"lint": "eslint ."` with the native flat configs (`import nextVitals from "eslint-config-next/core-web-vitals"` + `nextTs from "eslint-config-next/typescript"`) — do **not** keep `FlatCompat`.
- (b) Any config object that overrides a plugin rule must import and register that plugin in the *same* object (`plugins: { "@typescript-eslint": tseslint.plugin, react, "react-hooks": reactHooks }`).
- (c) A lint gate that never ran hides real bugs — the first run surfaced 130 errors, incl. genuine `react-hooks/rules-of-hooks` violations (hooks after an early return) and `<a href>` internal links. Fix the correctness findings; downgrade only the broad legacy-debt rules to `warn` (documented) so the gate is usable.
- (d) When hoisting a `const` helper above an early return for hooks-ordering reasons, hoist the helper too — otherwise the effect's post-render callback hits a temporal-dead-zone reference in the loading state.

### 124. Git-Ignore Assertions Must Target the Committed Rule, Not an Untracked Inner Ignore File (v3.40.0 pre-merge, CI)
**Issue**: `lib/__tests__/chunk-output.test.ts` asserted `git check-ignore -q .remember/now.md` → green locally, **red on CI** (`quality-gate`: 1 failed / 1268 passed). Root cause: `.remember/` was ignored only by an **untracked** `.remember/.gitignore` containing `*` — untracked (ignored by its own rule), so it never exists in a fresh checkout, and `git check-ignore` finds no matching rule there and exits non-zero. The sibling `.context/` assertion passed because its rule lives in the committed root `.gitignore`.

**Rule**:
- (a) Any scratch/state dir that must be ignored repo-wide needs its rule in the **committed** root `.gitignore`; an inner ignore file only protects the machine that created it.
- (b) A test that reads repo state (ignore rules, tracked files, git config) must be validated from a fresh checkout — green-locally/red-on-CI is the signature of state that exists only on the author's machine.
- (c) Same class as Lesson 120: assert against committed sources of truth, never against ambient local state.

### 126. Markdown Table-Cell Escaping Needs Backslash-First Ordering — and Index Rows Only Render Line 1 (v3.40.0 pre-merge, CI/CodeQL)
**Issue**: CodeQL `js/incomplete-sanitization` flagged `escapeCell` in `scripts/dev-checks/chunk-output.mjs` — it escaped `|` but not `\`, so tool output containing a data backslash directly before a pipe (`\|`) chained onto the escaped pipe and put an unescaped `|` into the markdown row, defeating the escaping and mis-parsing the table.
**Rule**:
- (a) When escaping for markdown table cells, replace backslashes **first** (`\` → `\\`), then pipes (`|` → `\|`) — both with the `/g` flag — so a data backslash can never sit directly before the escape of a following pipe.
- (b) When writing a regression test for a render path, check what the renderer actually renders: `renderIndex` puts only `chunk.lines[0]` (the FIRST line of each chunk) into the index table. A fixture that hides special characters on lines 2+ never exercises the escaping — the first attempt failed exactly so (`x|y`/`c\d` on lines 2–3 never reached the table).

### 125. A Warning-Gate Count Must Never Use `grep | wc -l` Under `bash -e` + `pipefail` (v3.40.0 pre-merge, CI)
**Issue**: the new `quality-gate` step *"Build + Turbopack warning gate"* could never pass. `WARN=$(grep -i "warn" log | grep -v "^npm warn" | wc -l | tr -d ' ')` — when there are **zero** warning lines, `grep` exits 1, `pipefail` propagates it out of the command substitution, and `bash -e` aborts the step. It dies identically for a clean build and a dirty one, and the `build warnings: N` line never prints. Locally the snippet runs fine because there is no `-e`/`pipefail` context.

**Rule**:
- (a) Turn count-only grep pipelines into something that always exits 0: `WARN=$(awk 'tolower($0) ~ /warn/ && $0 !~ /^npm warn/ {c++} END {print c+0}' log)` (awk exits 0 unconditionally); append `|| true` to any command substitution that must tolerate no-match.
- (b) GitHub Actions runs steps as `/usr/bin/bash -e`, so an inline step that worked in an interactive shell can abort *exactly when there is no match* — exercise the step under `bash -e -o pipefail` with a CLEAN fixture (must exit 0 and print the count) and a DIRTY fixture (must exit 1 and print the matches) before pushing (git-bash: `"C:\Program Files\Git\bin\bash.exe" -e -o pipefail gate.sh fixture.log`).
- (c) A guard that cannot pass is worse than no guard — this workflow was new in the branch (`973cae5`), so main never exercised it; the first PR run exposed it. When a failed step emits no diagnostic at all (no count line), treat that as a signal the step logic itself is broken, not as a pass.

### 127. A UI-Installed Netlify Build Plugin Can Break Deploys in Ways No Repo Fix Can Undo — the Site's Plugin List Is the Source of Truth (v3.40.0 post-merge, production deploy)
**Issue**: after merging PR #127, the Netlify production deploy failed at function bundling: `Bundling of function "emails" failed` — `/.netlify/functions-internal/emails/index.js` is a CommonJS module but the closest `package.json` (repo root) is `"type": "module"` → "Either use ESM syntax, rename to .cjs, or remove type:module". The function is generated by **`@netlify/plugin-emails@1.1.1` "from Netlify app"** — installed through the Netlify UI, not present in `package.json`, `netlify.toml`, or the repo at all.
**Rule**:
- (a) When a deploy fails on a generated artifact (`functions-internal`), check **`netlify api getSite --data '{"site_id":"…"}'`** → the `plugins` array lists UI/app-installed plugins alongside repo ones (`@netlify/plugin-nextjs`, `@netlify/plugin-lighthouse`, `@netlify/plugin-emails`). The extension catalog endpoint (`get-extensions`) does NOT include legacy app plugins (plugin-emails is absent) — so the extensions MCP is the wrong lever for them; a legacy app plugin is removed only in the site UI (Site configuration → plugins, or disabling its extension).
- (b) An in-repo workaround for a UI plugin's output is a race: `netlify.toml` can pre-create `package.json` `{"type":"commonjs"}` for the generated dir in the build command (commit `11d6947`), but a plugin's **onBuild runs AFTER build.command** and rewrites its own directory — the guard is wiped and the failure returns on the next deploy. The only durable fix is removing/disabling the plugin on the site itself.
- (c) After the UI change, verify the fix BEFORE trusting the next deploy: `getSite` shows `@netlify/plugin-emails` gone from `plugins`; then watch the retried deploy (`listSiteDeploys` / `getDeploy`) → `state: ready` and the `available_functions` array contains only the app's real functions (`___netlify-server-handler`, `__csp-violations`) and no `emails`.
- (d) The Netlify deploy failure was about plugin state, NOT the code — the build log's `P6003 planLimitReached` prisma lines during static generation are non-fatal plan-hold noise (SQLite fallback keeps pages generating, 185/185). Don't chase them when the real error is a later "Bundling of function <x> failed" stage.

### 128. A Memoized Fetch Must Not Memoize Failure "Forever" — Negative-TTL Cache + Deferred Retry + Get-Count-Delta Golden Assertion (v3.40.1, production cold start)
**Issue**: production cold start served empty analytics — `getMirrorBlobsStore()` (memoized on `globalThis`) THREW during boot (Blobs region context resolves one-shot, so the transient first-call resolution failed) and the null was memoized permanently. Every later `downloadMirrorSnapshotFromBlobs()` hit the memoized `null` and bailed → the v3.39.0 disk→Blobs→fresh boot restore chain never reached Blobs on cold instances → hot routes served an empty SQLite mirror all day (recommendations/screener/corp-actions/swing blank).
**Rules**:
- (a) A failure memo is different from a value memo. Memoize SUCCESS permanently; memoize FAILURE with a short negative TTL (`MIRROR_BLOBS_STORE_NEGATIVE_TTL_MS = 60_000`, `__sqliteMirrorBlobsStoreFailedAt` timestamp) so a transient resolution failure (env not ready yet, one-shot region-context resolution) is retried without re-throwing in a tight loop.
- (b) A deferred retry must clear the failedAt marker BEFORE its own attempt: the 30s deferred restore ran into the 60s negative TTL and self-blocked until `retryDeferredMirrorRestore()` was changed to `__sqliteMirrorBlobsStoreFailedAt = 0` first. Order matters: clear the block, then try the download.
- (c) A `.unref()`'d one-shot `setTimeout` for the deferred restore keeps Jest clean — no timers keep the event loop alive after tests; also expose a test-only hook (`runMirrorBlobsRetryForTests()`) and cancel the timer in `resetMirrorBlobsOverrides()` so golden tests are deterministic.
- (d) **Golden-test assertion trap**: `mirrorHasLiveData()` guards the swap, so a "skip" golden test must assert a **get-count delta** (`blobs.getCalls.length === getsBeforeRetry`) NOT `.some(arrayBuffer)` — boot 1's `restoreMirrorSnapshot()` already probes Blobs, so the byte presence check passes trivially. When a guard makes a post-condition unobservable-by-value, assert on the call count instead.
- (e) Runtime detection: an env-var-driven check can be false on prod even when the platform IS present — Netlify prod had NO `NETLIFY` var (only `AWS_REGION=us-east-2` + `ENVIRONMENT=production`). NEW `lib/netlify.ts` `isNetlifyRuntime()` checks the full family (`NETLIFY_BLOBS_REGION`, `NETLIFY_BLOBS_CONTEXT`, `NETLIFY_SITE_ID`, `NETLIFY_DEPLOY_ID`, `NETLIFY_AUTH_TOKEN`, `ENVIRONMENT==="production" && AWS_REGION`, `globalThis.netlifyBlobsContext`). Verify with the REAL deployed environment, not the local one.

### 129. A Fallback/Mirror Branch Is a Second Implementation of the Contract — Serving Raw SQL Rows Leaks snake_case and Silently Empties Every camelCase Consumer (v3.40.1 live verification, plan-limit hold)
**Issue**: on the live site during the P6003 hold, `/api/corporate-actions/combined` took its SQLite-mirror fast path (`app/api/corporate-actions/combined/route.ts:290-297` → `NextResponse.json({ data: actions, source: "sqlite_mirror" })`; identical shape in the `sqlite_backup` catch at `:445-448`). That branch returns the RAW rows straight from `lib/sqlite.ts` `getCorporateActions()` — `SELECT * FROM corporate_action` with column-name-keyed objects (`:4993-5006`, note `company_name` used at `:5011`) → keys `ex_date` / `company_name` / `action_type` / `dividend_per_share` / `dividend_yield`. The PRIMARY Prisma path maps camelCase first (`route.ts:353-363`: `exDate`, `recordDate`, `effectiveDate`, `dividendPerShare`, `dividendYield`). The mirror branch bypassed `formatted`/`enriched` entirely.
**Symptom**: `/markets/calendar` rendered the month grid with **zero** events even though the API returned 200 and 40 Sep-17 actions were fetched — `app/markets/calendar/page.tsx:101-103` does `if (!action.exDate) return false;` and `action.exDate` is `undefined` on mirror rows → EVERY action filtered out. `/markets/analytics` corporate-action tables rendered `-` for COMPANY/TYPE/DIVIDEND/YIELD; the admin corporate-actions page likewise. Mirror rows also showed `dividend_yield: null` (40/40) and identical `ex_date` (`2026-09-17T12:00:00.000Z`).
**Rules**:
- (a) A fallback/mirror branch is a SECOND implementation of the same contract — it MUST return the same field names and types as the primary path. Route it through the SAME mapper/`formatter` instead of returning raw persistence rows.
- (b) `SELECT *` → object mapping is a shape leak: SQLite columns are snake_case, Prisma models are camelCase. Structural types (`Array<Record<string, unknown>>`) and `any`-typed responses make the divergence invisible to `tsc` AND to tests.
- (c) Diagnostic signature: **API 200 + non-empty `data` + page/table empty** ⇒ suspect a two-branch field-name mismatch, not a data problem. Check the response `source` field (`sqlite_mirror` / `sqlite_backup` vs `db`), then diff the two branches' shapes.
- (d) Date-cell bucketing must not `toISOString()` a local-midnight date: `app/markets/calendar/page.tsx:98`/`:103` build `new Date(year, month, day)` (local/IST) then `toISOString().split("T")[0]` → the PREVIOUS UTC day → off-by-one for IST viewers. Use a local-date key (`getFullYear`/`getMonth`/`getDate`) for calendar keys.

### 130. An Unguarded `.filter`/`.map` on a Non-Array Error Body Turns a Recoverable 500 Into a Blank Page (v3.40.1 live verification, `/alerts`)
**Issue**: `/alerts` crashed to a blank shell on the live site (body ≈112 chars = header only) because `GET /api/alerts` returned 500 `{ error: "Failed to fetch alerts" }` (`app/api/alerts/route.ts:57-62` — `getUserAlerts` is Prisma-only, no SQLite fallback, so the P6003 hold made it throw) and the client called `data.filter(...)` on that object (`app/alerts/page.tsx:112`; also `:230` `alerts.filter` and `:519` `alerts.map`) → `TypeError: data.filter is not a function` → React Error Boundary replaced the page.
**Rules**:
- (a) An API error body is `{ error: string }`, NOT the success shape. A client that assumes an array MUST guard with `Array.isArray(...)` before `.filter`/`.map`/`.reduce`. One unguarded call converts a recoverable 500 into a full-page crash.
- (b) For list endpoints whose failure is already signalled by the status code, returning the success SHAPE (`[]`) on a degradable read lets content degrade instead of the app.
- (c) Verification tell: a "blank page, header only" in a walkthrough = an Error Boundary fired. Find the console `X.filter is not a function`, then trace which fetch fed the non-array.
- (d) Add the guard test that the e2e suite lacks: mock `fetch` → `{ ok: false, status: 500, json: () => ({ error }) }` and assert the page renders an error state rather than throwing. The suite logs in but never simulates hold-induced API failure, which is why this shipped.

### 131. An Unbounded Client Poll Against a Failing Endpoint Amplifies an Outage Into a Request Storm (v3.40.2, admin Workers tab under the P6003 hold)
**Issue**: `/admin/utils/workers` polled `/api/admin/workers` + `/api/admin/workers/status` on a fixed 10 s `setInterval`. Under the plan-limit hold BOTH 500'd, so every tick logged server + client errors — 186-188 console errors observed live on a single page visit — and kept hammering a database that was already refusing work. A fixed interval has no memory of failure: it retries at full rate forever.
**Rules**:
- (a) A polling client MUST back off on failure: self-schedule with `setTimeout` and grow the delay (10 s → 20 s → 40 s → 60 s cap) while requests fail, resetting to the base interval on the first success. `setInterval` cannot express this.
- (b) Give the user a visible tell once backoff kicks in ("Live updates paused after repeated failures — retrying with backoff.") — silent degradation looks identical to a frozen UI.
- (c) A poll loop must have a cancellation flag checked AFTER each `await` (`if (cancelled) return`) inside the effect cleanup, or a slow response can schedule a timer after unmount.
- (d) Healthcheck endpoints consumed by a poller deserve the same mirror fallback as the page they feed — fix the endpoint (v3.40.2 gave `workers/status` a SQLite mirror) AND the client (backoff), otherwise one hard-failing dependency still storms the server.

### 132. JS ISO-Date Builders Need a 2-Digit Day — `new Date("2026-09-1T…")` Is Invalid → `toISOString()` Throws → a Fail-Open Guard Silently Returns `null` (v3.40.3, predeploy mirror-backup)
**Issue**: the versioned backup key + content hash in `lib/services/mirrorBackup.ts` were built from `new Date().toISOString()`; a test helper assembled an "as-of" moment with `new Date(\`${y}-${m}-${d}T${h}:${min}:${s}Z\`)` — interpolating single-digit day/month (e.g. `2026-09-1T…`) → `Invalid Date` → `.toISOString()` THREW → the wrapped `try/catch` fail-open path returned `key: null`. The test then saw a "silent no-backup" where a backup was expected, and the assertion failure pointed at the WRONG layer (the store mock) because the throw was swallowed by design.
**Rules**:
- (a) Always build ISO timestamps with `String(part).padStart(2, "0")` — `new Date("2026-09-1T12:00:00Z")` is `Invalid Date` in V8; only `2026-09-01` parses. A shared `dayIso`/`pad2` helper in the test makes it impossible to regress.
- (b) A fail-open function (`createMirrorBackup` returns `key | null` on empty/throw) makes a DateTime/format bug look like "runtime degradation" — assert on the produced KEY format (`new RegExp(...)`) in tests, and keep the key-building code OUT of the swallow-all try/catch when it can't throw for business reasons.
- (c) Test-trap: when the failing helper is called from ANOTHER module's test, the error surfaces in the wrong file — verify the constructed value directly in the helper's own suite (unit-test `mirrorBackupKey()` + timestamp parsing before testing the orchestration path).

### 133. A Deploy-Time Call Back Into the Live App Must Be Orderly, Bell-Curve Gated and Always Green (v3.40.3, predeploy mirror-preservation guard)
**Issue**: Netlify builds the new instance from scratch on every deploy — during the P6003 plan-limit hold (until 2026-10-02) NO build-time step preserved the live SQLite mirror (outbox + snapshot + Blobs backups) or drained pending mirror rows to Prisma, so a deploy could cold-start with an empty mirror (same served-empty class as v3.40.1). The guard is a build-step Node script that calls the LIVE production instance's admin route before the new build serves traffic.
**Rules**:
- (a) **Order the mutation sequence explicitly** — `getOutboxPending()` → snapshot (persist + export + upload Blobs) → versioned backup → **only if the plan-limit breaker is CLOSED** → `pushSqliteToPrisma`. On a hold the guard must deliberately degrade to `backed_up`, not fail the build.
- (b) A build-time integration step is **fail-open**: any error → warn + **exit 0**. A deploy must never be blocked by the preservation attempt; `npm run quickbuild` remains the real gate.
- (c) Authenticate the build→app call with a dedicated env token (`DEPLOY_GUARD_TOKEN`, 64-hex, `crypto.timingSafeEqual` after a length check; 503 `guard_token_not_configured` when the token env is unset) + admin-session fallback — never rely on an unauthenticated admin route.
- (d) Keep versioned backups + retention (`MIRROR_BACKUP_KEEP = 5`) so a restore point survives multiple deploys; Blobs `list`/`delete` are optional-capability → prune error-tolerantly.
- (e) The script must print ONE summary line (`mode=… pending=… pushed=… backup=… pruned=…`) and NEVER log the token.

### 135. A tsc-Baseline Harness That Copies the Check Script Into a Temp Dir Loses the Root tsconfig.json — Unguarded readFileSync → ENOENT (v3.41.0, decision-engine suite; discovered in `lib/__tests__/check-tsc-baseline.test.ts`)

**Issue**: `check-tsc-baseline.test.ts` failed NOT on gate logic but at the harness boundary: `scripts/dev-checks/check-tsc-baseline.mjs` `buildGateTsconfig()` (:47) calls `readFileSync(resolve(ROOT, "tsconfig.json"))` with no existence guard, and `makeHarness()` copies the script + committed `tsc-baseline.json` into a temp dir WITHOUT the root `tsconfig.json` → every harness case that reached `buildGateTsconfig()` threw `ENOENT`. The 46-error baseline gate itself still passed (per-dir comparison) — the suite was red for an environmental reason.

**Rules**:
- A check script that reads repo files must guard its reads (`existsSync` before `readFileSync`) — OR its harness must supply those files; here the correct fix was the **harness**: `makeHarness()` now writes a stub `tsconfig.json` (`{ "include": ["**/*.ts"], "exclude": [] }`) so copy-into-temp-dir harnesses behave like the repo.
- When a harness copies the script but not its inputs, every harness case fails identically — suspicion falls on the harness, not the gate logic; reproduce the harness layout before touching the script.
- Fixed at discovery; `check-tsc-baseline.test.ts` 9/9 green, still enforces the 46-error baseline.

### 136. A Server-Side Credentials Sign-In Can Succeed While the Browser Never Leaves `/auth/signin` — Retry-With-Probe Login, Don't Assert the Form (v3.41.1, `e2e/decision-monitoring.spec.ts`, first WebKit login in the suite)
**Issue**: `decision-monitoring.spec.ts` was the FIRST e2e spec to log in on **WebKit** (auth.setup + login.spec are chromium-only, so the WebKit login class was never exercised). Under the single-threaded dev-server, the server-side credentials sign-in SUCCEEDED (dev-server.log showed `Auth: Login successful ... userId=1` on every admin POST) but the `signIn("credentials", {redirect:false})` **302 + Set-Cookie (JWE ≈1900 chars) response never completed client-side** — WebKit stayed on `/auth/signin` with the form still filled and NO error banner. Asserting "submitted the form with correct creds" was green; the page never navigated.
**Rules**:
- A credentials-login helper must be **resilient, not asserting**: fixed attempts (3) + a successful-navigation probe (12 s URL probe, 45 s on the last attempt) → on stall: reload → check the session cookie/URL → re-submit. Never "fill + click + expect URL" for browser-conditional flakiness classes (WebKit, single-threaded dev-server, huge JWE cookies).
- When an auth call fails with NO banner, check the SERVER log before blaming credentials — `Auth: Login successful` + stuck form ⇒ the response never arrived (transport), not auth failure.
- Login specs stay chromium-only in this repo; any spec that must log in on a second browser engine carries the stall risk — reuse the resilient helper rather than inlining `fill`/`click`/`expect`.
- E2E specs must NOT hardcode credential literals (user directive) — read from env (`E2E_ADMIN_EMAIL || ADMIN_EMAIL`, `E2E_ADMIN_PASSWORD || ADMIN_PASSWORD`) and `test.skip()` when unset; `e2e/` is in `SECRETS_SCAN_OMIT_PATHS` but the spec carries no literals by design anyway.
- Strict-mode locator trivia: pages with duplicated labels (the AI grid and the new Decision Engine tab both render "Success Rate"/"Avg Latency") need `.last()` — strict mode throws on 2 matches.

### 138. A P6003 Plan-Limit-Hold 500 Masquerades as an Empty State — Read Paths Whose Write Path Mirrors MUST Read the Mirror Back; a Fallback That Can't Serve Must Rethrow the ORIGINAL Error (v3.41.2, Spec 01, `/recommendations` History/Performance/Ideas + `lib/services/syncedDataService.ts`)
**Issue**: under the Prisma plan-limit hold (P6003, breaker open ~2026-09-24 → ~2026-10-02) the prod /recommendations History/Performance/Ideas tabs 500'd with `{error}` on every load: the READ paths (top-stocks `$queryRaw`, `recommendationPerformanceService`, `syncedDataService`) were Prisma-only while the WRITE path WAS mirroring the same rows into SQLite — real product data existed in the mirror, no code read it. The client then turned the 500 into a **masked 500-as-empty-state**: `data.success === false` → silently render "no recommendations yet" (no error card, no retry) so the outage looked like "nothing generated today". Two independent bugs: (1) read/write asymmetry — a write path that mirrors MUST have a mirror read fallback for every consumer route; (2) a failed fetch was treated as an empty list by the UI.
**Rules**:
- **Read the mirror back**: any route whose write path goes through `syncedDataService` gets a breaker/hold-aware mirror read fallback (`mirrorReadMarketCache`); a write-only mirror is a data tomb, not a backup. Step ordering: getSqliteFallback → mirror query → serialize → cache (1 h) → degraded response; Prisma happy path stays byte-identical.
- **Never mask a 500 as an empty state**: a failed fetch (`success === false` or throw) must render an explicit error card + Retry BEFORE the empty-state branch — a user seeing "no recommendations yet" during an outage is seeing a lie. (Extension of Lesson 130 — that was a blank page; this is the silent-empty variant.)
- **A fallback that can't serve must RETHROW THE ORIGINAL error** (not a friendlier `[]`): if the mirror is exhausted/not-ready the API returns the SAME 500 the caller would have gotten — degraded `[]` is only correct when the mirror genuinely has data. Non-hold DB errors (real bugs) must propagate as-is; gate the fallback purely on `isDbUnavailableError` (message/code-based → matches P6003 "hold on your account").
- **Zero Prisma ops in fallback branches**: the fallback path must never touch Prisma (createMany-count rules, plan-op discipline) — it exists specifically FOR the hold.
- **A mock mirror getter returns rows independent of SQL args**: when the fake `sql.js` tracker returns data regardless of `where`/`orderBy`, the JS-side filter/order you moved into the fallback must STILL mirror the SQL semantics exactly (status filter, timerange, newest-first) or the degraded shape silently diverges — test it with fixtures that violate the SQL args.
- **Windows/TS**: BigInt literals (`10n`) fail `tsc` here — use `BigInt(10)`.
**Fix**: NEW `lib/sqlite.ts getRecommendationRuns()` + `topStocksFromSqlite()` (serialize → 1 h cache) + `getPerformanceListFromSqlite()` (`listItemFromMirrorTracker`, JS status filter) + breaker/hold-aware `mirrorWriteThrough`/`mirrorReadMarketCache` (steps 2+3); HistoryTab explicit `error` card + Retry (`"Failed to load recommendations history"`, rendered before the empty list). New suite `recommendationsPlanLimitFallbacks.test.ts` 21/21; full 109/109 suites (1440 pass / 4 skip / 0 fail), tsc 46 exact (0 new), quickbuild 189/189, e2e recommendations.spec.ts 10/10 (live dev server).

### 139. An ESLint disable-Directive for a Rule That Is NOT Registered in the Native Flat Config IS a Config Error — "Five Clean Comments" Bred Three New Errors (v3.41.3, laya suites)
**Issue**: the NEW weights-gated laya suites reused `// eslint-disable-next-line jest/no-disabled-tests` comments (copied from legacy jest-plugin-era guidance) above the existing 4 static `describe.skip` clients; `npm run lint` went 0 → 3 errors: `Definition for rule 'jest/no-disabled-tests' was not found`. The repo's `eslint.config.mjs` is a NATIVE flat config (Next 16 removed `next lint`, Lesson 123) that registers `next`/`react`/`react-hooks`/`@typescript-eslint` ONLY — no jest plugin; ESLint 9 resolves every disable directive against the ACTIVE config and throws for an unknown rule, so the "clean comments" themselves were the errors.
**Rules**:
- Check the config BEFORE copying a disable directive — `npx eslint <file> --print-config` shows exactly which rules are live per file; copying comments from older files/era guidance is how a "5-line cleanup" becomes 3 new errors.
- Data-gated skipping needs NO jest plugin: `const maybeDescribe = weightsAvailable ? describe : describe.skip;` then `maybeDescribe("...", () => {...})` — the file keeps its lint surface, the skip is data-driven and comment-free (matches the D5 skip precedent; the 4 legacy IndexedDB skips are static `describe.skip`, still fine).
- Touching a legacy disable directive to "fix lint" can silently re-enable suppressed debt — if the rule isn't registered the directive is a config error; if it IS registered as warn, deleting it surfaces the warn (conservative: leave legacy directives alone unless they error).
- Fix here = delete the 3 comments only (`layaTokenizer.test.ts`, `layaAgent.test.ts`, `layaDecisionModel.test.ts`); keep the `maybeDescribe` gates. lint back to 0 errors / 1155 pre-existing warnings.

### 140. A Shell Command Built from Environment Values Trips CodeQL — Run Child Processes Shell-Free via execFileSync + argv (v3.41.3, check-tsc-baseline)
**Issue**: PR #132's CodeQL `js/shell-command-injection-from-environment` (Medium) flagged `scripts/dev-checks/check-tsc-baseline.mjs` — the gate called `execSync()` with a command string that interpolated the `TSC_BASELINE_CMD` env var (the test seam) and a computed path (`npx tsc --noEmit -p <gateTsconfig>`); CodeQL treats both env-derived content and interpolated absolute paths in a shell command as injection surface.
**Rules**:
- Prefer `execFileSync`/`spawn` with an **argv array** over `execSync`/`exec` with a string — no shell, so metacharacters and interpolated env/paths are inert; CodeQL's shell-injection rules match no sink.
- Don't shell out to `npx` for the real path: run the LOCAL shim directly — `process.execPath` + `resolve(ROOT, "node_modules/typescript/bin/tsc")` — same binary `npx` resolves, no PATH lookup, no shell string.
- Keep a test seam shell-free too: tokenize the env string into argv (double-quote-aware split) AND gate it (`NODE_ENV === "test"` — Jest propagates it via the harness `env: {...process.env}`) so the env var is **inert outside the test harness**. Add a test pinning the real shim path (`existsSync("node_modules/typescript/bin/tsc")`).
- Fix here: `execSync` → `execFileSync`, `splitArgs()` quote-aware tokenizer, seam gated on `NODE_ENV=test`, default = node + local tsc shim. Suite 10/10; real run `total 46 (delta +0) / prod 0` OK; full **116/116 suites (1539 pass / 4 skip / 0 fail)**; lint 0 errors (1155 pre-existing warnings).

## Update Log
- 2026-09-25: Added Lesson 140 (v3.41.3 — PR #132 CodeQL `js/shell-command-injection-from-environment` medium on `scripts/dev-checks/check-tsc-baseline.mjs`). **A shell command built from environment values trips CodeQL — run child processes shell-free**: the gate called `execSync()` with a command string interpolating the `TSC_BASELINE_CMD` test-seam env var and a computed path (`npx tsc --noEmit -p <gateTsconfig>`). Fix = `execFileSync(bin, argsArray)` (no shell at all), default invocation = `process.execPath` + local `node_modules/typescript/bin/tsc` (what `npx` resolves, minus the shell/npx lookup), and the env seam tokenized with a double-quote-aware `splitArgs()` and gated to `NODE_ENV === "test"` so it is inert in the pre-commit hook/CI; pinned with a new `existsSync` test. check-tsc-baseline suite **10/10**, real gate run OK (46/0), full **116/116 suites (1539 pass / 4 skip / 0 fail)**, lint 0 errors / 1155 warnings. Context: v3.41.3 = spec 18 = Laya real inference (commit `3c765b9`, PR #132); CodeQL was reviewed during the merge-decide step.
- 2026-09-25: Added Lesson 139 (v3.41.3 Spec 18 Laya real inference — discovered as 3 NEW eslint errors from "innocent" disable-directives). **An ESLint disable-directive for a rule that is NOT registered in the native flat config IS an error**: `eslint.config.mjs` (Lesson 123, Next 16 removed `next lint`) is a native flat config with NO jest plugin, so `// eslint-disable-next-line jest/no-disabled-tests` in 3 NEW laya suites threw `Definition for rule 'jest/no-disabled-tests' was not found` — check `npx eslint <file> --print-config` before copying directives; keep data-gated `maybeDescribe = weightsAvailable ? describe : describe.skip` instead of disable comments. Context: v3.41.3 = spec 18 = NEW `lib/services/laya/` (P1 pure-TS ports of every decode algorithm, Python 1:1; P2 `tokenizer.ts` WASM lazy-singleton — v1 special ids CLS 50281/SEP 50282/PAD 50283/MASK 50284, `add_special_tokens=false` parity; P3 `decisionModel.ts` chained ONNX encoder→head, onnxruntime-node@1.30.0, int64/bool casts, K≥2 pad, `act_logits` graph-name read — spike camelCase-key bug corrected, probe hidden-1024, lazy singleton) + `agent.ts` systemOne decode + `LayaRealProvider` behind `DECISION_LAYA_REAL=1` parity gate (laya-mock default byte-identical); Jest vm × native-realm → child-process probe `scripts/dev-checks/laya-forward.ts`; weights `lib/services/laya/weights/v1/` gitignored + `scripts/fetch-laya-weights.mjs`; NEW 7 laya suites; full **116/116 suites / 1538 pass / 4 skip / 0 fail**; tsc **46 exact (0 new)**; lint **0 errors (1155 pre-existing warnings)**; quickbuild **189/189**; doc budget **85.4/100 KB**; live real-ping + HTTP ping user-accepted (plan step 21); commit as v3.41.3 pending user approval.
- 2026-09-25: Added Lesson 138 (v3.41.2 Spec 01 recommendations plan-limit fallbacks — discovered via the prod `/recommendations` 500s under the P6003 hold + the HistoryTab masked-500-as-empty-state). **A P6003 plan-limit-hold 500 masquerades as an empty state**: read paths whose write path mirrors MUST read the mirror back (`getRecommendationRuns` → `topStocksFromSqlite`/`getPerformanceListFromSqlite`/`mirrorReadMarketCache`); a fallback that can't serve must RETHROW the ORIGINAL error (degraded `[]` only when the mirror HAS data — never mask a 500 as empty, Lesson 138 extends Lesson 130 blank-page class to the silent-empty variant); only `isDbUnavailableError` caught (message/code-based → P6003); zero Prisma ops in fallbacks; a mock mirror's tracker getter ignores SQL args so the JS-side status/timerange filtering must mirror SQL semantics; HistoryTab error card + Retry rendered BEFORE the empty-state list (same as PerformanceTab). Context: v3.41.2 = spec 01 = NEW `lib/sqlite.ts` getRecommendationRuns + top-stocks route fallback (serialize → 1 h cache) + recommendationPerformanceService `listItemFromMirrorTracker`/`getPerformanceListFromSqlite` (JS status filter, timerange) + syncedDataService steps 2+3 breaker/hold-aware mirror write-through/read; NEW `recommendationsPlanLimitFallbacks.test.ts` **21/21**; full **109/109 suites / 1440 pass / 4 skip / 0 fail**; tsc **46 exact (0 new)**; lint 0; quickbuild **189/189**; e2e **recommendations.spec.ts 10/10** (live dev server); commit as v3.41.2 pending user approval.
- 2026-09-24: Added Lesson 137 (v3.41.1 auth-gate hardening — discovered via the full-suite e2e auth flake, first seen in `e2e/auth.setup.ts` at `feature/ph22-decision-engine` head). **Auth.js (NextAuth v5) double-submit CSRF race**: on RTL page load (SessionProvider + `useSession`) TWO concurrent no-cookie `GET /api/auth/session` requests fire while the single-threaded dev server is busy; each mints its own csrf token cookie (trace: token A `6bc47ccd…` set at 31.749, token B `c9a217c3…` at 31.761) and the browser jar keeps the LAST-arriving Set-Cookie (B); `signIn()`'s csrf GET (carrying jar=B) returns body token A; the credentials POST sends cookie B vs body A → `csrfTokenVerified=false` → `MissingCSRF` → the signin page renders `"Invalid email or password"` even though the credentials are correct. Playwright retries in `playwright.config.ts` CANNOT fix it — every config retry opens a FRESH clean context that re-rolls the same race. Fix = **in-context resubmit loop** (max 2 attempts): after the first POST's banner appears, the jar holds ONE settled token, so a second click succeeds (its csrf GET carries the settled cookie); loop = click → `expect(page).toHaveURL` (12 s probe attempt 1, 45 s attempt 2) → on catch: attempt 2 → throw; "Invalid email or password" visible → continue WITHOUT reload (re-submit, jar already settled); otherwise reload → re-fill → retry. Applied to `e2e/auth.setup.ts` (all browsers) + `e2e/login.spec.ts` demo-login test. Context: v3.41.1 = spec 17 = Decision Engine monitoring + e2e hardening; root-cause evidence = Playwright `page.on("request")`/`response` trace + jar state (`getState().cookies` post-POST single `__Secure-authjs.csrf-token`); full e2e (headless, 5 projects) re-verified green on auth gate — **86 passed / 1 failed (pre-existing `navigation.spec.ts` News-webkit dev-server-starvation waitForURL flake) / 2 did-not-run (serial-group skips)**; tsc **46 = exact baseline**; lint 0; unit **108/108 suites / 1416 pass / 4 skip / 0 fail**; quickbuild OK; commit pending user approval.
- 2026-09-24: Added Lesson 136 (v3.41.1 Spec 17 Decision Engine monitoring + e2e hardening — discovered via `e2e/decision-monitoring.spec.ts`, the first spec in the suite to log in on WebKit). **A server-side credentials sign-in can succeed while the browser never leaves `/auth/signin`**: under single-threaded dev-server load the credentials `signIn` 302 + Set-Cookie (JWE ~1900 chars) response never completes client-side (WebKit); dev-server.log showed `Auth: Login successful userId=1` on EVERY admin POST while the page stayed on the signin form with no banner → fix = resilient `loginAsAdmin()` (3 attempts, 12 s URL probe / 45 s last, reload → session re-check → re-submit) + `pingAdmin()` one inline retry + strict `.last()` on duplicated "Success Rate"/"Avg Latency" labels (AI grid renders the same labels). Also per user directive: e2e specs must NOT hardcode credential literals — `E2E_ADMIN_EMAIL/PASSWORD || ADMIN_EMAIL/PASSWORD`, `test.skip` when unset. Context: v3.41.1 = spec 17 = NEW `lib/services/decision/monitoring.ts` (zero-Prisma ring buffer max 500 + stats aggregation) wired into client evaluate/ping + POC A/B + `GET/DELETE /api/admin/decision/monitoring` + Decision Engine tab in AI Monitoring + OpenAPI; full **108/108 suites / 1416 pass / 4 skip / 0 fail**; tsc **46 = exact baseline**; lint 0; quickbuild 189/189; full e2e **94 passed / 1 flaky (pre-existing nav flake) / 0 failed**; commit pending user approval.
- 2026-09-24: Added Lesson 135 (v3.41.0 Spec 16 Decision Engine — discovered via `check-tsc-baseline.test.ts` in the decision-engine suite). **A check script must guard its repo reads, or its harness must supply the files**: `scripts/dev-checks/check-tsc-baseline.mjs` `buildGateTsconfig()` :47 does an unguarded `readFileSync(resolve(ROOT, "tsconfig.json"))` while `makeHarness()` copies the script into a temp dir without the root `tsconfig.json` → every harness case threw ENOENT at the harness boundary (the 46-baseline gate still passed). Fix = `makeHarness()` writes a stub `tsconfig.json` (`{ "include": ["**/*.ts"], "exclude": [] }`); suite 9/9 green. Context: v3.41.0 = spec 16 Decision Engine core (Laya-only mock engine, confidence-gated ACT/REVIEW) + POC A/B + evaluate/ping routes + admin panel; full **107/107 suites / 1401 pass / 4 skip / 0 fail**; tsc **46 = exact baseline**; lint 0; quickbuild 188/188; commit pending user approval.
- 2026-09-22: Added Lesson 134 (v3.40.5 Laya / System-One decision-engine RESEARCH — docs-only). **Verify vendor/link claims BEFORE wiring them into a design**: "Jev is open-source" premise disproved — `github.com/answers-ai/jev` is a **404**; Jev 1.13.0 is TypeSafe AI's proprietary hosted API (`POST https://api.typesafe.ai/v1/systemone`, model `jev-latest`) and Laya is the only Apache-2.0 local alternative (HF `convaiinnovations/laya`, non-autoregressive System-1, RLCD, ~33 ms/pass, never generates text). **Primary source blocked ⇒ recover the claim, and say so**: the Medium article is HTTP **403** (Cloudflare) — search-result snippets are the recoverable evidence, and the gap must be recorded in the research doc, not papered over. **Research = durable artifact, not a session**: parallel-subagent findings collapsed into `memory.md` (NOT an injected instruction file — doc budget verified 75.6/100 KB) + `docs/laya.md` + `docs/designDoc/ph22-laya-decision-engine-design.md`, so future sessions never re-scrape. Evidence trail: HF card + docs.typesafe.ai (full tree) + api.typesafe.ai + console.typesafe.ai = reachable; medium.com = 403 (snippets only).
- 2026-09-19: Added Lessons 132-133 (v3.40.3 predeploy mirror-preservation guard, Spec 14, branch `feature/predeploy-mirror-preserve` on `main` tip `27c0770`). **132** — JS ISO-date builders need a 2-digit day: `new Date("2026-09-1T…")` is `Invalid Date` → `.toISOString()` throws → the fail-open `createMirrorBackup` silently returned `key: null` and the test failure pointed at the wrong (store-mock) layer; fix = `String(day).padStart(2, "0")` shared helper + direct unit test of `mirrorBackupKey()` format. **133** — deploy-time call back into the live app: explicit ordering `getOutboxPending` → snapshot (persist+export+upload) → versioned backup → **breaker-closed-only** `pushSqliteToPrisma` (hold → `backed_up` by design); fail-open warn + exit 0 (build never blocked; quickbuild is the gate); `DEPLOY_GUARD_TOKEN` 64-hex + `timingSafeEqual` + length check (503 when unset+presented) + admin-session fallback; `MIRROR_BACKUP_KEEP=5` versioned `backups/sqlite-mirror-<ISO>.sqlite` + fail-tolerant prune; one-summary-line output, token never logged. Phase 1 `lib/sqlite.ts` edits (`SyncTrigger "deploy"`, `MirrorBlobsStoreLike` widened, `getMirrorBlobsStore` exported); NEW `lib/services/mirrorBackup.ts` (14 tests) + `app/api/admin/predeploy/preserve/route.ts` (GET read-only diagnostics + POST execution, 11 tests, `runtime="nodejs"`+`dynamic="force-dynamic"`) + `scripts/predeploy/preserve-mirror.mjs` (build guard, URL chain `DEPLOY_PRIME_URL`→`URL`→`NEXT_PUBLIC_BASE_URL`→fallback); `netlify.toml` build command = `node scripts/predeploy/preserve-mirror.mjs && npx prisma generate && npm run quickbuild`; `.env.example` `DEPLOY_GUARD_TOKEN=`; OpenAPI `/api/admin/predeploy/preserve` (tags `Admin - Deploy`); `DEPLOY_GUARD_TOKEN` upserted via Netlify MCP `manage-env-vars` (production, builds+runtime, site `78401e5d-b137-4b6d-94bb-ad1ec8de6b05`, temp file deleted — never echoed). Verified: full **99/99 suites / 1334 pass / 4 skip / 0 fail** (25 new tests), `tsc` **46 = exact baseline (prod 0)**, lint **0 errors** (4 new TS files clean), doc budget **74.9/100 KB**, `quickbuild` **185/185**. Legacy finding (record-only): `.github/workflows/deploy.yml` publishes `./out` via `nwtgck/actions-netlify@v3.0` — NOT the real netlify.toml → quickbuild → `.next` path. Corrected stale v3.40.1/v3.40.2 "PENDING USER" rows → MERGED via PRs #128 (`15fa0a3`) / #129 (`38ee7da`).
- 2026-09-18: Added Lesson 131 (v3.40.2 — the admin Workers tab polled `/api/admin/workers` + `/api/admin/workers/status` on a fixed 10 s `setInterval`; under the P6003 hold both 500'd → 186-188 console errors per visit + a request storm against an already-refusing DB; a fixed interval has no memory of failure. Fix = self-scheduling `setTimeout` backoff (10→20→40→60 s cap, reset on success) driven by `fetchData()` returning `Promise<boolean>`, a `pollPaused` "retrying with backoff" hint at ≥3 failures, and a `cancelled` flag checked after each `await`; the endpoint itself also gained the same SQLite mirror fallback as its consumers, because fixing only the client still storms a hard-failing dependency). Also fixed in the same branch: BUGS 15/16/17 — NEW shared `lib/services/corpActionMirror.ts` `mapMirrorCorporateAction` applied to BOTH mirror branches of `/api/corporate-actions/combined` (a fallback branch is a second implementation of the contract, Lesson 129) + calendar local `toDayKey()` (IST off-by-one); `/api/alerts` session-scoped mirror fallback + `/alerts` `Array.isArray`/`fetchError` guard; `/api/admin/workers/status` GET+POST admin auth + mirror fallback; `dividendCalendarService.fetchDividends` mirror fallback; 4 NEW suites / 26 tests; full 97/97 suites / 1309 pass / 4 skip / 0 fail; tsc 46/46 prod 0; doc budget OK 76,669/102,400 B; lint 0 errors.
- 2026-09-18: Added Lessons 129-130 from the **v3.40.1 live-site verification** (admin OTP flow; findings verified against source). **129** — the `/api/corporate-actions/combined` SQLite-mirror fast path (`route.ts:290-297`, `source:"sqlite_mirror"`; `:445-448` `sqlite_backup`) returns RAW rows from `lib/sqlite.ts getCorporateActions()` (`SELECT * FROM corporate_action` → snake_case `ex_date`/`company_name`/`action_type`/`dividend_per_share`/`dividend_yield`) while the primary Prisma path maps camelCase (`route.ts:353-363`) → `app/markets/calendar/page.tsx:101-103` (`if (!action.exDate) return false`) drops EVERY action → empty month grid despite 40 fetched rows; analytics/admin corp-action tables render `-`; a fallback branch is a second implementation of the contract and must reuse the same mapper; `SELECT *` object-mapping is a snake_case shape leak invisible to `tsc` (structural types); diagnostic = API 200 + non-empty `data` + empty page ⇒ diff the two branch shapes via the response `source`; plus calendar `toISOString()` on local-midnight → IST off-by-one. **130** — `/alerts` blank-page crash: `GET /api/alerts` 500 `{error:"Failed to fetch alerts"}` (`app/api/alerts/route.ts:57-62`, Prisma-only) + unguarded `data.filter(...)` (`app/alerts/page.tsx:112`; also `:230`/`:519`) → `TypeError` → React Error Boundary; list clients must `Array.isArray` guard (or the API return `[]` on degradable reads); "blank page, header only" = Error Boundary; add the mocked-500 guard test the e2e suite lacks.
- 2026-09-18: Added Lesson 128 (v3.40.1 production cold-start — `getMirrorBlobsStore()` memoized a transient store-resolution THROW as a permanent `null`, so the v3.39.0 disk→Blobs→fresh restore never reached Blobs and hot routes served an empty SQLite mirror; fix = negative-TTL failure caching (`MIRROR_BLOBS_STORE_NEGATIVE_TTL_MS` 60s, `__sqliteMirrorBlobsStoreFailedAt`) + one-shot `.unref()`'d 30s deferred restore (`scheduleDeferredMirrorRestore`/`cancelDeferredMirrorRestore`/`retryDeferredMirrorRestore`/`runMirrorBlobsRetryForTests`) that clears failedAt BEFORE downloading (30s retry vs 60s TTL self-block) + `mirrorHasLiveData()` swap guard; golden skip-test asserts `blobs.getCalls.length === getsBeforeRetry` get-count delta NOT `.some(arrayBuffer)` because boot 1's restore already probes Blobs; NEW `lib/netlify.ts` `isNetlifyRuntime()` — prod has no `NETLIFY` env, only `AWS_REGION`+`ENVIRONMENT=production`; full suite 93/93 / 1283 pass / 4 skip; tsc 46 exact; quickbuild 185/185; pre-existing Jest teardown noise proven pre-existing via isolated 16/16 backtestDataService run). (Netlify production deploy failure after PR #127 merge — `Bundling of function "emails" failed`: UI-installed legacy plugin `@netlify/plugin-emails@1.1.1` "from Netlify app" generates a CommonJS `emails/index.js` under repo-root `"type": "module"`; the plugin is absent from the repo AND from the `get-extensions` catalog so the extensions MCP cannot remove it; `netlify api getSite` → `plugins` array is the source of truth (nextjs + lighthouse only AFTER the user disabled the Emails extension in the Netlify UI); in-repo guards race the plugin — `netlify.toml` pre-creating `.netlify/functions-internal/emails/package.json` `{"type":"commonjs"}` (commit `11d6947`) is wiped because the plugin's onBuild regenerates its dir after build.command (prior attempts `dbeba23`/`112b281` also failed); after the UI fix the retried deploy `6aacebd948646f5a24068933` went `ready` on `commit_ref 898a3f6` with available_functions = only `___netlify-server-handler` + `__csp-violations`, secrets scan 900/0 — **v3.40.0 now live**; also PR #127 merged (merge `48df4d4`, main tip `898a3f6`), post-merge cleanup = `git fetch --prune` + delete local `feat/agentic-context-orchestration`). Verify deploys via `listSiteDeploys` / `getDeploy` (deploy id ≠ build id — getDeploy wants the `id` field, not `build_id`).
- 2026-09-18: Added Lesson 126 (CodeQL `js/incomplete-sanitization` — `escapeCell` in `scripts/dev-checks/chunk-output.mjs` escaped `|` but not `\`, so tool output carrying a data backslash before a pipe (`\|`) chained onto the escape and put an unescaped `|` into the markdown index row; fix = backslash-first ordering `value.replace(/\\/g, "\\\\").replace(/\|/g, "\\|")` — both `/g` — with the rationale in a comment; regression test = 3 fixtures, one per case (`x|y` → `x\|y`, `c\d` → `c\\d`, `a\|b` → `a\\\|b`), each as the chunk's FIRST line because `renderIndex` only renders `chunk.lines[0]` — the first fixture attempt put `x|y`/`c\d` on lines 2–3 which never reach the table; full suite re-run alone **92/92, 1270 passed / 4 skipped**). Also: branch cleanup (user-directed) — `fix/turbopack-tracing-harness` confirmed an ancestor of PR #127's branch and deleted, `feat/sqlite-durable-mirror` fully merged in `main`, `fix/sqlite-upsert-worker-undefined-bind`'s `85c18d9` content byte-identical to `797e449` — all three deleted local+remote; 54 local branches fully contained in `origin/main` deleted (verified per-branch via `git merge-base --is-ancestor`); LOST set (`buildfix`, `docs`, `feat8`, `fix/cleanup-todos-bugs`, `fix1`, `fix2`, `ph9`–`ph18`, `` ph`7 ``) kept — deleting would orphan unmerged commits; remotes now = `main` + PR #127 branch + 2 dependabot, PR #127 kept until merged.
- 2026-09-18: Added Lesson 125 (the W7 `quality-gate` "Build + Turbopack warning gate" could never pass — `WARN=$(grep -i "warn" … | grep -v "^npm warn" | wc -l | tr -d ' ')` exits 1 on ZERO matches and `bash -e` propagates it, so CI aborted after every build without printing `build warnings:`; reproduced locally under git-bash `-e -o pipefail`; fix = `awk 'tolower($0) ~ /warn/ && $0 !~ /^npm warn/ {c++} END {print c+0}'` (always exit 0) + `|| true` on the failure-branch `grep | head`; verified clean fixture → exit 0 "build warnings: 0" (npm-warn line excluded) and dirty fixture → exit 1 with both warnings listed). PR #127 run 2: `Run tests` green (the `.remember/` fix) — `quality-gate` then failed only on the gate bug; `context-budget` + `tsc-baseline` + CodeQL pass.
- 2026-09-18: Added Lesson 124 (CI-only failure in `lib/__tests__/chunk-output.test.ts` — `git check-ignore -q .remember/now.md` passed locally but failed in the `quality-gate` job because `.remember/` was ignored only by an untracked `.remember/.gitignore` (`*`), which a fresh checkout never has; fix = `/.remember/` added to the committed root `.gitignore` beside `/.context/`, suite re-verified 12/12). Also: **PR #127** `feat/agentic-context-orchestration` → `main` open/MERGEABLE (52 files, +5481/−2053); pushed via the SSH URL because an HTTPS push was rejected — *"refusing to allow an OAuth App to create or update workflow `.github/workflows/quality-gate.yml` without `workflow` scope"* (the `gh` OAuth token lacks the `workflow` scope; SSH key auth works, and the fix for future HTTPS pushes is `gh auth refresh -h github.com -s workflow`). CI: `context-budget`, `tsc-baseline`, CodeQL `Analyze` pass; `test` pending.
- 2026-09-18: Added Lesson 123 (Next 16 removed `next lint` — `npm run lint` failed as a bad directory; `eslint .` then hit `@eslint/eslintrc` `FlatCompat` → `TypeError: Converting circular structure to JSON`, and the native flat configs then required per-config-object plugin registration for the repo's rule overrides; added pre-merge follow-up to v3.40.0: `lint` → `eslint .`, `eslint.config.mjs` rewritten to native `eslint-config-next` flat exports + `react`/`react-hooks`/`@typescript-eslint` registered in the override object; fixed 7 real findings — 3 `react-hooks/rules-of-hooks` (`app/admin/utils/ingest-csv/page.tsx` effects hoisted above the admin guard **with `fetchHistory` hoisted too** to avoid a TDZ crash in the loading state, and `app/components/analytics/GoogleAnalytics.tsx` restructured to an unconditional `useEffect` gated internally), 2 `@next/next/no-html-link-for-pages` (`app/markets/screener/advanced/page.tsx` breadcrumb `<a>` → `<Link>`, already imported), 2 `@typescript-eslint/no-this-alias` (test mocks → arrow `run: (params) => this.run(sql, params)`); legacy debt downgraded to `warn` and documented (`@typescript-eslint/no-require-imports` ×88 CJS/jest/server requires, `react/no-unescaped-entities` ×21, eslint-plugin-react-hooks v6 React-Compiler rules `set-state-in-effect` ×7 / `preserve-manual-memoization` ×3 / `purity` ×2 / `refs` ×1 / `set-state-in-render` ×1) → **0 errors / 1139 warnings, exit 0**; verified tsc 46/0 exact, jest 92/92 (1269 pass), quickbuild 185/185 + 0 warnings, doc budget 73.8 KB; no new packages)
- 2026-09-18: Added Lesson 120 (harness tests must never touch the real repo — the CLI-spawn helper hardcoded the repo script path so "harness" cases ran the REAL `check-tsc-baseline.mjs` against the REAL repo, and the `--update` case overwrote the committed `scripts/dev-checks/tsc-baseline.json` with `{total:2,prod:1}`, silently poisoning the hook + CI gate while the suite stayed green; fix = `run(script, args, cwd, tscCmd?)` with the path as an explicit parameter + tmpdir harness mirroring `<root>/scripts/dev-checks/` + `afterAll` byte-identity guards on `tsc-baseline.json`/`.opencode/opencode.json`; recovered via `git checkout`) + Lesson 121 (the real context leak is the **conversation transcript**, not the injected docs — large tool outputs/long summaries are re-sent every request and stay resident after compaction, so v3.39.4's 72 KB slim was not enough; fixes = redirect-to-file + narrow reads, durable memory over re-reads (memory MCP `search_nodes` is a literal contiguous substring match → empty result ≠ no memory), and more `compaction.reserved` headroom; age-chunk only append-only logs, never a live rulebook) + Lesson 122 (verify a capability before designing around it — all four `task` subagent types fail with the free-tier provider error, so orchestration is capability-detected Tier A/B and the limit is recorded as an environment constraint rather than assumed working). v3.40.0 = agentic context + orchestration + harness on `feat/agentic-context-orchestration` (commits `b6552ca`→`6a9135a`): W1 `.agents/rules/tool-output-protocol.md` + `chunk-output.mjs` (+12 tests) · W2 `.agents/rules/durable-memory.md` · W3 `compaction.reserved` 30000 · W4 orchestrator + `/orchestrate` (primary; 16 agents / 11 commands) · W5+W6 orchestrator-health + handoff SCHEMA v1.1 · W7 `check-tsc-baseline.mjs` + committed `tsc-baseline.json` `{46,0}` + doc-budget `--json` + hook §8 + CI `context-budget`/`tsc-baseline` jobs + Turbopack warning gate · Phase 7 tests 23/23; verified `tsc 46` exact baseline, `quickbuild` **0 Turbopack warnings / 185/185**, doc budget **73.0 KB / 100 KB**; no migration, no new packages; merge/PR pending user)
- 2026-09-18: Added Lesson 119 (agent context is a budget — `.opencode/opencode.json` `instructions` injects full files every request; the set had grown to 358 KB ≈ 90K tokens and re-injected instantly after each compaction → compaction loop; fix = thin-index injected files + NEW `.context/slim-docs.mjs` (`AGENTS.md` 191,017 → 20,260 B, `TODO.md` 134,054 → 20,792 B; detail → `.agents/changelog/versions-index.md` + `todo-quick-reference-archive.md`) + NEW `.agents/INDEX.md` manifest + NEW `.agents/session-archive/` (`.context/chunk-history.mjs`: `Primer.md` 1159 → 213 lines, `agent-memory.md` 1092 → 373 lines) + context-budget rule `.agents/rules/session-memory-rules.md` §7 + `scripts/dev-checks/check-doc-sizes.mjs` (100 KB total / 32 KB per-file); `Lessons.md` NOT age-chunked — a rulebook's old entries remain active rules; Turbopack all 33 `/*turbopackIgnore: true*/` sites applied → quickbuild **0 warnings** (was 33), 185/185 pages, tsc **46 = exact baseline**, no migration, no new packages; branch `fix/turbopack-tracing-harness`, commit pending user)
- 2026-09-13: Added Lesson 117 (jest.clearAllMocks() keeps implementations — `mockReturnValue` persists across describe blocks; the v3.38.1 timeframe describe re-isolates via `findManyMock.mockResolvedValue([])` + `getSqliteFallbackMock.mockReturnValue(null)` in its own beforeEach — `ai-monitoring.test.ts` 5/5) + Lesson 118 (a mocked module must export EVERY function callers invoke — missing `enqueueWriteBehind` in the `@/lib/sqlite` mock → unhandled `TypeError` rejection after the dynamic import; fix `enqueueWriteBehind: jest.fn()` at L30); added v3.38.1 entry (Swing generatedAt push-sink NULL guard — Prisma 23502 NOT NULL on `swing_signals.create` every ~6h + 23503 FK cascade destroying victory rows; root cause `upsertSwingAnalysisJob` bound `created_at` ONLY inside the payload → mirrored `generated_at` NULL → push sink passed `generatedAt: undefined`; Fix 1 = top-level `generatedAt: new Date()` in `lib/services/swingRecommendationService.ts` ~L1227, Fix 2 = `lib/sqlitePushSinks.ts` ~L497 fallback `() => (row.generatedAt as string) ?? row.created_at`; + AI-monitoring "Last hour" fix — `getAiCallsMerged` never filtered the in-memory ring (only `getAiStatsMerged` re-filtered after merging); Fix 3 = `lib/services/ai/ai-monitoring.ts` L331-340 `cutoff = Date.now() - timeframeMinutes*60*1000` memory-ring filter + passes `timeframeMinutes` to `getPersistedAiCalls`; tests ai-monitoring +86 (timeframe describe 5/5 incl. enqueueWriteBehind mock), sqlite +30 (23502 regression — `params[3]` created_at binding + `job-null-gen` row), swing +21 (40/40); full **88 suites / 1207 pass / 4 skip / 0 fail**, tsc **46 = exact baseline (0 new)**, no migration, no new packages, diff 6 files +157/−3; live :3000 "Last hour" clean + 0 console errors; on `fix/db-health-ops-count-manual-sync` on top of committed v3.38.0 `d5645a9`; doc commit pending user — do not commit/push/merge without explicit user approval)
- 2026-09-11: Added Lesson 116 (a "steady-state" serve path that only gates on age re-runs the whole pipeline on every plain load — `lib/services/swingRecommendationService.ts` ~L988-1135: done/failed served INDEFINITELY, `force=1` the ONLY regeneration trigger (supersedes pending/running + kicks `checkSwingPerformance()`), status-anchored liveness `staleBefore = now − SWING_JOB_STALE_MS` (`pending→createdAt`, `running→startedAt`, else NaN), newest-TERMINAL fallback for fresh in-flight, Prisma fallback serves done without re-scan when the mirror is empty, durable pending job on plain first load — NO in-memory-only run; swing **57/57** incl. 5 new, tsc 46 = exact baseline, no migration, diff 2 files +266/−12; on `fix/workers-tasks-monitoring` on top of PR #118 merge `10e2a00`; CODE+TESTS+LIVE-VERIFIED 5/5, commit/push pending user; issue #119 carry); added v3.36.0 entry (Swing steady-state serve fix — see `.agents/changelog/versions-v3.36.md`; 2 commits pending user approval: code + docs; merge/deploy PENDING USER)
- 2026-09-11: Added Lesson 115 (v3.35.0 — fire-and-forget Prisma write in tested code = real-DB write in Jest: `setIntelligenceCache` un-awaited `upsert` (`cache.ts` :101-124) raced the `beforeEach` `deleteMany` teardown in `intelligence.test.ts` → stale row → spurious `INTELLIGENCE_CACHE_HIT` flaky fails at :187/:246/:279, the "documented pre-existing flake" since v3.25.0 now FIXED; prisma mock factory + `jest.clearAllMocks()` keeps impls; targeted 13/13, full 86/86 suites 1170 pass / 4 skip / 0 fail — FIRST fully-green full run; tsc 46 = exact baseline; test-only +21 on PR #118 branch on top of v3.34.1 merge, committed `2036724` + pushed; PR #118 merge/deploy pending user)
- 2026-09-11: added v3.34.1 entry (sql.js WASM async-load gate fix — `getSqlJs()` async `fs.readFile`/stream WASM load left "Cannot log after tests are done" noise after a Jest file finished; fix = synchronous `wasmBinary` load (`fs.readFileSync` try/catch per candidate path) + memoized `getSqlJs()`; sqlite + cron-daemon 88/88 under CI=true --runInBand zero noise; full 1154 pass / 4 skip / 1 fail = pre-existing `intelligence.test.ts` flake; tsc 46 baseline; commit `d91fb01` on `feat/db-health-monthly-ops` merged into PR #118 branch).
- 2026-09-11: Added Lesson 114 (resetSqliteStateForTests() nulls the module's captured state IN PLACE — sqlite.test.ts monthly describe test-3 must `await ensureSqliteBackup();` BEFORE `resetOpsMonthlyForTests()` or every later test throws `TypeError: Cannot read properties of null (reading 'persistTimeCorrection')`; fix → sqlite 83/83, targeted 94/94 across 7 suites, tsc 46 = exact baseline); added v3.34.0 entry (Monthly Query Consumption — db-health monthly-ops window, deferred Plan 09 rev-v3 c/d, branch `feat/db-health-monthly-ops` on top of committed v3.30.0 `653b617`: NEW pure `lib/services/opsMonthly.ts` globalThis `__opsMonthly` ledger — monthKey = `getIstDayKey().slice(0,7)`, lazy seed + fresh ledger at rollover, `foldOpsCounterIntoMonthly()` idempotent Math.max high-water merge, `buildQueryConsumption()` no-double-count aggregation (perDay newest-first ≤31, `DB_PLAN_LIMIT_OPS_MONTHLY` default 200_000), module NEVER invoked at module load (suites mock `@/lib/prisma` without named exports); `lib/sqlite.ts` persist/restore under `_backup_meta` `"ops_monthly"` (:1653 — restore :1421, persist :1440, 60s tick :1705) discarding stale previous-month snapshots; db-health GET `queryConsumption` (+12) + Monthly Query Consumption card (+77) + `.env.example` +5; no hot-path change — `lib/prisma.ts` = 6-line comment; sqlite 83/83, targeted 94/94, tsc 46 = exact baseline, no migration, diff 7 files +310/−2 + 2 new; **deferred**: `query_cache`; code + tests complete, MERGED into PR #118 branch per user-approved plan — PR #118 merge/push/deploy pending USER)
- 2026-09-10: Added Lesson 113 (a one-shot leader election with stop-only `onLost` callbacks never recovers — v3.28.2 stopped engines on lost locks but nothing re-acquired, so the prod scheduler stayed dead since 2026-09-08 07:06 UTC until a manual admin "Start Engine"; NEW `watchLeaderRole` self-healing loop: standby → adaptive probe → claim → re-probe OWN row → leader + 300s heartbeat, renewal-fail → internal onLost → standby + fast 60s re-probe; `LEADER_STALENESS_MS` 15→10 min; `getLeaderWatchStatuses()` zero-Prisma diagnostics; steady-state = 1 findUnique/300s); added v3.33.0 entry (Leader Watchdog Self-Heal — `leaderWatch.test.ts` 8 + `instrumentation.test.ts` 7 rewritten + `dbHealthRoute.test.ts` +1 + `cron-daemon.test.ts` +1 engine-restart; constant fixes `leader.test.ts:68`/`sqlite.test.ts:282`; targeted **125/125**, full **1154 pass / 4 skip / 1 fail** = pre-existing `intelligence.test.ts` flake; tsc **46 = exact baseline (0 new)**; on `fix/leader-watchdog-self-heal` on top of committed v3.32.1 `bcde7ae`; spec/plan 11 + all docs committed with commit 1, push/merge/deploy pending) + v3.33.1 entry (Swing performance touch-tracking fix — intraday HIGH/LOW touches count as target/stop hits via NEW `maxHighSincePosting`/`minLowSincePosting` + ONE `$queryRaw` window + `dayHigh`/`dayLow` bridge, BUY touch target-wins; `swingPerformanceService.test.ts` 27/27, full 1154 pass / 4 skip / 1 fail, tsc 46 baseline; own commit by workstream).
- 2026-09-10: Added Lesson 112 (a Web `Request` body is single-use — `restore`/`set_time_correction` in `app/api/admin/db-health/route.ts` re-read `req.json()` on the consumed stream → both 400'd with the wrong error (`"Invalid restore payload"` — broken since v3.21.2; `"Invalid payload"` — masking the zod error since v3.32.0); fix = parse the body ONCE at POST top and reuse `requestBody`; regression `dbHealthRoute.test.ts` 5/5 with real `Request` objects via `jsonPost` enforcing `bodyUsed`; mock `req.json()` as a real-Request spy, never a plain jest.fn); added v3.32.1 entry (db-health POST body-parsed-once hotfix — post-merge of v3.32.0 PR #117 `38a27bf` on `main`, code+tests+live-verified, docs complete, commit pending user).
- 2026-09-10: Added Lesson 111 (parseIstDateTimeLocal round-trip guard comparing parsed-UTC vs IST input silently corrupts epochs on non-IST hosts; explicit IST round-trip via `toIstIso(parsed).slice(0,16) === input` + no `process.env.TZ` in tests).
- 2026-09-09: Added Lesson 110 (`isLeader()` returns a Promise — `if (!isLeader("sqlite-sync"))` is ALWAYS truthy (`!Promise` = false) and silently disabled the single-writer leader gate on the 6h PUSH engine, so every instance pushed to Prisma on the 6h tick — fixed by `await`ing the check; the `sync_history` direction ledger is the observability hook that proves the PUSH engine is actually leader-gated); added v3.31.0 entry (SQLite-first NSE read architecture + low-frequency Prisma sync — sync_history ledger + boot hydration on every instance (`syncFromPrisma` opts reason/skipReconcile/leaderBypass/force) + NSE rate guard `lib/services/nseRateGuard.ts` + `_sync_outbox` 6h PUSH engine `pushSqliteToPrisma`/`lib/sqlitePushSinks.ts` (grouped latest-op-wins, chunk-200 sinks, `reconcileControlToPrisma`) + NSE captures SQLite+outbox only (auto-promote env-gated) + jobs write SQLite-first (recs/swing/perf — 5 mirror tables, 7 write-through helpers) + admin long-lived datasets SQLite-first (+7 helpers, 4 CRUD routes flipped) + db-health wiring (GET outbox/derived-counts/sync-history; POST push_to_prisma; UI Outbox + Derived-counts + Push); sqlite.test.ts **68/68** (11 new P8), instrumentation +1, NEW nseRateGuard 6, full **1105 pass / 4 skip / 1 fail** (1 = documented pre-existing `intelligence.test.ts` flake), tsc 46 = exact baseline, no migration; code committed `9303bd7`→`653b617`, 9 commits on `fix/v3.29.1-header-watchlist` on top of committed v3.30.0; **deferred (plan rev-v3 c/d)**: `query_cache` + db-health monthly-ops window; doc commit pending user)
- 2026-09-07: Added Lesson 109 (schema-init SQL parse failure = silent SQLite Not Ready + full Prisma fallback on every SQLite-first read — a stray `;` inside a `--` comment made sql.js `exec()` throw `near "Prisma": syntax error` on every boot; mirror-freshness must be touched at the high-frequency read site (`touchControlMirror` in `discoverPendingTask`) or an idle mirror silently goes stale; `upsertCronJob` must bind datetimes as `toISOString()` via NEW `toIso(v)`, never locale `String(Date)`, or the 12h `reconcileControlToPrisma` read-back writes corrupt `nextRun`); added v3.30.0 entry (daemon control-plane cadence — `LEADER_STALENESS_MS` 15min/`LEADER_HEARTBEAT_MS` 300s, `SWING_DRAIN_INTERVAL_MS` 900s, `REAP_INTERVAL_MS` 300s — + touch-freshness + schema-init root cause + Netlify WASM staging `scripts/copy-sql-wasm-netlify.mjs` → `.next/sql-wasm.wasm` + Date-bind fix; targeted 96/96, tsc 46 = exact baseline, no migration; on `fix/v3.29.1-header-watchlist` on top of committed v3.29.2 `1e907f1`; Phase 1–4 committed `8af65cc`, Date-bind + docs committed in the v3.30.0 doc commit; **Plan 09 SQLite-first read architecture user-approved + implementation starting (Phase 0)**)
- 2026-09-07: Added Lesson 108 (Playwright MCP a11y snapshots truncate deep/combined option lists — a caret-combobox with optgroup children reported "7 options" until the combobox element itself was re-snapshotted with an element target ref → **all 11 options** across 3 optgroups; always re-snapshot the specific interactive element before concluding options are missing. Model-governance rule: permanent locks (built-in models) MUST be enforced at the API/route layer (400 on DELETE) AND reflected in the UI, never only hidden client-side — `app/api/admin/ai/config/route.ts` DELETE zod-validates `modelId` (NOT `id` — `{id}` → 400 "modelId is required"), builtin → 400 "Cannot remove built-in models" (nothing persisted), unknown → 404, catalog → hidden, custom → removed, ACTIVE-model removal resets `ai_config.model` → `DEFAULT_MODEL` + `resetLLM()`; `BUILTIN_MODEL_IDS` = `openrouter/free` + `openrouter/auto` must stay in sync with `AI_FALLBACK_MODELS` (sync-guard test)); added v3.29.2 entry (Admin AI model management — `BUILTIN_MODELS`/`BUILTIN_MODEL_IDS` in `lib/services/ai/config.ts`, 3-optgroup active-model select in `app/admin/ai/page.tsx` Model Library; 14 new tests — `aiModelCatalog.test.ts` 4 + `adminAiConfigModelManagement.test.ts` 10 — guard-run `modelChain.test.ts` 5 → 19/19; tsc 46 = exact baseline; live-verified builtin DELETE 400 + UI remove/re-add CATALOG persistence + 11 dropdown options across 3 optgroups; on `fix/v3.29.1-header-watchlist` on top of committed v3.29.1 `6e8db23`, commit pending user; ⚠️ unrelated v3.28.x-era working-tree changes must NOT mix into the commit)
- 2026-09-06: Added Lesson 107 (component local `loading` flag cleared only on the authenticated path = dead-code unauthenticated branch — the watchlist logged-out **infinite-skeleton** bug found during user-requested browser + Chrome DevTools visual observation: `app/watchlist/page.tsx` :303 `if (status === "loading" || loading)` dead-coded the sign-in card because the local `loading` only clears inside `fetchWatchlists()` (authenticated-only); fix `status === "loading" || (status === "authenticated" && loading)`, regression test in `app/watchlist/__tests__/page.test.tsx` 3/3 fails pre-fix; logged-in-first e2e structurally misses logged-out branches — test the logged-out state explicitly); added v3.29.1 entry (header overflow fix `app/Header.tsx` CSS-only — 372 DOM overflow checks + 9-width quick-check loop, 0 overflow @1440/@375; watchlist skeleton fix; e2e 87 passed / 2 flaky / 0 failed; jest full 1043 pass / 4 skip / 1 fail, sole = documented pre-existing `intelligence.test.ts` flake; tsc 46 = exact baseline; no migration; on `main` on top of merged v3.29.0 `d7e54cf`, commit pending user). Tooling: Windows cmd `&` is a separator NOT a background operator (`npx tsc … & echo TSC-STARTED` keeps tsc foreground and the 30s tool kill truncated it, log SIZE=0 — run foreground with a full 120s timeout; `start /B cmd /c` verify by polling the output file; Playwright login needs a hydration wait — fill → click → wait for header/avatar before next step)
- 2026-09-05: Added Lesson 106 (NSE "Securities available for trading (Equity segment)" — the `www.nseindia.com/static/market-data/securities-available-for-trading` listing is a JS-guarded human page (curl → 404 HTML shell); the machine CSV lives on the archives host: `https://archives.nseindia.com/content/equities/EQUITY_L.csv` (SYMBOL,NAME OF COMPANY, SERIES, DATE OF LISTING, PAID UP VALUE, MARKET LOT, ISIN NUMBER, FACE VALUE — 2,570 clean rows, series EQ/BE/BZ, no dupes). Generated symbol constants should expose an O(1) case-insensitive `isNseSymbol` (Set/Record lookup) instead of array `.includes()` on hot paths. NSE ISINs are `^IN[0-9A-Z]{10}$`, NOT `^INE…$` — SME scrips start `IN9`. `node:https` has NO default export — `import { get as httpsGet } from "node:https"` (TS1192). Windows cmd: no `wc`; bash-style `for … in` loops error (`u was unexpected`) and multi-line inline `node -e` scripts fail silently — probe with compact single-line `node -e` / `curl -w "%{http_code} %{size_download}"`, or write temp scripts under the pre-approved temp dir); added v3.28.5 entry (NSE scrip-list constant — `scripts/fetch-nse-scrips.ts` + `lib/services/nseScripList.ts` 2,570-scrip constant + `nseScripList.test.ts` 11/11; tsc 46 = exact baseline; commit pending user)
- 2026-09-04: Added Lesson 105 (fail-open leader lock + log-only `onLost` = zombie workers — `onLost` MUST call its engine's stop function (worker → `stopWorkerEngine()`, cron → `stopCronDaemon()`), else a DB blip that makes every instance start a worker poll loop + cron daemon (fail-open `acquireLeaderLock`) leaves the losers polling forever after the leader row is reclaimed; sqlite-sync can stay log-only since sync is per-run leader-gated; test the `onLost` callback via `startLeaderHeartbeat(role, cb)` capture, not just the start path); added v3.28.2 entry (post-ship audit — AI-call tracking/write-behind, Recommendations/Performance/IPO/Swing persistence, cron sync-from-sqlite all verified; lost-leader engine-stop fix in `instrumentation.ts` + NEW `instrumentation.test.ts` 5; suite **1003 pass / 4 skip / 1 fail** = 998 + 5, only fail = documented pre-existing `intelligence.test.ts` flake, excluding it **72 suites / 1003 / 4 / 0**; tsc 46 = exact baseline; 85/85 targeted; no migration; on `fix/v3.28.1-sqlite-self-heal`, commit pending user)
