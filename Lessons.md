...
## Update Log
- 2026-09-04: Added Lesson 104 (query-cache `cacheStrategy` is hard-typed `never` on the base `PrismaClient` read-args and a manual client intersection does not override `never` — inject it at the **call-site boundary** via a pure `withAccelerateCache(strategy)(args)` transform typed `Parameters<T>[0]`/`ReturnType<T>` so contextual argument typing (`orderBy` literals) is preserved and `cacheStrategy` spread through at runtime, keeping the top type `AccelerateClient = PrismaClient` for the 400+ call sites; ALSO any new named export from `lib/prisma.ts` breaks test factories that mock with `{__esModule, default}` only — the named export resolves `undefined`, so add a matching pure stub and loosen exact-args assertions on wrapped reads to `expect.objectContaining`); added v3.27.0 entry (Prisma Postgres Migration Phase 1-3 — `withAccelerate()` in the accelerate branch + `withAccelerateCache` boundary helper + `ACCELERATE_CACHE_TTL` env knob, 5 direct-Prisma read sites wrapped `{ttl,swr}` (corp-actions, chartinkScreenerResult, recommendationTracker ×2, dailyPrice ×2, marketCache), test-coupling fixes (2 mock-factory stubs + `expect.objectContaining`), spec+plan committed `db5a5cc`; tsc 46 = exact baseline 0 new, `prisma validate` valid + generate ok, full suite 995 pass / 4 skip / 2 fail [2 = pre-existing `intelligence.test.ts` async-cache flake, fails identically in isolation, untouched — excluding it 71 suites / 995 pass / 4 skip / 0 fail], no schema change → no migration; 8-file diff + docs, commit pending user)
