...
async function main() {
  ...
  await prisma.user.createMany({
    skipDuplicates: true,
    data: [
      { email: 'alice@example.com', name: 'Alice', password: await hash('password', 10) },
      { email: 'bob@example.com', name: 'Bob', password: await hash('password', 10) },
      { email: 'charlie@example.com', name: 'Charlie', password: await hash('password', 10) },
      { email: 'diana@example.com', name: 'Diana', password: await hash('password', 10) },
      { email: 'edward@example.com', name: 'Edward', password: await hash('password', 10) },
    ],
  });
