...
|---------|---------|--------|
| **v3.21.1** · Sep 02 | **DB Health ops visibility** — SQLite ops-counter persistence + Total Operations/Plan Usage UI + sql.js WASM fix **+ per-type DB-error summary + lazy SQLite re-init**: live-site "SQLite Not Ready" FIX (sql.js WASM — `serverExternalPackages: ['sql.js']` + `resolveSqlWasm()` → `initSqlJs({ locateFile })`); IO-count reconciliation (user-approved "Display + persist" — `getIstDayKey` shared source in `lib/prisma.ts`, `persistOpsCounter()`/`restoreOpsCounter()` key `ops_counter` in `_backup_meta` w/ IST-day guard + `Math.max` merge, 60s timer from `instrumentation.ts`); `/api/admin/db-health` returns `totalOperations`/`planLimit` (`DB_PLAN_LIMIT_OPS` default 10,000)/`planOperationsRemaining` + persists; UI 6th "Total Ops Today" card + "Plan Operations Usage" bar + "Plan Ops n% Used" badge >80%; **NEW `classifyDbError()` (lib/db-utils.ts, `DbErrorType` 6 buckets) + per-type `dbErrorCounts` in recordDbError + `getDbErrorCounts()`; `persistDbErrorCounts()`/`restoreDbErrorCounts()` (key `db_error_counts`, IST-day + Math.max) on the same 60s tick; NEW `ensureSqliteBackup()` lazy on-demand init (`_initPromise` finally-reset — never stuck disabled) + `resetSqliteStateForTests()` in-place test hook; `/api/admin/db-health` GET returns `dbErrorSummary {day, counts}`; UI per-type error chips + IST-day footnote**; sqlite.test mock fixes + 8 new tests → **suite 932/4** | [versions-v3.21.md](./.agents/changelog/versions-v3.21.md) |
...
- docs(readme): add Documentation + Agentic Coding sections and normalize @File.md refs across agent docs
- docs(session): mark ph20 commit + PR update complete [skip ci]
- document Playwright --ui / --headed debug + show-report workflow for error/issue debugging
- edge-safe instrumentation + DB-backed daemon liveness + all-system-job outcome counters + swing/IPO audit tags + friendly AI-failure errors
- enhance application structure and API responses
- enhance dark mode support for stock recommendations page
- enhance database connection handling with remote support and accelerate extension
...
- v3.20.1: DB ops optimization — reduce from ~22K to ~4.2K ops/day
- v3.20.2: DB Health enhancements + Daily Price Cache batch writer
- v3.20.3 follow-up: playwright-debug skill + agent wiring
- v3.20.3: Plan-limit hold resilience — Prisma P6003 recognition + circuit breaker + non-blocking audit/log + worker/cron backoff
- v3.20.4: Fix plan-limit breaker false-positive + add missing intelligence_cache migration
- v3.21.0: Professional Equity Research Decision Engine
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- v3.20.3: Plan-limit hold resilience — Prisma P6003 recognition + circuit breaker + non-blocking audit/log + worker/cron backoff
- v3.20.4: Fix plan-limit breaker false-positive + add missing intelligence_cache migration
- v3.20.5: Prod migration-runner fix — Prisma DIRECT_URL support + intelligence_cache applied to prod
- v3.21.0: Professional Equity Research Decision Engine
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
...
- update agent memory and lessons with 502 fix findings
- update changelog [skip ci]
- update changelog after v3.21.1 commit/push [skip ci]
- update changelog after v3.21.1 increment [skip ci]
- update config
- update environment variables and improve user authentication handling
- update handoff for #69 session-persistence fix [skip ci]
- update homepage to load data
...
- v3.21.1: per-type DB-error classification + DB Health error-summary UI + lazy SQLite re-init
- v3.21.2: stock-quote tiering (cache → SQLite → Prisma, zero after-hours writes) + TTL ms→s fix + SQLite backup/restore in db-health
- v3.21.3: Prisma OpenTelemetry tracing (opt-in) + Prisma Compute P1001 false-alarm diagnosis in db-health
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- v3.21.2: stock-quote tiering (cache → SQLite → Prisma, zero after-hours writes) + TTL ms→s fix + SQLite backup/restore in db-health
- v3.21.3: Prisma OpenTelemetry tracing (opt-in) + Prisma Compute P1001 false-alarm diagnosis in db-health
- v3.22.0: write-behind log store (SQLite-primary, Prisma promotion for important logs only) + leader election + audit-tag gap fill + WASM hotfix
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- Fix: Add explicit cookie configuration for NextAuth session (#39)
- Implement NSE market data ingestion, display, and admin utilities with new database models and APIs.
- Implement SQLite-primary read tier during plan-limit breaker holds
- Implement comprehensive NSE market data display with dedicated pages, components, and API routes for indices and stocks.
- Implement core application structure, authentication, user management, portfolio features, and API routes.
- Implement core application structure, financial charting, API e… (#35)
- Implement core application structure, fix corporate actions database seeding, and stabilize NextAuth authentication. (#42)
...
- fix(netlify): omit .githooks from secrets scan + clean example telegram placeholders
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
- fix(netlify): omit public/sql-wasm.wasm from secrets scan (breaks build)
- fix(recommendations): price-based AI fallback target/SL + backfill script (v3.5.1)
- fix(screener): TradingView change field is % on NSE — fix 57 templates + Short Term Breakouts (v3.5.2)
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
...
- docs(readme): add Documentation + Agentic Coding sections and normalize @File.md refs across agent docs
- docs(session): mark ph20 commit + PR update complete [skip ci]
- docs(v3.24): breaker observability + leader-heartbeat renewal + AI-monitoring two-tier merge (PR #113)
- document Playwright --ui / --headed debug + show-report workflow for error/issue debugging
- edge-safe instrumentation + DB-backed daemon liveness + all-system-job outcome counters + swing/IPO audit tags + friendly AI-failure errors
- enhance application structure and API responses
- enhance dark mode support for stock recommendations page
...
- feat(broadcast,ai,cron): v3.7.1 BUY/SELL-only Telegram broadcast + AI connection-test cron (fallback probing + audit + status) + CI e2e fix + analytics side-nav
- feat(ci,docs): publish Playwright report to GitHub Pages + link in README
- feat(cron): SQLite-primary daemon control plane — high-frequency worker/cron check-reads + task-status writes off Prisma to the local SQLite mirror; Prisma written ONLY at the 12h syncFromPrisma reconcile
- feat(db): add chartink screener models migration + migration ledger doc (v3.5.5)
- feat(fo,api): v3.7.0 F&O analytics UI complete + NSE option-chain-v3 migration + MCP getOptionChain/getFoExpiries (28 fns); carry #68 serverless logs notice
- feat(ipos,events,ai): v3.6.4 IPO issue size + NSE events feed + AI IPO JSON report + MCP/Telegram; v3.6.3 direction-aware levels + recs page redesign; carry v3.6.2 dividend TZ fix
- feat(live-prices): wire SSE into portfolio/watchlist + HistoryTab null-guard (v3.5.1)
...
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
- fix(cron): plan-limit breaker false-OPEN stalls cron daemon + leader heartbeat never renewed + AI-monitoring two-tier merge
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
- fix(netlify): node-cron dispatch in background fn + dynamic service import
...
- cron next-run now evaluated in UTC on every host (was host-local timezone)
- deployfix
- docs(accelerate): accelerate caching never active + standalone Accelerate retires Dec 1 2026
- docs(changelog): split versions-v1.md/v2.md into per-minor files + refactor root CHANGELOG.md to compact pointer index
- docs(changelog): split versions-v3.md into per-minor files (v3.13..v3.0) + index stub
- docs(handoff): mark PR #67 merged into main
- docs(handoff): v3.12.0 committed + pushed — PR #95 open, merge pending [skip ci]
- docs(prisma-postgres): add migration spec + implementation plan (Dec 2026 deadline)
- docs(readme): add Documentation + Agentic Coding sections and normalize @File.md refs across agent docs
- docs(session): mark ph20 commit + PR update complete [skip ci]
- docs(v3.24): breaker observability + leader-heartbeat renewal + AI-monitoring two-tier merge (PR #113)
- document Playwright --ui / --headed debug + show-report workflow for error/issue debugging
...
- feat(live-prices): wire SSE into portfolio/watchlist + HistoryTab null-guard (v3.5.1)
- feat(price-sync): v3.10.0 historical daily_prices sync job — NIFTY50+trackers+screener scope, EQ N-day backfill, chunked upserts (fixes swing indicators '—' on prod)
- feat(prisma-postgres): wire withAccelerate() + cacheStrategy at 5 direct-Prisma read sites (phase 1-3 of Dec 2026 Accelerate retirement)
- feat(recommendations): performance tracking & archival (v3.5.0)
- feat(recommendations): run trigger source + BUY/SELL filter + AI monitoring persistence (v3.5.0 follow-up)
- feat(screener): chartink 117-registry PRIMARY + TV fallback unified runner + DB capture (v3.5.5/v3.5.6)
- feat(sqlite-first): SQLite-primary NSE data store - mirror + read-first + write-through + instant promote
- feat(swing): v3.9.0 Swing Trading Signals tab + NSE candlestick chart buttons
- feat(swing,db,worker): v3.13.0 DB-backed Swing AI analysis job — durable SwingAnalysisJob replaces cache-only fire-and-forget
- feat(v3.14.0): swing signal persistence + performance tracking + spec-driven development workflow
- feat(v3.15.0): closed IPOs with current prices + IPO analysis TTL cleanup + pipeline redesign (HOLDs collapsible)
...
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
- fix(netlify): omit public/sql-wasm.wasm from secrets scan (breaks build)
- fix(prisma-postgres): guard withAccelerateCache() to skip cacheStrategy on non-Accelerate client
- fix(prod-triage): P2002 false-error reporting + plan-limit breaker false-trip + market-data-sync null guard + 6h reconcile cadence
- fix(recommendations): price-based AI fallback target/SL + backfill script (v3.5.1)
- fix(screener): TradingView change field is % on NSE — fix 57 templates + Short Term Breakouts (v3.5.2)
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
...
- feat(recommendations): run trigger source + BUY/SELL filter + AI monitoring persistence (v3.5.0 follow-up)
- feat(screener): chartink 117-registry PRIMARY + TV fallback unified runner + DB capture (v3.5.5/v3.5.6)
- feat(services): NSE scrip-list constant from Securities Available for Trading CSV (v3.28.5)
- feat(sqlite-first): SQLite-primary NSE data store - mirror + read-first + write-through + instant promote
- feat(swing): v3.9.0 Swing Trading Signals tab + NSE candlestick chart buttons
- feat(swing,db,worker): v3.13.0 DB-backed Swing AI analysis job — durable SwingAnalysisJob replaces cache-only fire-and-forget
- feat(symbols): NSE scrip-list constant consumer wiring + plans/specs 07 & 08 (v3.28.5 increment)
- feat(v3.14.0): swing signal persistence + performance tracking + spec-driven development workflow
- feat(v3.15.0): closed IPOs with current prices + IPO analysis TTL cleanup + pipeline redesign (HOLDs collapsible)
- fix MCP function count 28/31 → 29 (includes getInvestmentIntelligence)
- fix build error
...
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
- fix(netlify): node-cron dispatch in background fn + dynamic service import
- fix(netlify): omit .githooks from secrets scan + clean example telegram placeholders
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
...
- fix(prod-triage): P2002 false-error reporting + plan-limit breaker false-trip + market-data-sync null guard + 6h reconcile cadence
- fix(recommendations): price-based AI fallback target/SL + backfill script (v3.5.1)
- fix(recommendations): read-first route cache + edge-cache heavy latest-run reads (v3.28.4)
- fix(screener): TradingView change field is % on NSE — fix 57 templates + Short Term Breakouts (v3.5.2)
- fix(sqlite): partial-init self-heal + promote not-ready guard (v3.28.1)
- fix(sqlite): strip wb-only queued_at before auditLog.createMany (v3.28.3)
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
- fix(ui-ux): backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (v3.29.0)
- fix2
- fix2 (#29)
- fix3
- fix5
...
- mark PR #82 merge + prod backfill complete (327 trackers) in session docs [skip ci]
- mark v3.11.1 commit landed in session-todos [skip ci]
- mark v3.28.2 commit complete in session-todos [skip ci]
- mark v3.28.4 commit complete in session-todos [skip ci]
- mark v3.9.0 commits + PR #90 in session-todos [skip ci]
- never persist fallback all-HOLD runs on AI failure - keep last good run + AI-unavailable notice
- persist DB sessions at login and invalidate at signout (#69)
- polish README - fix Playwright badge org, add Quality Gate/Security/tech badges, refresh test counts, note in-process cron daemon, drop stale blob-store/netlify-functions references
...
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
- fix(ui-ux): backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (v3.29.0)
- fix(watchlist/header): logged-out infinite-skeleton fix + nav overflow fix (v3.29.1)
- fix2
- fix2 (#29)
- fix3
- fix5
...
- add DB plan limit resilience plan + TODO tracking
- add Lesson 40 — production build must include prisma migrate deploy
- add Plan 09 sqlite-first read architecture spec + plan
- add `export const dynamic = "force-dynamic"; // This disables SSG and ISR`
- add alert and analytics services with CRUD operations and market analytics
- add another condition for the env var
- add api route
...
- ci(playwright): add workflow-level permissions (CodeQL Medium fix)
- cleanup
- commit plan visual HTML diagrams (plans 01-09 + closed-ipos/pipeline-top100)
- cron next-run now evaluated in UTC on every host (was host-local timezone)
- deployfix
- docs(accelerate): accelerate caching never active + standalone Accelerate retires Dec 1 2026
- docs(changelog): split versions-v1.md/v2.md into per-minor files + refactor root CHANGELOG.md to compact pointer index
...
- docs(readme): add Documentation + Agentic Coding sections and normalize @File.md refs across agent docs
- docs(session): mark ph20 commit + PR update complete [skip ci]
- docs(sqlite): v3.31.0 Plan 09 SQLite-first read architecture (AGENTS/CHANGELOG/TODO/Primer/memory/Lessons)
- docs(v3.24): breaker observability + leader-heartbeat renewal + AI-monitoring two-tier merge (PR #113)
- document Playwright --ui / --headed debug + show-report workflow for error/issue debugging
- edge-safe instrumentation + DB-backed daemon liveness + all-system-job outcome counters + swing/IPO audit tags + friendly AI-failure errors
- enhance application structure and API responses
...
- enhance worker engine and alert system with auto-start and real-time checks
- ensure ENCRYPTION_KEY is validated at runtime for secrets encryption (#61)
- feat(admin-ai): true built-in models permanently locked - openrouter/free|auto non-removable (v3.29.2)
- feat(ai): v3.8.0 AI pre-flight gate + cron dedup + stale reaping + 8192 maxTokens
- feat(auth,cron,recs): v3.6.0 password-reset flow + market-sync cron + dividend-cards fix; v3.6.1 recs default sorts + performance price bridge + AI context + pen/perf plans
- feat(backtest): historical data cache chain + agentic framework (v3.4.3)
- feat(broadcast,ai,cron): v3.7.1 BUY/SELL-only Telegram broadcast + AI connection-test cron (fallback probing + audit + status) + CI e2e fix + analytics side-nav
...
- feat(ipos,events,ai): v3.6.4 IPO issue size + NSE events feed + AI IPO JSON report + MCP/Telegram; v3.6.3 direction-aware levels + recs page redesign; carry v3.6.2 dividend TZ fix
- feat(live-prices): wire SSE into portfolio/watchlist + HistoryTab null-guard (v3.5.1)
- feat(nse): rate guard single-flight + throttle + burst cooldown (Plan 09 Phase 3)
- feat(price-sync): v3.10.0 historical daily_prices sync job — NIFTY50+trackers+screener scope, EQ N-day backfill, chunked upserts (fixes swing indicators '—' on prod)
- feat(prisma-postgres): wire withAccelerate() + cacheStrategy at 5 direct-Prisma read sites (phase 1-3 of Dec 2026 Accelerate retirement)
- feat(recommendations): performance tracking & archival (v3.5.0)
- feat(recommendations): run trigger source + BUY/SELL filter + AI monitoring persistence (v3.5.0 follow-up)
- feat(screener): chartink 117-registry PRIMARY + TV fallback unified runner + DB capture (v3.5.5/v3.5.6)
- feat(services): NSE scrip-list constant from Securities Available for Trading CSV (v3.28.5)
- feat(sqlite): NSE captures SQLite+outbox only, auto-promote off (Plan 09 Phase 5)
- feat(sqlite): _sync_outbox + 6h SQLite->Prisma push engine (Plan 09 Phase 4)
- feat(sqlite): admin long-lived datasets SQLite-first (Plan 09 Phase 7)
- feat(sqlite): boot hydration opts - leaderBypass + skipReconcile gate (Plan 09 Phase 2)
- feat(sqlite): db-health page wiring + push engine opts + derived counts + hot-route headers (Plan 09 Phase 8)
- feat(sqlite): rec/swing write-through helpers + push sinks (Plan 09 Phase 6 Step 3)
- feat(sqlite): rec/swing/perf mirror schema parity (Plan 09 Phase 6)
- feat(sqlite): recs/swing/perf jobs write SQLite-first (Plan 09 Phase 6 Step 4-5)
- feat(sqlite): sync_history durable ledger + recordSyncHistory prune (Plan 09 Phase 1)
- feat(sqlite-first): SQLite-primary NSE data store - mirror + read-first + write-through + instant promote
- feat(swing): v3.9.0 Swing Trading Signals tab + NSE candlestick chart buttons
- feat(swing,db,worker): v3.13.0 DB-backed Swing AI analysis job — durable SwingAnalysisJob replaces cache-only fire-and-forget
- feat(symbols): NSE scrip-list constant consumer wiring + plans/specs 07 & 08 (v3.28.5 increment)
...
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
- fix(cron): plan-limit breaker false-OPEN stalls cron daemon + leader heartbeat never renewed + AI-monitoring two-tier merge
- fix(daemon/leader): throttled control-plane cadence + touchControlMirror + WASM netlify copy (v3.30.0)
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
...
- mark v3.28.2 commit complete in session-todos [skip ci]
- mark v3.28.4 commit complete in session-todos [skip ci]
- mark v3.29.2 + v3.29.1 commits complete in session-todos [skip ci]
- mark v3.9.0 commits + PR #90 in session-todos [skip ci]
- never persist fallback all-HOLD runs on AI failure - keep last good run + AI-unavailable notice
- persist DB sessions at login and invalidate at signout (#69)
- polish README - fix Playwright badge org, add Quality Gate/Security/tech badges, refresh test counts, note in-process cron daemon, drop stale blob-store/netlify-functions references
...
- feat(swing,db,worker): v3.13.0 DB-backed Swing AI analysis job — durable SwingAnalysisJob replaces cache-only fire-and-forget
- feat(symbols): NSE scrip-list constant consumer wiring + plans/specs 07 & 08 (v3.28.5 increment)
- feat(time): v3.32.0 admin time synchronisation — DB NOW() probe + persisted IST correction + db-health card (Plan 10)
- feat(v3.14.0): swing signal persistence + performance tracking + spec-driven development workflow
- feat(v3.15.0): closed IPOs with current prices + IPO analysis TTL cleanup + pipeline redesign (HOLDs collapsible)
- fix MCP function count 28/31 → 29 (includes getInvestmentIntelligence)
- fix build error
...
- fix(sqlite): partial-init self-heal + promote not-ready guard (v3.28.1)
- fix(sqlite): strip wb-only queued_at before auditLog.createMany (v3.28.3)
- fix(sqlite-push): sequential \$N placeholders + reserved-word quoting in SQLite→Prisma push sinks
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
- fix(ui-ux): backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (v3.29.0)
- fix(watchlist/header): logged-out infinite-skeleton fix + nav overflow fix (v3.29.1)
...
- v3.21.3: Prisma OpenTelemetry tracing (opt-in) + Prisma Compute P1001 false-alarm diagnosis in db-health
- v3.22.0: write-behind log store (SQLite-primary, Prisma promotion for important logs only) + leader election + audit-tag gap fill + WASM hotfix
- v3.32.0 admin time synchronisation (AGENTS/CHANGELOG/TODO/Primer/memory/Lessons/HANDOFF/spec/plan/session)
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- fix posts
- fix type issue
- fix(admin): v3.32.1 db-health POST body-parsed-once (restore + set_time_correction)
- fix(ai): pass saved AI config into rec pipeline; record cron runs on serverless
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
- fix(cron): plan-limit breaker false-OPEN stalls cron daemon + leader heartbeat never renewed + AI-monitoring two-tier merge
...
- feat(cron): SQLite-primary daemon control plane — high-frequency worker/cron check-reads + task-status writes off Prisma to the local SQLite mirror; Prisma written ONLY at the 12h syncFromPrisma reconcile
- feat(db): add chartink screener models migration + migration ledger doc (v3.5.5)
- feat(db-health): v3.34.0 monthly query consumption — db-health monthly-ops window (Plan 09 rev-v3 c/d) — ledger module lib/services/opsMonthly.ts + SQLite _backup_meta persistence + GET queryConsumption block + Monthly Query Consumption card
- feat(fo,api): v3.7.0 F&O analytics UI complete + NSE option-chain-v3 migration + MCP getOptionChain/getFoExpiries (28 fns); carry #68 serverless logs notice
- feat(ipos,events,ai): v3.6.4 IPO issue size + NSE events feed + AI IPO JSON report + MCP/Telegram; v3.6.3 direction-aware levels + recs page redesign; carry v3.6.2 dividend TZ fix
- feat(leader): v3.33.0 watchdog self-heal — watchLeaderRole replaces one-shot boot election (spec 11)
- feat(live-prices): wire SSE into portfolio/watchlist + HistoryTab null-guard (v3.5.1)
- feat(nse): rate guard single-flight + throttle + burst cooldown (Plan 09 Phase 3)
- feat(price-sync): v3.10.0 historical daily_prices sync job — NIFTY50+trackers+screener scope, EQ N-day backfill, chunked upserts (fixes swing indicators '—' on prod)
- feat(prisma-postgres): wire withAccelerate() + cacheStrategy at 5 direct-Prisma read sites (phase 1-3 of Dec 2026 Accelerate retirement)
...
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
- fix(ci): v3.34.1 load sql.js wasm synchronously via wasmBinary — eliminates 'Cannot log after tests are done' gate flake (cron-daemon fireJob)
- fix(cron): plan-limit breaker false-OPEN stalls cron daemon + leader heartbeat never renewed + AI-monitoring two-tier merge
- fix(daemon/leader): throttled control-plane cadence + touchControlMirror + WASM netlify copy (v3.30.0)
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
...
- fix(sqlite): strip wb-only queued_at before auditLog.createMany (v3.28.3)
- fix(sqlite-push): sequential \$N placeholders + reserved-word quoting in SQLite→Prisma push sinks
- fix(swing): v3.33.1 touch-tracking — intraday HIGH/LOW counts as target/stop hit
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
- fix(ui-ux): backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (v3.29.0)
- fix(watchlist/header): logged-out infinite-skeleton fix + nav overflow fix (v3.29.1)
...
- share recommendationsCache on globalThis so worker invalidation reaches API routes (separate module graphs)
- small change
- sync v3.35.0 commit state — 2036724 pushed to PR #118
- test fix
- test(e2e): Playwright cross-browser suite 89 tests + CI workflow + docs (v3.5.3)
- test(intelligence): mock prisma to isolate fire-and-forget IntelligenceCache upsert — fixes pre-existing CI flake
- testfix1
- testfix2
- turn post list into server component
- update /setup page
...
- feat(sqlite): sync_history durable ledger + recordSyncHistory prune (Plan 09 Phase 1)
- feat(sqlite-first): SQLite-primary NSE data store - mirror + read-first + write-through + instant promote
- feat(swing): serve done/failed analysis jobs indefinitely — force=1-only regeneration with performance kick
- feat(swing): v3.9.0 Swing Trading Signals tab + NSE candlestick chart buttons
- feat(swing,db,worker): v3.13.0 DB-backed Swing AI analysis job — durable SwingAnalysisJob replaces cache-only fire-and-forget
- feat(symbols): NSE scrip-list constant consumer wiring + plans/specs 07 & 08 (v3.28.5 increment)
- feat(time): v3.32.0 admin time synchronisation — DB NOW() probe + persisted IST correction + db-health card (Plan 10)
- feat(v3.14.0): swing signal persistence + performance tracking + spec-driven development workflow
- feat(v3.15.0): closed IPOs with current prices + IPO analysis TTL cleanup + pipeline redesign (HOLDs collapsible)
- feat(worker): worker engine monitoring fixes 1-5 — long-task busy heartbeat, worker-logs API route, Scheduler Leadership card, cron-ledger running-guard, daemon leader cross-check (issue #119)
- fix MCP function count 28/31 → 29 (includes getInvestmentIntelligence)
- fix build error
- fix deploy2
- fix deployment issues
...
- check env var value
- chore(agents): extensible skills/agents/commands system (v3.4.3)
- chore(deps): npm audit fixes (v3.38.2) — next 16.3.5 + eslint-config-next/@next/third-parties 16.3.5 + csv-parse 7.0.2 + morgan 1.12.1 + nodemailer 9.1.1 bumps, mysql2 override 3.24.4 (GHSA-rgwj-5xj2-c3m3), @netlify/blobs removal — npm audit 0 critical / 3 high (deepmerge-ts via @prisma/config, no 7.x patch)
- ci(playwright): add workflow-level permissions (CodeQL Medium fix)
- cleanup
- commit plan visual HTML diagrams (plans 01-09 + closed-ipos/pipeline-top100)
- cron next-run now evaluated in UTC on every host (was host-local timezone)
...
- feat(cron): SQLite-primary daemon control plane — high-frequency worker/cron check-reads + task-status writes off Prisma to the local SQLite mirror; Prisma written ONLY at the 12h syncFromPrisma reconcile
- feat(db): add chartink screener models migration + migration ledger doc (v3.5.5)
- feat(db-health): set_ops_counter admin authority-fix + Sync Operations Count UI (v3.38.0) — ledger sync via setOpsMonthlyDay/setOpsMonthlyTotal, audit action ADMIN_DB_SET_OPS_COUNTER, unit + route tests
- feat(db-health): v3.34.0 monthly query consumption — db-health monthly-ops window (Plan 09 rev-v3 c/d) — ledger module lib/services/opsMonthly.ts + SQLite _backup_meta persistence + GET queryConsumption block + Monthly Query Consumption card
- feat(fo,api): v3.7.0 F&O analytics UI complete + NSE option-chain-v3 migration + MCP getOptionChain/getFoExpiries (28 fns); carry #68 serverless logs notice
- feat(ipos,events,ai): v3.6.4 IPO issue size + NSE events feed + AI IPO JSON report + MCP/Telegram; v3.6.3 direction-aware levels + recs page redesign; carry v3.6.2 dividend TZ fix
- feat(leader): v3.33.0 watchdog self-heal — watchLeaderRole replaces one-shot boot election (spec 11)
...
- fix(daemon/leader): throttled control-plane cadence + touchControlMirror + WASM netlify copy (v3.30.0)
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(db-health): swing generatedAt push-sink NULL guard (Prisma 23502/23503) + AI-monitoring timeframe filter (v3.38.1)
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
...
- re-add enforce dynamic
- refine /setup page
- refresh v3.38.1 status (PR #121 OPEN) [skip ci]
- remooving admin seed
- remove excessive Prisma logging on each import
- remove export dynamic
- remove prisma-postgres required-extensions to stop auto migrate deploy
...
- update USAGE.md
- update agent memory and lessons with 502 fix findings
- update changelog (v3.38.2 Dependabot fixes, PR #121 OPEN) [skip ci]
- update changelog [skip ci]
- update changelog after v3.21.1 commit/push [skip ci]
- update changelog after v3.21.1 increment [skip ci]
- update config
...
- fix posts
- fix type issue
- fix(admin): db-health defensive response parsing — readable error when the route returns a non-JSON body
- fix(admin): v3.32.1 db-health POST body-parsed-once (restore + set_time_correction)
- fix(ai): pass saved AI config into rec pipeline; record cron runs on serverless
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
...
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(db-health): swing generatedAt push-sink NULL guard (Prisma 23502/23503) + AI-monitoring timeframe filter (v3.38.1)
- fix(deps): re-add @netlify/blobs ^11.0.3 (Netlify edge-bundling import, v3.38.2 removal 07f42fe broke the deploy build)
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
...
- re-add @netlify/blobs (^10.7.7) so Netlify edge-fn bundler resolves it - unblocks deploy (build failed: Could not resolve @netlify/blobs in csp-nonce/ua_blocker edge functions)
- re-add enforce dynamic
- record Netlify deploy fix — @netlify/blobs re-added ^11.0.3 (PR #123 merged, deploy GREEN, live-verified 0 console errors) [skip ci]
- refine /setup page
- refresh v3.38.1 status (PR #121 OPEN) [skip ci]
- remooving admin seed
- remove excessive Prisma logging on each import
...
- fix type issue
- fix(admin): db-health defensive response parsing — readable error when the route returns a non-JSON body
- fix(admin): deploy_prep undefined error — 503 breaker fast-fail + extractErrorMessage detail support (v3.39.0)
- fix(admin): v3.32.1 db-health POST body-parsed-once (restore + set_time_correction)
- fix(ai): pass saved AI config into rec pipeline; record cron runs on serverless
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
...
- Simplify auth config for production
- Simplify middleware - remove problematic imports, use Node.js runtime
- Sqlite durable mirror spec/plan/changelog (Spec 12) [skip ci]
- Telegram /recommendations using wrong model (#63)
- Telegram bot integration + admin panel + notifications (v3.4.0)
- Temp: disable middleware to test 502
- Trigger deploy with env fix
...
- change latest posts logic
- changeing nvm to 20
- changelog — set_ops_counter queryConsumption response fix (f470e6d) [skip ci]
- chartfix
- check env var value
- chore(agents): extensible skills/agents/commands system (v3.4.3)
- chore(deps): npm audit fixes (v3.38.2) — next 16.3.5 + eslint-config-next/@next/third-parties 16.3.5 + csv-parse 7.0.2 + morgan 1.12.1 + nodemailer 9.1.1 bumps, mysql2 override 3.24.4 (GHSA-rgwj-5xj2-c3m3), @netlify/blobs removal — npm audit 0 critical / 3 high (deepmerge-ts via @prisma/config, no 7.x patch)
...
- enhance worker engine and alert system with auto-start and real-time checks
- ensure ENCRYPTION_KEY is validated at runtime for secrets encryption (#61)
- failsafe hold batch — analytics SQLITE-only + screener + closed-ipo, recommendations preserved in SQLITE
- feat(admin-ai): true built-in models permanently locked - openrouter/free|auto non-removable (v3.29.2)
- feat(ai): v3.8.0 AI pre-flight gate + cron dedup + stale reaping + 8192 maxTokens
- feat(auth,cron,recs): v3.6.0 password-reset flow + market-sync cron + dividend-cards fix; v3.6.1 recs default sorts + performance price bridge + AI context + pen/perf plans
- feat(backtest): historical data cache chain + agentic framework (v3.4.3)
...
- feat(sqlite): boot hydration opts - leaderBypass + skipReconcile gate (Plan 09 Phase 2)
- feat(sqlite): db-health page wiring + push engine opts + derived counts + hot-route headers (Plan 09 Phase 8)
- feat(sqlite): durable mirror snapshot — disk + Netlify Blobs restart survival (Spec 12)
- feat(sqlite): rec/swing write-through helpers + push sinks (Plan 09 Phase 6 Step 3)
- feat(sqlite): rec/swing/perf mirror schema parity (Plan 09 Phase 6)
- feat(sqlite): recs/swing/perf jobs write SQLite-first (Plan 09 Phase 6 Step 4-5)
- feat(sqlite): sync_history durable ledger + recordSyncHistory prune (Plan 09 Phase 1)
...
- feat(symbols): NSE scrip-list constant consumer wiring + plans/specs 07 & 08 (v3.28.5 increment)
- feat(time): v3.32.0 admin time synchronisation — DB NOW() probe + persisted IST correction + db-health card (Plan 10)
- feat(ui): WCAG a11y contrast fixes + flat-change neutral state + index charts open TradingView + mobile menu aria-label
- feat(v3.14.0): swing signal persistence + performance tracking + spec-driven development workflow
- feat(v3.15.0): closed IPOs with current prices + IPO analysis TTL cleanup + pipeline redesign (HOLDs collapsible)
- feat(worker): worker engine monitoring fixes 1-5 — long-task busy heartbeat, worker-logs API route, Scheduler Leadership card, cron-ledger running-guard, daemon leader cross-check (issue #119)
- fix MCP function count 28/31 → 29 (includes getInvestmentIntelligence)
...
- fix(admin): db-health defensive response parsing — readable error when the route returns a non-JSON body
- fix(admin): deploy_prep undefined error — 503 breaker fast-fail + extractErrorMessage detail support (v3.39.0)
- fix(admin): set_ops_counter response includes queryConsumption
- fix(admin): v3.32.1 db-health POST body-parsed-once (restore + set_time_correction)
- fix(ai): pass saved AI config into rec pipeline; record cron runs on serverless
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
...
- fix(recommendations): read-first route cache + edge-cache heavy latest-run reads (v3.28.4)
- fix(screener): TradingView change field is % on NSE — fix 57 templates + Short Term Breakouts (v3.5.2)
- fix(sqlite): normalize worker task/status date binds via toIsoVal
- fix(sqlite): partial-init self-heal + promote not-ready guard (v3.28.1)
- fix(sqlite): strip wb-only queued_at before auditLog.createMany (v3.28.3)
- fix(sqlite-push): sequential \$N placeholders + reserved-word quoting in SQLite→Prisma push sinks
- fix(swing): v3.33.1 touch-tracking — intraday HIGH/LOW counts as target/stop hit
...
- mark v3.9.0 commits + PR #90 in session-todos [skip ci]
- never persist fallback all-HOLD runs on AI failure - keep last good run + AI-unavailable notice
- note user preference for headed Chrome when running the full e2e suite
- persist DB sessions at login and invalidate at signout (#69)
- polish README - fix Playwright badge org, add Quality Gate/Security/tech badges, refresh test counts, note in-process cron daemon, drop stale blob-store/netlify-functions references
- polish setup page
- prod reliability — txn timeout, top-50 cap, telegram live prices, history prices, AI monitoring persistence, DB logs tab
...
- v3.22.0: write-behind log store (SQLite-primary, Prisma promotion for important logs only) + leader election + audit-tag gap fill + WASM hotfix
- v3.32.0 admin time synchronisation (AGENTS/CHANGELOG/TODO/Primer/memory/Lessons/HANDOFF/spec/plan/session)
- v3.39.0/v3.39.1 changelog updates (durable mirror + UI batch) [skip ci]
- v3.39.2 changelog updates (failsafe hold batch) [skip ci]
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- ensure ENCRYPTION_KEY is validated at runtime for secrets encryption (#61)
- failsafe hold batch — analytics SQLITE-only + screener + closed-ipo, recommendations preserved in SQLITE
- feat(admin): warm endpoint for corporate tabs + NSE session settle delay
- feat(admin-ai): true built-in models permanently locked - openrouter/free|auto non-removable (v3.29.2)
- feat(ai): v3.8.0 AI pre-flight gate + cron dedup + stale reaping + 8192 maxTokens
- feat(auth,cron,recs): v3.6.0 password-reset flow + market-sync cron + dividend-cards fix; v3.6.1 recs default sorts + performance price bridge + AI context + pen/perf plans
- feat(backtest): historical data cache chain + agentic framework (v3.4.3)
...
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
- fix(netlify): emails function CJS vs ESM — add netlify/functions/package.json commonjs
- fix(netlify): node-cron dispatch in background fn + dynamic service import
- fix(netlify): omit .githooks from secrets scan + clean example telegram placeholders
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
- fix(netlify): omit public/sql-wasm.wasm from secrets scan (breaks build)
...
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
- fix(netlify): omit public/sql-wasm.wasm from secrets scan (breaks build)
- fix(netlify): update build command to create commonjs package.json for emails function
- fix(prisma-postgres): guard withAccelerateCache() to skip cacheStrategy on non-Accelerate client
- fix(prod-triage): P2002 false-error reporting + plan-limit breaker false-trip + market-data-sync null guard + 6h reconcile cadence
- fix(recommendations): price-based AI fallback target/SL + backfill script (v3.5.1)
- fix(recommendations): read-first route cache + edge-cache heavy latest-run reads (v3.28.4)
...
- chore(agents): extensible skills/agents/commands system (v3.4.3)
- chore(deps): npm audit fixes (v3.38.2) — next 16.3.5 + eslint-config-next/@next/third-parties 16.3.5 + csv-parse 7.0.2 + morgan 1.12.1 + nodemailer 9.1.1 bumps, mysql2 override 3.24.4 (GHSA-rgwj-5xj2-c3m3), @netlify/blobs removal — npm audit 0 critical / 3 high (deepmerge-ts via @prisma/config, no 7.x patch)
- chore(netlify): commonjs guard for .netlify package.json + v3.39.3 docs (quickbuild gate)
- ci(playwright): add workflow-level permissions (CodeQL Medium fix)
- cleanup
- commit plan visual HTML diagrams (plans 01-09 + closed-ipos/pipeline-top100)
- cron next-run now evaluated in UTC on every host (was host-local timezone)
...
- check env var value
- chore(agents): extensible skills/agents/commands system (v3.4.3)
- chore(context): W3 compaction headroom - reserved 10000 -> 30000 (user-approved)
- chore(deps): npm audit fixes (v3.38.2) — next 16.3.5 + eslint-config-next/@next/third-parties 16.3.5 + csv-parse 7.0.2 + morgan 1.12.1 + nodemailer 9.1.1 bumps, mysql2 override 3.24.4 (GHSA-rgwj-5xj2-c3m3), @netlify/blobs removal — npm audit 0 critical / 3 high (deepmerge-ts via @prisma/config, no 7.x patch)
- chore(netlify): commonjs guard for .netlify package.json + v3.39.3 docs (quickbuild gate)
- ci(playwright): add workflow-level permissions (CodeQL Medium fix)
- cleanup
...
- deployfix
- docs(accelerate): accelerate caching never active + standalone Accelerate retires Dec 1 2026
- docs(agents): agent-memory bullet for the Turbopack warning-gate fix (PR #127 CI run 2)
- docs(agents): v3.40.0 phase 8 docs + phase 9 verification (changelog, lessons 120-122, primer, memory, session finalisation)
- docs(changelog): split versions-v1.md/v2.md into per-minor files + refactor root CHANGELOG.md to compact pointer index
- docs(changelog): split versions-v3.md into per-minor files (v3.13..v3.0) + index stub
- docs(context): slim injected instruction files (358KB->72KB) + modularize history + budget guard
- docs(handoff): mark PR #67 merged into main
- docs(handoff): v3.12.0 committed + pushed — PR #95 open, merge pending [skip ci]
- docs(prisma-postgres): add migration spec + implementation plan (Dec 2026 deadline)
- docs(readme): add Documentation + Agentic Coding sections and normalize @File.md refs across agent docs
...
- docs(sqlite): v3.31.0 Plan 09 SQLite-first read architecture (AGENTS/CHANGELOG/TODO/Primer/memory/Lessons)
- docs(v3.24): breaker observability + leader-heartbeat renewal + AI-monitoring two-tier merge (PR #113)
- docs(v3.40.0): spec + plan + decisions/flow + handoff for agentic context/orchestration/harness epic
- document Playwright --ui / --headed debug + show-report workflow for error/issue debugging
- edge-safe instrumentation + DB-backed daemon liveness + all-system-job outcome counters + swing/IPO audit tags + friendly AI-failure errors
- enhance application structure and API responses
- enhance dark mode support for stock recommendations page
...
- feat(admin): warm endpoint for corporate tabs + NSE session settle delay
- feat(admin-ai): true built-in models permanently locked - openrouter/free|auto non-removable (v3.29.2)
- feat(agents): W4 orchestrator agent + /orchestrate (Tier A/B, health-aware)
- feat(agents): W5+W6 subagent health monitoring + handoff schema v1.1
- feat(ai): v3.8.0 AI pre-flight gate + cron dedup + stale reaping + 8192 maxTokens
- feat(auth,cron,recs): v3.6.0 password-reset flow + market-sync cron + dividend-cards fix; v3.6.1 recs default sorts + performance price bridge + AI context + pen/perf plans
- feat(backtest): historical data cache chain + agentic framework (v3.4.3)
- feat(broadcast,ai,cron): v3.7.1 BUY/SELL-only Telegram broadcast + AI connection-test cron (fallback probing + audit + status) + CI e2e fix + analytics side-nav
- feat(ci,docs): publish Playwright report to GitHub Pages + link in README
- feat(context): W1 tool-output protocol + chunk-output.mjs + 12 tests
- feat(context): W2 durable memory - two-tier protocol (.remember + memory MCP)
- feat(cron): SQLite-primary daemon control plane — high-frequency worker/cron check-reads + task-status writes off Prisma to the local SQLite mirror; Prisma written ONLY at the 12h syncFromPrisma reconcile
- feat(db): add chartink screener models migration + migration ledger doc (v3.5.5)
- feat(db-health): set_ops_counter admin authority-fix + Sync Operations Count UI (v3.38.0) — ledger sync via setOpsMonthlyDay/setOpsMonthlyTotal, audit action ADMIN_DB_SET_OPS_COUNTER, unit + route tests
- feat(db-health): v3.34.0 monthly query consumption — db-health monthly-ops window (Plan 09 rev-v3 c/d) — ledger module lib/services/opsMonthly.ts + SQLite _backup_meta persistence + GET queryConsumption block + Monthly Query Consumption card
- feat(dev-checks): W7 harness - tsc baseline + context budget gates
- feat(fo,api): v3.7.0 F&O analytics UI complete + NSE option-chain-v3 migration + MCP getOptionChain/getFoExpiries (28 fns); carry #68 serverless logs notice
- feat(ipos,events,ai): v3.6.4 IPO issue size + NSE events feed + AI IPO JSON report + MCP/Telegram; v3.6.3 direction-aware levels + recs page redesign; carry v3.6.2 dividend TZ fix
- feat(leader): v3.33.0 watchdog self-heal — watchLeaderRole replaces one-shot boot election (spec 11)
- feat(live-prices): wire SSE into portfolio/watchlist + HistoryTab null-guard (v3.5.1)
...
- fix(auth,logs,security): join-approve login fix, logs/ dir + blob mirror, env-only DEFAULT_PASSWORD, llms.txt + robots discovery (v3.5.7)
- fix(backtest): prod backtest_history gap — lazy CREATE TABLE IF NOT EXISTS self-healing
- fix(build): silence 33 Turbopack dynamic-filesystem-access tracing warnings
- fix(ci): Turbopack warning gate could never pass (grep|wc exits 1 on zero matches)
- fix(ci): v3.34.1 load sql.js wasm synchronously via wasmBinary — eliminates 'Cannot log after tests are done' gate flake (cron-daemon fireJob)
- fix(codeql): backslash-first table-cell escaping in chunk-output.mjs (js/incomplete-sanitization)
- fix(context): W2 - document memory-MCP literal search semantics (empty result != no memory)
- fix(cron): plan-limit breaker false-OPEN stalls cron daemon + leader heartbeat never renewed + AI-monitoring two-tier merge
- fix(daemon/leader): throttled control-plane cadence + touchControlMirror + WASM netlify copy (v3.30.0)
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(db-health): swing generatedAt push-sink NULL guard (Prisma 23502/23503) + AI-monitoring timeframe filter (v3.38.1)
...
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
- fix(lint): restore eslint gate on Next 16 — native flat config + 7 real findings
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
- fix(netlify): emails function CJS vs ESM — add netlify/functions/package.json commonjs
- fix(netlify): node-cron dispatch in background fn + dynamic service import
- fix(netlify): omit .githooks from secrets scan + clean example telegram placeholders
...
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
- fix(swing,perf,worker): v3.12.0 request-time split for swing AI analysis + prod-stability batch
- fix(test): commit the .remember/ ignore rule + Lesson 124 (CI-only failure)
- fix(ui-ux): backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (v3.29.0)
- fix(watchlist/header): logged-out infinite-skeleton fix + nav overflow fix (v3.29.1)
- fix2
- fix2 (#29)
...
- sync v3.35.0 commit state — 2036724 pushed to PR #118
- test fix
- test(dev-checks): v3.40.0 Phase 7 harness tests (tsc baseline + context budget)
- test(e2e): Playwright cross-browser suite 89 tests + CI workflow + docs (v3.5.3)
- test(intelligence): mock prisma to isolate fire-and-forget IntelligenceCache upsert — fixes pre-existing CI flake
- testfix1
- testfix2
...
- fix(sqlite): normalize worker task/status date binds via toIsoVal
- fix(sqlite): partial-init self-heal + promote not-ready guard (v3.28.1)
- fix(sqlite): production recovery serving — deferred Blobs restore + Netlify detection + health telemetry (v3.40.1)
- fix(sqlite): strip wb-only queued_at before auditLog.createMany (v3.28.3)
- fix(sqlite-push): sequential \$N placeholders + reserved-word quoting in SQLite→Prisma push sinks
- fix(swing): v3.33.1 touch-tracking — intraday HIGH/LOW counts as target/stop hit
- fix(swing): v3.9.1 analysisStatus honesty fix — done only when a stock carries AI targets
...
- v3.39.0/v3.39.1 changelog updates (durable mirror + UI batch) [skip ci]
- v3.39.2 changelog updates (failsafe hold batch) [skip ci]
- v3.40.1 changelog, lessons 128, primer, handoff, session todos [skip ci]
- v3.5.1 carry-forward session + handoff update [skip ci]
- v3.5.2 screener change-percent fix changelog + session update [skip ci]
- v3.8.0 session decisions [skip ci]
- v3.9.0 changelog + session docs [skip ci]
...
- fix(lint): restore eslint gate on Next 16 — native flat config + 7 real findings
- fix(logger,monitoring): v3.13.0 Netlify logs-dir fallback — prod Server Logs tab was always empty
- fix(mirror): BUGS 15/16/17 - shared mirror mapper, alerts/workers/dividend fallbacks, IST day key (v3.40.2)
- fix(netlify): emails function CJS vs ESM — add netlify/functions/package.json commonjs
- fix(netlify): node-cron dispatch in background fn + dynamic service import
- fix(netlify): omit .githooks from secrets scan + clean example telegram placeholders
- fix(netlify): omit CI workflow from secrets scan — unblocks prod deploys
...
- fix(daemon/worker): upsertCronJob Date-bind toIso + v3.30.0 docs (v3.30.0)
- fix(db-health): swing generatedAt push-sink NULL guard (Prisma 23502/23503) + AI-monitoring timeframe filter (v3.38.1)
- fix(deploy): predeploy mirror-preservation guard - build-time snapshot/backup/drain before every deploy (v3.40.3)
- fix(deps): re-add @netlify/blobs ^11.0.3 (Netlify edge-bundling import, v3.38.2 removal 07f42fe broke the deploy build)
- fix(dividends): DividendMonthView timezone keying — noon-UTC ex-dates no longer land a day late in IST
- fix(hygiene): mask credential literals in Lessons + obfuscate hook block-lists
- fix(instrumentation): stop worker/cron engines on leadership loss - single-active-worker enforcement (v3.28.2)
