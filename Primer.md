...
## Last Updated
2026-09-04 (v3.27.0 — Prisma Postgres Migration Phase 1-3: `withAccelerate()` wiring + `cacheStrategy` at 5 direct-Prisma read sites. On `v3.26.0-prod-failure-triage` on top of v3.26.0 work. `withAccelerateCache` boundary helper resolves the base-type `cacheStrategy: never` blocker. tsc 46 = baseline, suite 995 pass / 4 skip (2 fails = documented pre-existing intelligence.test.ts flake). Docs complete; **commit pending user**.)
