...
## Last Updated
2026-08-15 (v3.11.3 Full serverless purge — Netlify treated as a persistent server: `CRON_DAEMON_DISABLED` opt-out REMOVED from `instrumentation.ts` + `cron-daemon.ts` (⚠️ BREAKING vs v3.11.0 doc — daemon must run on Netlify now); `lib/netlify-logger.ts` DELETED + `@netlify/blobs` dropped (41 packages removed); `lib/logger.ts` + `worker-logger.ts` stripped all Blob/serverless branches; monitoring route/page dropped `serverless:` fields + amber ephemeral-logs banner; ~25-file comment sweep; `DataFetcher.test.tsx` describe.skip REWRITTEN for the current `apiUrl`+`render` API — 9/9 pass; **suite 709 pass / 4 skip** (was 700/11); `npx tsc --noEmit` 46 errors (down from 71 baseline, 0 new). Commit pending (no push/deploy) — branch `fix/cron-tz-swing-perf`. Prior: v3.11.2 cache-singleton (suite 700) + v3.11.1 no-fake-HOLD (suite 696) + v3.11.0 daemon committed `6c4ef41` unpushed.)
