...
## Last Updated
2026-08-15 (v3.11.1 No-fake-HOLD Today's Picks — AI-failure runs no longer overwrite the last good run: `runDailyRecommendations` partitions on `success` — ONLY `success:true` verdicts persisted (entries/trackers/predictions); zero successes → run `failed` with NO entries (`uniqueStocks:0` + `aiUnavailable:true` metadata + `run_failed` event + `SCREENER_RUN_FAILED` audit + cache invalidate) so Today's Picks falls back to the previous good run + **no Telegram broadcast**; partial failure → failed/capped entries `deleteMany` (`symbol notIn analyzed`); NEW lightweight `latestRun` row (second `findFirst`, `select id/runDate/status`) exposed by `/api/recommendations`; `DailyPicksTab` amber banner "⚠️ AI analysis unavailable on <date> — showing picks from <date>" (`data-testid="ai-unavailable-notice"`); genuine all-HOLD (`success:true`) days unchanged; suite **696 pass** (was 694; +2 total/partial-failure tests), tsc 71 exact baseline, Playwright-verified on :3000 (API `latestRun`, banner via intercepted payload, normal state clean, 0 console errors); **commit pending user; no push/deploy** — branch `fix/cron-tz-swing-perf`. Prior: v3.11.0 node-cron daemon + `daysTracked` fix + v3.10.1 batch committed `6c4ef41` (unpushed; hook passed, tsc clean, suite 694 pass/11 skip).)
