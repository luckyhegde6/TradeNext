...
**Tests**: NEW `app/watchlist/__tests__/page.test.tsx` **3/3** (unauthenticated → sign-in prompt + no skeleton [regression: fails pre-fix]; loading → skeleton; authenticated-empty → CTA; mocks useSession/fetch/AiActionButton/Autocomplete/useLivePrices).
**Verification**: tsc **46 = exact baseline (0 new)**; no schema change → no migration; full jest **1043 pass / 4 skip / 1 fail** (1 = documented pre-existing `intelligence.test.ts` flake).
...
**Docs**: AGENTS.md v3.29.1 row, CHANGELOG index + `.agents/changelog/versions-v3.29.md` v3.29.1 section, TODO.md row, Primer (this), agent-memory, Lessons #107, session-todos, latest.md handoff, HANDOFF.md, `.agents/sessions/2026-09-06-v3.29.1-header-watchlist/`. **Commit pending user (no push/merge without approval).**

### v3.29.0 — UI/UX audit fixes: backtest symbol-gate softening + AI-failure error surfacing + mobile-nav Alerts (Sep 05 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMIT PENDING USER
**Fixes** (branch `fix/v3.28.1-sqlite-self-heal`, on top of v3.28.5 `6700076`): **(1) Backtest gate softened** — `app/api/backtest/run/route.ts` no longer 404s on unknown symbols; v3.28.5's `isBacktestSymbolAllowed` **REMOVED** (dead code — `lib/services/symbolReference.ts` now exports only `mergeSymbolSuggestions`); the route always falls through to `getBacktestData(symbolUpper)` and derives `symbolSource = "known"|"unlisted"` (:83) from `prisma.symbol.findUnique` purely for labeling (echoed :178) — the ONLY no-data failure is the pre-existing `barCount < 50 → 400` guard. Live-verified: unlisted `RBLBANK` → **200**, `symbolSource:"unlisted"`, 70 bars. `symbolReference.test.ts` **11 → 7** (4 gate tests removed with the helper). **(2) AI-failure surfacing** — NEW `lib/aiErrorMessage.ts` (`extractErrorMessage(error, fallback)` unwraps `Error.message` and `{error:{message}}` shapes); `AiActionButton` gains `error?: string | null` → red status line (:114-116, hidden while loading); `app/watchlist/page.tsx` throw-site (:254) normalizes `err.error || HTTP <status>` and passes `error={aiError}` — fixes BOTH the swallowed `/api/ai/query` 500s AND the `[object Object]` render (bonus regression; live-verified with a simulated 500). **(3) Mobile nav** — logged-in quick-access `grid-cols-2` (:312) now includes **F&O Analytics** (:334) + **Alerts** (:337) (desktop `/alerts` :141, `/fo` :147; verified 375×812).
**Tests**: NEW `lib/__tests__/backtestSymbolFallthrough.test.ts` **4** (node-env; unlisted+enough bars → 200 `unlisted`; <50 bars → 400; listed → 200 `known`; unauth → 401) + `lib/__tests__/watchlistAiError.test.ts` **8** + `app/components/__tests__/AiActionButton.test.tsx` **4** (path deviation from convention, plain `test(`).
**Verification**: tsc **46 = exact baseline (0 new)**; targeted **23/23**; full suite **1043 pass / 4 skip / 1 fail** (1 = documented pre-existing `intelligence.test.ts` flake — excluding it 72 suites / 1043 / 4 / 0; net delta +12 vs the 1031 pre-v3.29.0 baseline); no schema change → no migration.
**Live verified** (Playwright :3000, admin): `RBLBANK` backtest → 200 unlisted 70 bars; simulated AI 500 → red line extracted message + button enabled; mobile grid complete; `[object Object]` regression fixed; test watchlist deleted.
**Docs**: AGENTS.md v3.29.0 row, CHANGELOG index + `.agents/changelog/versions-v3.29.md` (NEW), TODO.md row, Primer, agent-memory, session-todos, HANDOFF, `.agents/sessions/2026-09-05-fix-v3.28.1-sqlite-self-heal/` (decisions + flow). **Commit pending user (no push/merge without approval; branch push carries v3.28.5 `6700076` too — do not amend).**

### v3.28.5 — NSE scrip-list constant (Securities available for trading — Equity) (Sep 05 2026) — ✅ CODE + TESTS VERIFIED, DOCS DONE, COMMIT PENDING USER
**User directive**: extract NSE's "Securities available for trading" Equity CSV into "a constant which gets loaded for the Symbol references".
**Data source (resolves previous URL blocker)**: the `www.nseindia.com/static/market-data/securities-available-for-trading` listing is the JS-guarded human page (curl → 404 HTML shell). The machine endpoint is **`https://archives.nseindia.com/content/equities/EQUITY_L.csv`** — HTTP 200, 181,507 B, header `SYMBOL,NAME OF COMPANY, SERIES, DATE OF LISTING, PAID UP VALUE, MARKET LOT, ISIN NUMBER, FACE VALUE`; **2,570 clean rows** (8 fields each, no embedded commas/quotes/dupes), series **EQ 2,288 / BE 254 / BZ 28**.
**Implemented**: NEW `scripts/fetch-nse-scrips.ts` generator (dry-run default, `--write` gate, BOM strip, ≥2000-row sanity guard; npm `fetch:scrips` script) → commits `lib/services/nseScripList.ts` (464 KB, `NseScrip` + `NSE_SCRIPS` + derived `NSE_SYMBOL_SET`/`NSE_SCRIP_BY_SYMBOL` + `isNseSymbol`/`getNseScrip`/`searchNseSymbols`; sorted; O(1) case-insensitive symbol check).
**Tests**: NEW `nseScripList.test.ts` **11/11** (dataset sanity incl. ISIN `^IN[0-9A-Z]{10}$` — SME scrips start `IN9`; fixtures RELIANCE/TCS/INFY/20MICRONS; helpers incl. prefix-priority search).
**Verification**: tsc **46 = exact baseline (0 new)**; targeted 11/11; full suite unchanged baseline; no migration.
**Docs**: AGENTS.md v3.28.5 row + NSE Integration Notes bullet, CHANGELOG index + `.agents/changelog/versions-v3.28.md`, TODO.md row, Primer, agent-memory, Lessons #106. **Commit pending user** (on `fix/v3.28.1-sqlite-self-heal`, on top of v3.28.4). **Consumer wiring DONE (v3.28.5 increment)**: backtest listed-fall-through + symbols/search constant-first merge + IPO `listed` flag/badge — new tests 11/11 + 5/5, tsc 46 baseline (0 new).

### v3.28.4 — Read-first recommendations route + edge-cache the heavy latest-run reads (Sep 05 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMIT PENDING USER
**Why (two compounding issues, user-approved "Both" fix)**: **(1) key collision** — `app/api/recommendations/route.ts` wrote its serialized `responseBody` under the **service's** `LATEST_KEY` (`"recommendations:latest"`), clobbering the `LatestCacheEntry {runId, newestRunId, data}` → the cross-instance fingerprint check read `cached.runId === undefined` → the heavy stocks-include query (95 rows) re-ran on **EVERY request** (db-health read-tier showed `recommendations.prisma` 14/14 huge-query misses). **(2) heavy reads unedge-cached** despite v3.27.0's `withAccelerateCache`.
**Fix (route + service)** — route gains `ROUTE_CACHE_KEY = "recommendations:api:latest"` + `ROUTE_CACHE_TTL_SECONDS = 60` + typed `RouteRecommendationsCacheBody`, with a **read-first memory fast path** after the breaker block (`recordRead("recommendations.memory", {hit:true})`, `servedFrom: "memory_cache"`, zero Prisma); all three legacy `"recommendations:latest"` refs (breaker fallback, `set`, DB-error fallback) switched to the route key so the service key is never touched by the route. `getLatestRecommendations`' two heavy `findFirst` reads (`latestRun` stocks-include + `newestRun` lightweight select) wrapped in `withAccelerateCache({ttl:60, swr:30})`; **fingerprint probes stay uncached** (cross-instance staleness guard). `findFirst`+`include` payload falls back to the bare model through the wrapper → re-used the existing `as RunWithStocks | null` cast at the `serializedStocks` usage (matches line-1308 pattern).
**Tests** (+1 regression in `lib/__tests__/dailyRecommendationService.test.ts`) — factory gains the pure `withAccelerateCache` stub (spread preserves keys — existing `where/select` assertions still pass) + NEW regression asserting fingerprint calls lack `cacheStrategy` while heavy calls[2]/[3] carry `{ttl:60, swr:30}` → **34/34**.
**Verification**: tsc **46 = exact baseline (0 new)**; targeted recs 34/34 + readTier/recPerf 25/25; full suite **1004 pass / 4 skip / 2 fail** (2 = documented pre-existing `intelligence.test.ts` async cache-flake — excluding it 72 suites / 1004 / 4 / 0); no schema change → no migration.
**Docs**: AGENTS.md v3.28.4 row, CHANGELOG index + `.agents/changelog/versions-v3.28.md` (also removed the orphaned duplicate `# v3.28.2` header at EOF), TODO.md row, Primer, agent-memory, session-todos, HANDOFF. **Commit pending user (no push/merge without approval)**.
...
### v3.28.2 — Lost-leader engine stop (single-active-worker enforcement) (Sep 04 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMIT PENDING USER
**On top of**: v3.28.1 (`718b5d2`), branch `fix/v3.28.1-sqlite-self-heal`.
**Post-ship audit** (user request — "tasks/worker failing, only 1 worker/task active at a time"): verified all persistence paths — AI-call tracking persists (`trackAiCall` → memory ring + `enqueueWriteBehind("server_log")`, two-tier `getPersistedAiCalls` merge); Recommendations (`tracker createMany` + `stock createMany` + run.update, no-fake-HOLD deleteMany intact); performance tracking (status updates + history creates + 360d archive); IPO details (DB `market_cache` ipo_analysis + memory); Swing trackers (`persistSwingTrackers` + `patchSwingSignalAnalysis` on `analysisStatus==="done"`); crons synced during normal sync (`syncFromPrisma` pulls `cron_job`, `reconcileControlToPrisma` pushes nextRun/heartbeats/task statuses at the 6h sync). ✅ all verified.
**Defect found**: multiple workers executing different tasks concurrently. `acquireLeaderLock` fails OPEN on DB-unavailable (every instance starts worker poll + cron daemon during a blip); on DB recovery the losers' `renewLeaderLock` returns false → `onLost` fires → `instrumentation.ts` only logged → **kept polling forever** → N active workers/tasks.
**Fix (surgical, `instrumentation.ts` only)**: worker onLost → `stopWorkerEngine()` (poll timeout + heartbeat + scheduler intervals); cron onLost → `stopCronDaemon()` (node-cron tasks + intervals); sqlite-sync onLost stays log-only (gated per-run by `isLeader`).
**Tests**: NEW `lib/__tests__/instrumentation.test.ts` (5 — starts when leader for all 3 roles; worker onLost → stopWorkerEngine; cron onLost → stopCronDaemon; not-leader → no start; non-node runtime → early return).
**Verification**: tsc **46 = exact baseline (0 new)**; targeted 85/85 (sqlite 36 + daemon-sqlite-first + dbOpTiering + historical + leader); full suite **1003 pass / 4 skip / 1 fail** (1003 = 998 + 5; 1 = documented pre-existing `intelligence.test.ts` flake — excluding it 72 suites / 1003 / 4 / 0). No schema change → no migration.
**Docs**: AGENTS.md v3.28.2 row, CHANGELOG index + `.agents/changelog/versions-v3.28.md`, this Primer, agent-memory, Lessons #105, session-todos, HANDOFF. **Commit pending user approval (do not push/merge without explicit approval).**
...
**Why**: Per user directives — "all NSE sync triggers against SQLite; all things cached, reads hit SQLite FIRST → live fetch, NOT Prisma fallback for NSE reads; add SQLite schema similar to Prisma + keep updated + indexes." Make SQLite the primary live store for NSE-backed market data (stock list, corporate actions, daily OHLCV/backtest prices, Chartink captures) with a hot memory cache + an instant SQLite→Prisma promote engine (both ~60s background timer AND end-of-task flush).
**Implement**: (1) **`lib/sqlite.ts` SCHEMA** — NEW `symbols`, `daily_price`, `chartink_screener_result` tables + indexes (daily_price OHLCV mirror keyed `NSE:${SYMBOL}` to match Prisma `daily_prices.ticker`); extended `corporate_action` (+8 cols) + `chartink_screener` (+5 cols); idempotent `ensureNseColumns(db)` ALTER-guard; SQLite-Fallback interface +7 methods + type-safe `createFallback` impls via local `sv()` converter. (2) **Write mirrors** — `executeStockSync`→`cacheSymbol`+`flushNseToPrisma()`; `executeCorpActionsSync`→`cacheCorporateActions`+`flushNseToPrisma()` (`(actions as any[])` cast + `parseActionPurpose`); `executeHistoricalPriceSync` (dry-run-false)→`cacheDailyPriceBars(\`NSE:${symbol}\`, …)` mapping normalized `SecurityWiseHistoricalBar` fields (`b.date`/`b.open`/…) + `flushNseToPrisma()`; `runChartinkUnifiedScreeners`→`cacheChartinkResults(templateId, rows)` per run. (3) **Read-first** — `getBacktestData` NEW SQLite-first step (`getSqlitePriceRange(\`NSE:${sym}\`)`, `source:"sqlite"` when ≥20 bars, memory-cached, falls through to DB/NSE); `BacktestDataResult.source` union +`"sqlite"`. (4) **Promote engine** — `promoteNseToPrisma()` (leader-gated, reads SQLite, chunked `createMany`), `flushNseToPrisma()`, `startNsePromoteFlush()` (60s unref'd) booted from `instrumentation.ts`. (5) **Dev-gated Prisma query logger** (`PRISMA_QUERY_LOG=1`, dev-only, constructor `log:["query"]` — `$extends` wrapper doesn't propagate `$on('query')`). (6) **Tests** — `historicalPriceSyncService.test.ts` mock `@/lib/sqlite` fixed (`cacheDailyPriceBars`+`flushNseToPrisma`); backtest/chartink/worker suites unaffected.
**Tests**: Full suite **995 pass / 4 skip / 2 fail** — the 2 fails = documented pre-existing `intelligence.test.ts` async cache-flake (confirmed fails identically in isolation; excluding it: 71 suites / **995 pass / 4 skip / 0 fail** from these changes). Targeted: `backtestDataService.test.ts` 16/16, chartink suites 38/38, `historicalPriceSyncService.test.ts` 15/15. tsc **46 = exact baseline** (0 new); no schema change → no migration.
**Docs**: AGENTS.md v3.28.0 row, CHANGELOG index + `.agents/changelog/versions-v3.28.md`, this Primer, agent-memory. Spec `.agents/specs/06-sqlite-first-nse-store.md` + plan `.agents/plans/06-sqlite-first-nse-store.md`.
**Status**: Code + tests + docs verified; **diff pending user commit + push** (on branch, no auto-commit/push/deploy).

### v3.27.0 — Prisma Postgres Migration Phase 1-3: `withAccelerate()` wiring + `cacheStrategy` at 5 query sites (Sep 04 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMIT PENDING USER
**Branch**: `v3.26.0-prod-failure-triage` (on top of v3.26.0 work). Deployable unit.
**Why**: Standalone Prisma Accelerate (`prisma+postgres://accelerate.prisma-data.net`) **retires Dec 1, 2026** (~3 months out). Long-term fix = move to Prisma Postgres (edge caching built-in; Phase 0 = manual Console provisioning at deploy-time, no code). Phase 1-2 ship the intermediate win: activate the already-installed `@prisma/extension-accelerate` `withAccelerate()` on the runtime client + explicit **`cacheStrategy`** on the 5 highest-frequency direct-Prisma **reads**.
**Fix**: (1) **Phase 1 (`lib/prisma.ts`)** — `new PrismaClient({ accelerateUrl }).$extends(withAccelerate())` in the accelerate branch (extension order documented: `withAccelerate()` applied FIRST so the `$allOperations` circuit-breaker/op-count/timeout extension wraps the cached reads); `type AccelerateClient = PrismaClient` top type (reverted from a manual intersection — the base `PrismaClient` model read-args hard-code `cacheStrategy: never`, and intersections don't override `never`); `let prismaClient: any` + explicit `$allOperations` param types preserved the 46-error tsc baseline. NEW **`withAccelerateCache(strategy)(args)`** boundary helper exported from `lib/prisma.ts` — preserves Prisma contextual typing via `Parameters<T>[0]` (so `orderBy: 'asc'|'desc'` literals still resolve) while injecting `cacheStrategy` at the query boundary (runtime client supports it; base type doesn't — pure transform, no client-type restructuring). NEW `ACCELERATE_CACHE_TTL = Number(process.env.PRISMA_ACCELERATE_CACHE_TTL) || 300` export. (2) **Phase 2 — 5 sites**: `corporateAction.findMany` `{ttl:300,swr:60}` (corp-actions/combined route); `chartinkScreenerResult.findMany` `{ttl:900,swr:300}` (`chartinkScreenerService`, the actual direct Prisma read); `recommendationTracker.findMany` ×2 `{ttl:600,swr:60}` (`recommendationPerformanceService`, both sort paths); `dailyPrice.findFirst` ×2 `{ttl:60,swr:30}` (`lib/stock-service.ts`; the `aggregate` left unwrapped); `marketCache.findUnique` `{ttl:300,swr:60}`. (3) **Test coupling fixes** — `recommendationPerformanceService.test` + `chartinkScreenerService.test` mock `@/lib/prisma` with factory objects returning only `{__esModule, default}`, so the new named export was `undefined` → added the pure `withAccelerateCache` stub `(s)=>(a)=>({...a,cacheStrategy:s})` to both factories; chartink exact-args assertion → `expect.objectContaining({where, orderBy})`.
**Tests**: Full suite **995 pass / 4 skip / 2 fail** — the 2 fails = documented pre-existing `intelligence.test.ts` async cache-flake (confirmed fails identically in isolation; `intelligence.ts`/`cache.ts`/`audit.ts` untouched; excluding it: 71 suites / **995 pass / 4 skip / 0 fail** from these changes). tsc **46 = exact baseline** (0 new); `prisma validate` valid + `prisma generate` ok; `npm run lint` fails with the documented pre-existing `next lint` CLI directory quirk (unrelated); no schema change → no migration.
**Docs**: AGENTS.md v3.27.0 row, CHANGELOG index + `.agents/changelog/versions-v3.27.md`, TODO.md row, this Primer, agent-memory, Lessons. Spec `.agents/specs/05-prisma-postgres-migration.md` + plan `.agents/plans/05-prisma-postgres-migration.md` (committed `db5a5cc`).
**Status**: Code + tests + docs verified; **diff pending user commit** (8 code files + doc files, on branch; no auto-commit/push/deploy).

### v3.26.0 — Prod-failure triage: P2002 false-errors + Plan-limit breaker false-trip + "a is not iterable" Market Sync + 12h→6h sync cadence (Sep 03 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMIT PENDING USER
**Branch**: `main` (production incident triage, on top of v3.25.0). Deployable unit.
**Why**: Three prod failure signatures surfaced from the DB Errors panel, deploy logs, and admin Tasks panel. (1) The DB Errors panel showed 3× `WorkerStatus create` P2002 — benign leader-election races being recorded as DB faults. (2) The plan-limit circuit breaker opened on ANY `isDbUnavailableError`, so a transient Accelerate-proxy `fetch failed`/connection blip on a HEALTHY DB froze the whole app for the 5-min cooldown with zero recovery (the v3.24.0 false-positive recurring — the biggest prod issue; logs 17:00:34 `fetch failed` → 17:04:36 "Plan limit circuit breaker open" → 17:28/17:29 repeat → "Swing analysis processor crashed" + "Cron daemon resync deferred"). (3) The scheduled `Daily Market Sync (System)` cron failed 2/9 + 3/9 at 01:01 with minified `a is not iterable`. Plus user directive to drop the 12h sync cadence to 6h.
**Fix**: (1) **P2002** (`lib/prisma.ts`) — NEW `isBenignUniqueConflict(err)` (code `P2002`); the `$allOperations` `.catch` SKIPS `recordDbError` for them (error still propagates unchanged — only the diagnostic recording is skipped). (2) **Breaker false-trip** (`lib/prisma.ts`) — trip now ONLY on `isPlanLimitHoldError` (P6003 / "hold on your account" / "planLimitReached" / query timeout); transient comms errors keep driving per-query graceful degradation (worker backoff + cached/empty fallbacks) without freezing the global breaker. Removed the now-unused `isDbUnavailableError` import. (3) **"a is not iterable"** (`lib/services/worker/worker-service.ts`) — `executeMarketDataSync` (`market_data`) now guards `for (const stock of getIndexStocks(indexName))` with `if (!Array.isArray(stocks) || stocks.length === 0) throw new Error("No stocks fetched from NSE (market data sync)")` — `getIndexStocks` returns `null` on NSE error (lines 783/809) → `TypeError: null is not iterable` (minified "a is not iterable"). (4) **12h→6h cadence (user directive)** (`lib/sqlite.ts`) — `PROBE_INTERVAL_MS` 12h→6h; the periodic probe now runs a full `syncFromPrisma()` (→`reconcileControlToPrisma`) on EVERY available tick (was down→up only), so the SQLite→Prisma control-plane reconcile reaches Prisma every 6h even on a healthy DB. Stale "12h" comments updated to 6h in sqlite.ts + swingRecommendationService + corp-actions/combined + recommendations routes (`IPO_ANALYSIS_CACHE_TTL_SECONDS` 12h cache TTL left unchanged).
...
### v3.24.0 — Plan-limit breaker false-OPEN observability + leader-heartbeat renewal + AI-monitoring two-tier merge (Sep 03 2026) — ✅ CODE + TESTS + DOCS VERIFIED, PR #113 PENDING USER
**Branch**: `fix/cron-breaker-leader` (commit `299472a`, PR #113). Deployable unit.
**Why**: (1) The in-process cron daemon IS running and fires on schedule, but every DB op it makes is rejected by an **OPEN plan-limit breaker on a healthy DB** — `fireJob`'s `prisma.cronJob.findUnique` throws `Plan limit circuit breaker open` → nothing spawns, `nextRun` never advances. (2) Leader heartbeats were never renewed — `renewLeaderLock` had ZERO call sites, so every `worker_status` `leader-<role>` row went stale after 5-min and a standby claimed it → split leadership → duplicate crons/sQLite-sync/worker on multi-instance bursts. (3) AI-monitoring "0 of 0 calls shown" — v3.22.0 write-behind promotes only important rows, so info-level SUCCESS AI calls never reached Prisma.
**Fix**: (1) **Breaker-trip observability** (`lib/prisma.ts`) — throttled `logger.warn` at the open site carrying model/operation/error/`classifyDbError` type (`BREAKER_TRIP_LOG_THROTTLE_MS=60s`). (2) **Leader heartbeat renewal** (`lib/services/leader.ts` + `instrumentation.ts`) — NEW `startLeaderHeartbeat(role, onLost?)` renews every `LEADER_HEARTBEAT_MS=60s` (unref'd `setInterval`, returns `stop()`), wired for `worker`/`cron-daemon`/`sqlite-sync`. (3) **AI-monitoring two-tier merge** — NEW `getWriteBehindLogsBySource` (`lib/sqlite.ts`) + two-tier `getPersistedAiCalls` (Prisma + SQLite, newest-first, sliced) in `ai-monitoring.ts` + NEW `ai-monitoring.test.ts` (3).
**Tests**: Full suite **989 pass / 4 skip / 0 fail** (was 986, +3); tsc 46 = baseline; no schema change → no migration.
**Docs**: AGENTS.md v3.24.0 row, CHANGELOG index + `.agents/changelog/versions-v3.24.md`, TODO.md row, Primer, agent-memory, Lessons.
**Status**: **PR #113 pending user merge/deploy.** Related: Netlify secrets-scan build fix PR #112 (branch `fix/netlify-secrets-scan-wasm`, commit `27af087`).
...
### Session 23 (September 5, 2026) — Read-first recommendations route + edge-cache heavy latest-run reads (v3.28.4, on branch `fix/v3.28.1-sqlite-self-heal`, on top of v3.28.3 `a1dd094`)
- **User-approved "Both" fix** for two compounding issues spotted via db-health read-tier telemetry: (1) **key collision** — the route wrote `responseBody` under the service's `LATEST_KEY` → clobbered `LatestCacheEntry {runId, newestRunId, data}` → `cached.runId === undefined` → cross-instance fingerprint check re-ran the 95-row stocks-include query EVERY request (`recommendations.prisma` 14/14 huge-query misses); (2) heavy reads had no edge cache despite v3.27.0 `withAccelerateCache`.
- **Fix**: route gains `ROUTE_CACHE_KEY = "recommendations:api:latest"` + `ROUTE_CACHE_TTL_SECONDS = 60` + typed `RouteRecommendationsCacheBody`; read-first memory fast path after the breaker block (`recordRead("recommendations.memory", {hit:true})`, `servedFrom: "memory_cache"`, zero Prisma); all three legacy `"recommendations:latest"` refs switched to the route key (service key never touched by route); `getLatestRecommendations`' two heavy `findFirst` reads wrapped in `withAccelerateCache({ttl:60, swr:30})`; fingerprint probes stay **uncached** (cross-instance staleness guard); `as RunWithStocks | null` cast re-used at the `serializedStocks` usage (line-1308 pattern).
- **Tests**: factory gains the pure `withAccelerateCache` stub (spread preserves keys) + NEW regression (fingerprint calls lack `cacheStrategy`, heavy calls[2]/[3] carry `{ttl:60, swr:30}`) → `dailyRecommendationService.test.ts` **34/34**.
- **Verification**: tsc **46 = exact baseline (0 new)**; targeted recs 34/34 + readTier/recPerf 25/25; full suite **1004 pass / 4 skip / 2 fail** (2 = documented pre-existing `intelligence.test.ts` flake — excluding it 72 suites / 1004 / 4 / 0). No schema change → no migration.
- **Docs**: AGENTS.md v3.28.4 row, CHANGELOG index + `.agents/changelog/versions-v3.28.md` (also removed orphaned duplicate `# v3.28.2` header at EOF), TODO.md row, Primer, agent-memory, session-todos, HANDOFF. **Commit pending user (no push/merge without approval)** — v3.28.3 `a1dd094` already committed+pushed; do not amend.

### Session 22 (September 4, 2026) — Lost-leader engine stop — single-active-worker enforcement (v3.28.2, on branch `fix/v3.28.1-sqlite-self-heal`, on top of v3.28.1 `718b5d2`)
- **Post-ship audit**: user reported "tasks/worker failing, only 1 worker/task active at a time". Verified every persistence path — AI-call tracking (`trackAiCall` → memory ring + SQLite `enqueueWriteBehind("server_log")`, two-tier `getPersistedAiCalls` merge), daily Recommendations (run + trackers + stocks + run.update persistence, no-fake-HOLD deleteMany intact), performance tracking (status updates + `RecommendationStatusHistory` + 360d archive), IPO details (DB `market_cache` ipo_analysis + memory + cleanup), Swing trackers (`persistSwingTrackers` + `patchSwingSignalAnalysis` on `analysisStatus==="done"`), cron sync (`syncFromPrisma` pulls `cron_job`; `reconcileControlToPrisma` at 6h sync pushes nextRun/lastRun/counters + heartbeats + task statuses). ✅ all persist.
- **Defect root cause**: `instrumentation.ts` `onLost` callbacks only `logger.warn`; combined with `acquireLeaderLock` fail-open on DB-unavailable (every instance starts worker poll loop + cron daemon during a blip), the losers' heartbeats failed after DB recovery → `onLost` fired → they **kept polling forever** → N active workers/tasks.
- **Fix (`instrumentation.ts` only)**: worker onLost → `stopWorkerEngine()`; cron onLost → `stopCronDaemon()`; sqlite-sync onLost stays log-only (per-run `isLeader` gated). Both stop helpers already existed and are synchronous.
- **Tests**: NEW `lib/__tests__/instrumentation.test.ts` (5 — leader-elected start for all 3 roles; worker/cron onLost → stop fired via `startLeaderHeartbeat(role, cb)` callback capture; not-leader no-start; non-node early return).
- **Verification**: tsc **46 = exact baseline (0 new)**; targeted 85/85 (sqlite 36 + daemon-sqlite-first + dbOpTiering + historical + leader); full suite **1003 pass / 4 skip / 1 fail** (1003 = 998 + 5; 1 = documented pre-existing `intelligence.test.ts` flake — excluding it 72 suites / 1003 / 4 / 0). No schema change → no migration.
- **Docs**: AGENTS.md v3.28.2 row, CHANGELOG index + `.agents/changelog/versions-v3.28.md`, Primer, agent-memory, Lessons #105, session-todos, HANDOFF. **Commit pending user (no push/merge without approval).**
