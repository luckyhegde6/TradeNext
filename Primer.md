...
## Last Updated
2026-08-15 (v3.11.0 In-process node-cron cron daemon — replaces Netlify scheduled functions: NEW `lib/services/worker/cron-daemon.ts` + root `instrumentation.ts` auto-start (guarded `NEXT_RUNTIME==="nodejs"`, not build phase, `CRON_DAEMON_DISABLED=1` opt-out for serverless); `startCronDaemon()` idempotent (ensureRecommendationCrons self-heal → syncCronJobs → 60s resync + heartbeat), `syncCronJobs()` register/drop/re-register (expression-change, invalid skip, deactivated drop, per-job timezone default Asia/Kolkata), `fireJob` re-fetches row → shared `spawnDueCronJob` (90-min dedup, indexName defaults, nextRun advance, triggeredBy system), heartbeat `workerStatus` upsert `cron-daemon-<host>-<pid>`; admin: zod enum gap FIX in `app/api/admin/cron/route.ts`, NEW `GET /api/admin/cron/daemon` liveness endpoint, Cron tab TASK_TYPES +3 + daemon status chip, workers engine route drives the daemon; **Netlify cron deleted** (5 functions + `netlify/functions/` + `[functions]` block); ledger outcome wiring (`recordCronRun(jobName, success, { skipSpawnCounted })` + NEW `recordSystemRunOutcome` in `worker-service.ts` executeTask — no double-count; manual runs stay on `recordManualRunLedger`); `daysTracked` sort 500 FIX (→ orderBy.createdAt) + regression test; **carried committed-unreleased v3.10.1 batch (`b35eca4`)**: honest latest-run, shared `modelChain.ts` fallback chain, swing tracker persistence (`@@unique([symbol, createdAt])`), tenure pills, PerformanceTab dark-theme fix; suite **686 pass** (was 673+11 skip; +13 = 12 cron-daemon tests + 1 skipSpawnCounted), tsc 71 exact baseline; docs updated (AGENTS.md, CHANGELOG, versions-v3, TODO, Lessons #72/#73, Primer, agent-memory); **commits pending user; no push/deploy** — branch `fix/cron-tz-swing-perf`, Netlify serverless must keep `CRON_DAEMON_DISABLED=1`)

---

## Current Project Status

### v3.11.0 — In-Process node-cron Cron Daemon (replaces Netlify scheduled functions) + `daysTracked` 500 fix + carried v3.10.1 batch (Aug 15 2026) — ✅ CODE + TESTS + DOCS VERIFIED, COMMITS PENDING USER, NO DEPLOY
