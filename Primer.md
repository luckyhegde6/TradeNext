...
### v3.13.0 — DB-backed Swing AI analysis job (Aug 16 2026) — ✅ CODE + TESTS + LIVE-VERIFIED, COMMIT PENDING USER, NO DEPLOY
**Branch**: `feat/swing-db-analysis-job` (on top of main; v3.12.0 merged via PR #95).
**Why**: the v3.12.0 async split fixed Netlify's 30s wall but left analysis state VOLATILE — the pending payload lived in the in-memory `staticCache` (LRU-evictable by the tab's 10s poll churn), only ONE in-flight job per process, and a Netlify instance restart mid-run lost everything.
**Fix (Option A, user-approved)**: NEW Prisma `SwingAnalysisJob` (migration `20260816000000_add_swing_analysis_job` applied locally via `migrate diff --from-config-datasource` + `db execute`, NOT `migrate dev` — local DB has no `_prisma_migrations` ledger; prod uses `migrate deploy`). `getSwingRecommendations({analyze:true})` pre-scans the DB — done/failed/pending/running served WITHOUT re-scanning (pending kicks `maybeProcessSwingAnalysis()`); absent → scan + create durable job + frozen pending feed; `force=1` supersedes pending/running jobs (`updateMany → failed "Superseded by a newer force refresh"`), re-scans, new job; empty feed → synchronous skipped. Processor claims via atomic `updateMany({where:{id,status:"pending"},data:{running,startedAt,attemptCount:{increment}}})` (multi-instance safe), re-reads before final write + aborts unless still `running` (supersede race), stale recovery 45 min / 2 attempts, audits START/COMPLETE/FAILED + RUN_COMPLETE, persists swing trackers when done (non-fatal), warms cache. **Cache holds ONLY final done/failed** — pending/running always served from the DB row. `cron-daemon.ts` 60s resync tick drains the queue (dynamic import, no circular dep). REMOVED: `SWING_PENDING_TTL`/`swingAnalysisInFlight`/`runSwingAnalysisInBackground` (grep 0 refs).
...
**Live-verified :3000**: `force=1` → **11.11s** pending feed (200 raw/20 top/34 templates) → job `68bbed30…` claimed (running, attempt 1) → 4 background batches (24.9s each, `poolside/laguna-xs-2.1:free`) → **done 20/20**; non-force 39ms frozen pending from DB during processing / 25ms cached done after; audit RUN_START 15:15:45 → ANALYSIS_START 15:15:47 → ANALYSIS_COMPLETE + RUN_COMPLETE 15:17:33; 5 new swing trackers (idempotent); Swing tab "AI targets ready" + 20/20 direction-aware targets (MARKSANS LONG 72% ₹333→₹380/₹310…), 0 console errors.
- **Status**: docs updated (AGENTS.md v3.13.0 row, CHANGELOG index + versions-v3.md entry, TODO.md row, Lessons #81, Primer, agent-memory, session-todos, session decisions/flow `2026-08-16-swing-db-job`); **commit pending user; no push/deploy**. On merge: Netlify rebuild → `migrate deploy` applies the table. Post-deploy: remove stale Netlify cron UI entries if present.
