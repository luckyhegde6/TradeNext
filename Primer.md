...
## Last Updated
2026-09-03 (v3.25.0 — SQLite-Primary daemon control plane: high-frequency worker/cron check-reads + task-status writes move off Prisma to the local SQLite mirror; Prisma written ONLY at the 12h `syncFromPrisma` reconcile — suite 994 pass / 4 skip (2 fails = pre-existing `intelligence.test.ts` flake); tsc 46 = baseline. Branch `fix/cron-breaker-leader` on top of v3.24.0. Commit/push pending user; `.env` must be restored to Accelerate URL before finish.)
