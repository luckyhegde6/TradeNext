...
## Last Updated
2026-08-14 (v3.10.0 Historical-price sync into `daily_prices` — Swing indicators "—" fix: NEW `historicalPriceSyncService` (scope = explicit OR NIFTY 50 ∪ 30d `RecommendationTracker` ∪ live `ChartinkScreenerResult`, capped 300; N-day EQ window via `fetchSecurityWiseHistoricalData`; multi-row `$executeRawUnsafe` ON CONFLICT upserts chunked 200; 200ms NSE delay; `maxDurationMs` cap; per-symbol tolerance; dry-run default) wired into `worker-service.ts` (`historical_price_sync` case + `executeHistoricalPriceSync`) + `run-cron-background.ts` (new `historical-price-sync` action + step 4 of market-sync `dryRun:false` 6-min budget) + NEW `scripts/backfill-daily-prices.ts`; suite **660 pass** (was 653), tsc 71 exact baseline; local dry-run verified (TCS 5d → 4 EQ bars, 0 written); **local `--apply` EXECUTED (user-approved): 266 symbols, 17,198 bars, 0 errors, 658s — DB 17,411 rows / 286 symbols**; PLUS `backtest_history` prod-gap **FIX** (user override — no migration ever created it; NEW `ensureBacktestHistoryTable` lazy CREATE TABLE IF NOT EXISTS + 3 indexes in `backtestDataService.ts`, memoized, retried, chain degrades to daily_prices/NSE; +7 tests); committed `b312de7` (feat) + `4d49e13` (docs) on `feat/historical-price-sync`, **PR #91 open**, pushed; no deploy; v3.9.1 committed `9247a9f` + docs `2eaeef8` on main)

---

## Current Project Status

### v3.10.0 — Historical-Price Sync into `daily_prices` (Swing Indicators "—" Fix) + `backtest_history` Prod-Gap FIX (Aug 14 2026) — ✅ CODE + TESTS VERIFIED, COMMITTED `b312de7`+`4d49e13`, PR #91 OPEN, NO DEPLOY
...
- **Status**: docs updated (AGENTS.md v3.10.0 row, CHANGELOG index + versions-v3, TODO, BUGS.md #11→fix built, Primer, agent-memory, Lessons, session-todos); committed `b312de7` (feat) + `4d49e13` (docs, `[skip ci]`) on `feat/historical-price-sync`; pushed; **PR #91 open**; **NO deploy** (user-managed). Prod `--apply` NOT run — not needed (market-sync step 4 auto-backfills after deploy); prod DB write still requires explicit permission.
