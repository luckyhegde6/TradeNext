...
## Last Updated
2026-08-14 (v3.10.0 Historical-price sync into `daily_prices` — Swing indicators "—" fix: NEW `historicalPriceSyncService` (scope = explicit OR NIFTY 50 ∪ 30d `RecommendationTracker` ∪ live `ChartinkScreenerResult`, capped 300; N-day EQ window via `fetchSecurityWiseHistoricalData`; multi-row `$executeRawUnsafe` ON CONFLICT upserts chunked 200; 200ms NSE delay; `maxDurationMs` cap; per-symbol tolerance; dry-run default) wired into `worker-service.ts` (`historical_price_sync` case + `executeHistoricalPriceSync`) + `run-cron-background.ts` (new `historical-price-sync` action + step 4 of market-sync `dryRun:false` 6-min budget) + NEW `scripts/backfill-daily-prices.ts`; suite **653 pass** (was 638), tsc 71 exact baseline; local dry-run verified (TCS 5d → 4 EQ bars, 0 written); **`--apply` NOT run — needs user permission**; PLUS plan-only (user decision) for prod `backtest_history` gap → `.agents/docs/plan-backtest-history-prod-gap.md` + BUGS.md #11; on branch `feat/historical-price-sync`, commit pending; no deploy; v3.9.1 committed `9247a9f` + docs `2eaeef8` pushed to main)

---

## Current Project Status

### v3.10.0 — Historical-Price Sync into `daily_prices` (Swing Indicators "—" Fix) + `backtest_history` Prod-Gap Plan (Aug 14 2026) — ✅ CODE + TESTS VERIFIED, COMMIT IN PROGRESS
...
**Plan only (user decision 2026-08-14)**: MCP `getHistoricalData` 500 — `public.backtest_history` missing on prod → NEW `.agents/docs/plan-backtest-history-prod-gap.md` (options A apply-missing-migration recommended / B lazy CREATE TABLE / C daily_prices-first chain) + BUGS.md rows #11 (backtest gap, planned) + #12 (daily_prices gap, fix built).
**Verification**: NEW `lib/__tests__/historicalPriceSyncService.test.ts` — 15 tests (scope resolution incl. empty-explicit, dry-run no-write, apply multi-chunk >200 bars, per-symbol error tolerance, `maxDurationMs` guard, upsert SQL shape); **suite 653 pass** (was 638); `npx tsc --noEmit` 0 errors on touched files (71 = exact baseline). Local dry-run: `npx tsx --env-file=.env scripts/backfill-daily-prices.ts --symbols TCS --days 5` → 4 EQ bars fetched (window 09→14-08-2026), **0 written**, 0 errors, 0.8s.
- **Status**: docs updated (AGENTS.md v3.10.0 row, CHANGELOG index + versions-v3, TODO, BUGS.md, Primer, agent-memory, Lessons #69/#70, session-todos); committing on `feat/historical-price-sync`; **`--apply` (local or prod) NOT run — needs explicit user permission**; NO deploy.
