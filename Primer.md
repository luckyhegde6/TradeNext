...
## Last Updated
2026-08-15 (v3.11.2 Stale recs cache across module graphs FIX — `recommendationsCache` now a `globalThis` singleton: Next.js dev loads `instrumentation.ts` (worker/cron daemon) + API routes as SEPARATE module graphs → `lib/cache.ts` was evaluated TWICE → two independent NodeCache instances, so the worker's `invalidateRecommendationsCache()` flushed ITS copy while the route kept serving the stale 23h run ("Last updated: 14/8/2026" after v3.11.1 re-ran recs). Fix mirrors `lib/prisma.ts`: cache lives on `globalThis.__recommendationsCache`, every importer resolves the SAME instance; other caches unchanged (short TTLs, no cross-module invalidation). NEW `cacheSingleton.test.ts` (4 tests — `jest.resetModules()` re-require proves identity/cross-instance visibility/worker→route `flushAll`/shared keys); **suite 700 pass** (was 696); tsc 71 exact baseline (0 new); no UI change → no Playwright re-run. Commit pending (no push/deploy) — branch `fix/cron-tz-swing-perf`. Prior: v3.11.1 no-fake-HOLD fix (suite 696, Playwright-verified) + v3.11.0 daemon committed `6c4ef41` unpushed.)
