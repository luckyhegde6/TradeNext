...
## Last Updated
2026-09-04 (v3.28.2 — Lost-leader engine stop, single-active-worker enforcement, on top of v3.28.1 on branch `fix/v3.28.1-sqlite-self-heal`). Post-ship audit verified every persistence path (AI-call tracking → SQLite write-behind `server_log` + two-tier merge; Recommendations/Performance/IPO/Swing persist post-analysis; crons synced via `syncFromPrisma` pull + `reconcileControlToPrisma` 6h push). Remaining defect: `instrumentation.ts` `onLost` only logged — combined with fail-open `acquireLeaderLock` on DB-unavailable, multiple instances ran worker poll + cron daemon during a blip and the losers **kept polling forever** after the leader row was reclaimed. Fix: worker onLost → `stopWorkerEngine()`, cron onLost → `stopCronDaemon()`, sqlite-sync onLost stays log-only; NEW `instrumentation.test.ts` (5). tsc 46 = baseline; full suite 1003 pass / 4 skip / 1 fail (5 new; 1 = documented pre-existing `intelligence.test.ts` flake — excluding it 72 suites / 1003 / 4 / 0). Docs complete; **commit pending user**.
