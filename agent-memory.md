...
- **Status**: commits on `feat/historical-price-sync` — `b312de7` (feat: service + worker wiring + CLI + tests) + `4d49e13` (docs [skip ci]) — **pushed, PR #91 open**; suite 660 pass + tsc 71 baseline verified; local `--apply` done; **NO deploy** (user-managed); prod `--apply` NOT run — not needed (market-sync step 4 auto-backfills after deploy); prod DB write still requires explicit permission.
