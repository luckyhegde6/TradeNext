...
4. Prisma ORM attribution package install. → **RESOLVED + IMPLEMENTED as a SEPARATE increment (v3.21.3, not bundled)**: user approved the FULL OTel set + OTLP exporter. Installed `@prisma/instrumentation` (7.10.0) + `@opentelemetry/api sdk-trace-node resources semantic-conventions sdk-trace-base instrumentation context-async-hooks exporter-trace-otlp-http` (25 packages added). New `lib/otel.ts` `otelSetup()` — **strictly opt-in** (`PRISMA_OTEL_ENABLED=1` else no-op), `NodeTracerProvider` + `PrismaInstrumentation` + OTLP/HTTP exporter (console fallback), idempotent on globalThis; wired into `lib/prisma.ts` BEFORE the PrismaClient singleton. `.env.example` documents the env vars. NEW `lib/__tests__/otel.test.ts` (4 no-op guard tests). **Verified: tsc 46 = exact baseline (0 new); suite 945 pass / 4 skip (was 941/4, +4).** The quote-tiering bundle itself was committed + pushed as `7409616` (v3.21.2) on `feat/db-health-ops-visibility`; OTel increment commit pending user approval.