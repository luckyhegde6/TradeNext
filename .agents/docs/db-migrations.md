...
|-----------|------|---------------------------|----------------|
| `20260811150000_add_password_reset_request` | 2026-08-11 (v3.6.0) | `PasswordResetRequest` (map: `password_reset_requests`) + `status`/`email` indexes | Admin-moderated **password reset requests**: mirror of the `JoinRequest` moderation pattern so a forgotten-password user can request a reset and an admin approves/rejects it **without letting the requester choose their own password** (approval sets the `DEFAULT_PASSWORD` env value, bcrypt-hashed). One row per request, `status` lifecycle `pending → approved/rejected`, `email` NOT unique (a user could legitimately request again later, dedup is enforced only against *pending* rows at creation) — hence index on `[email]` and `[status]`, not a unique constraint. **Backfill:** none — table starts empty. **Rollback:** dropping the table loses only moderation history (no account data). **Verification:** delta generated via `migrate diff` (git-HEAD schema snapshot → current), full 33-migration replay on `tradenext_shadow` scratch DB passed. |
...
- **Auth/Users**: `User`, `UserSession`, `JoinRequest`, **`PasswordResetRequest` (v3.6.0)**, `VerificationToken` (via user fields)
