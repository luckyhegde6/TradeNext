...
| File | Contents |
|------|----------|
| [versions-v3.19.md](./changelog/versions-v3.19.md) | **v3.19.0–v3.19.2** — DB plan limit resilience (graceful degradation, op reduction, write budget guard, admin OTP, staticCache key fix) + SQLite backup layer (sql.js in-memory fallback, route fallback chains, Accelerate proxy error detection) + SQLite expanded schema (10 tables, 6 new: worker_status, server_log, audit_log, cron_job, cron_run, worker_task) + recovery sync (5-min background probe, auto-sync on Prisma recovery) + admin DB health dashboard (`/admin/utils/db-health`) |
