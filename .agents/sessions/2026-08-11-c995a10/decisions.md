...
## D26. DividendMonthView timezone bug: noon-UTC ex-dates landed one calendar day late in IST

**Decision:** Replace the UTC-derived date keys (`date.toISOString().split("T")[0]`) in `DividendMonthView.tsx` with an exported local-component helper `toLocalDateKey(date)` (`getFullYear/getMonth/getDate` padStart) used for BOTH dividend bucketing and grid-cell keying. Add `data-testid="cell-<localDateKey>"` per cell for the regression test.

- *Context:* User reported the `/dividends` calendar looked off while the summary cards showed `0 / ₹0 / ₹0 / —`. Root-caused in two parts: (1) the cards are CORRECT — all local corp-action ex-dates are Aug 10–11 and today is Aug 12, so zero future ex-dates exist locally (`view=upcoming` summary verified via API); (2) REAL BUG — the calendar grid displayed the Aug-10 dividends (9) on day 11 and Aug-11 dividends (10) on day 12. Cause: seed stores ex-dates at `12:00 UTC` (`parseDateCA` → `new Date(y,m,d)` local midnight); old code keyed dividends by UTC date while grid cells were keyed by UTC-converted LOCAL dates — in IST (+05:30), `new Date(2026,8,11).toISOString()` yields `2026-08-10` (18:30 UTC prev day), so dividends matched the WRONG (next-day) grid cell. Verified on dev :3000 before fix: +6 on day 11, +7 on day 12 (and a stale dev-server build even showed older data).
- *Why this approach:* Both sides of the map must derive keys from LOCAL calendar components — that makes `exDate(noon UTC)` and `new Date(y,m,d)` (midnight local) land on the SAME key. Export + testid make the regression guard possible. `DividendListView` uses `toLocaleDateString("en-IN")` (already correct — no change). Test asserts pre-fix code FAILS all 4 tests in IST (`process.env.TZ = "Asia/Kolkata"` pinned at top — jest workers run UTC by default where the shift never reproduces; verified: stash old code → 4 FAIL; restore fix → 4 PASS).
- *Impact:* `app/components/dividends/DividendMonthView.tsx` (+exported `toLocalDateKey`, `data-testid` on cells), NEW `app/components/dividends/__tests__/DividendMonthView.test.tsx` (4 tests: local-key formatting, midnight-grid vs noon-UTC key equality, 10th-vs-11th placement, multi-day grouping). Live re-verified :3000: day 10 = PTC/JIOFIN/MAJESAUT +6, day 11 = RATNAMANI/DVL/CASTROLIND +7, day 12 empty, cards still `0/₹0/₹0/—` (correct), 0 console errors. Suite **453 pass** (449 + 4), tsc clean on touched files. No deploy (pending user).

## D27. v3.6.3 — Direction-aware target/stop-loss evaluation (ITC SELL bug)

**Decision:** Build a pure, testable level evaluator `lib/services/recommendationLevelEvaluator.ts` (`evaluateRecommendationLevels`) that enforces direction invariants and corrects bad AI levels at normalization time, plus wire a mirror direction-aware target/SL comparison into the performance tracker.

- *Context:* User reported ITC showing **SELL ₹279, Target ₹306.9, Stop Loss ₹265.05** — a BUY-style layout on a SELL call (target above price, stop below). Root cause: `normalizeRecommendation` in `recommendation-agent.ts` used a direction-BLIND fallback (`target = price*1.1`, `stop = price*0.95`) whenever AI returned 0/missing; when the model returned contradictory levels they passed through untouched. Secondary bug: `dailyRecommendationService.checkRecommendationPerformance` compared `currentPrice >= targetPrice → target_achieved` / `currentPrice <= stopLoss → stop_loss_hit` — correct for BUY but INVERTED for SELL.
- *Why this approach:* One pure function = unit-testable without Prisma; drives BOTH normalization (`recommendation-agent.ts`) and the new backfill script (`scripts/backfill-recommendation-levels.ts`); invariant model (BUY target>price>stop, SELL target<price<stop, HOLD tight band) matches what a retail user expects. Defaults: BUY 1.10×/0.95×, SELL 0.90×/1.05× (direction sign −1), HOLD 1.05×/0.95×; bounds 0.3×–3× price; 2-dp rounding; `valid:false` + corrections[] when anything was replaced; returns raw unvalidated when price ≤ 0.
- *Impact:* NEW `lib/services/recommendationLevelEvaluator.ts`, NEW `lib/__tests__/recommendationLevelEvaluator.test.ts` (13 tests incl. ITC regression: target 306.9/SL 265.05 @ 279 SELL → corrected target < 279, stop > 279), NEW `scripts/backfill-recommendation-levels.ts` (idempotent; fixes persisted trackers), `recommendation-agent.ts` (normalizeRecommendation routes through evaluator + `logger.warn` on corrections — `DEFAULT_TARGET_MULTIPLIER`/`DEFAULT_STOP_LOSS_MULTIPLIER` constants kept since the service still imports them), `dailyRecommendationService.ts` (perf check: SELL `currentPrice <= targetPrice → target_achieved`, `currentPrice >= stopLoss → stop_loss_hit`; BUY/HOLD unchanged), 3 new agent tests (ITC correction, valid-SELL unchanged, BUY 0/0 defaults unchanged). **Suite 484 pass** (471 + 13), tsc clean on touched files, live-verified :3000. Placeholder tracker defaults (`price*1.2`/`0.95`) left as-is — they're HOLD placeholders overwritten by AI results in the same run.

## D28. v3.6.3 — Recommendations page layout: sidebar Sections nav + IPOs status sections

**Decision:** Convert the horizontal Recommendations tab strip to a vertical sidebar (sticky on desktop, horizontally scrolling on mobile) and rewrite the IPOs tab into status-grouped sections with separate Open/Close columns.

- *Context:* User asked for "sidenavs and side tabs so the breadcrumbing of the tabs will be easier", Current IPOs alongside Upcoming, separate Open/Close columns, and colour-coded status labels. The NSE IPO endpoint already returns all statuses (`Active:5, Forthcoming:1, Closed:2` live-verified via `curl /api/recommendations/ipos?refresh=1`) so this is pure UI. Also: cards showed the full 43-screener list ("screens like this should be usually hidden and only on click on the 43 screeners it should expand") and summary cards should appear only on Today's Picks.
- *Why this approach:* `flex lg:flex-row` with `<aside class="lg:w-56"><nav class="lg:sticky lg:top-24">` keeps desktop breadcrumb cheap and mobile scrollable; status sections use fixed order Active → Forthcoming → Closed with per-status emerald/amber/gray pills + emerald-tinted open rows (`bg-emerald-500/[0.06] hover:[0.12]`); screener chips collapsed to `MAX_VISIBLE_SCREENERS=3` + "+N more" with an expand toggle; summary cards gated to `activeTab === "picks"`.
- *Impact:* `app/recommendations/page.tsx` (layout + summary gating + 📈 header icon), `app/components/recommendations/IposTab.tsx` (rewritten; Open/Close columns, section dividers, StatusPill), `app/components/recommendations/RecommendationCard.tsx` (`screenerExpanded`, `MAX_VISIBLE_SCREENERS=3`), `app/components/recommendations/DailyPicksTab.tsx` (grid `md:2 xl:3` for the sidebar). Live-verified :3000 desktop + mobile (375px): sidebar SECTIONS, summary cards only on Picks, IPO sections/colours, 43-screeners expand/collapse, 0 console errors.
